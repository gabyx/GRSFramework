  ...
  void setupBodiesLinear(
     ...
     double dist, bool jitter, double delta)
   {
     ...
     for(unsigned int i=0;i< init_state.m_nSimBodies;i++){
       init_state.m_SimBodyStates[i].m_q.template tail<4>() = TLayoutConfig::Quaternion(1,0,0,0);
...
template<typename TLayoutConfig>
void setupBodyPositionAxisAngle(RigidBodyState<TLayoutConfig> & rigibodyState, typename const TLayoutConfig::Vector3 & pos, typename TLayoutConfig::Vector3 & axis, typename TLayoutConfig::PREC angleRadian){
