...
ContactGraph::ContactGraph(boost::shared_ptr<DynamicsSystemType> pDynSys):
    m_nodeCounter(0),
    m_edgeCounter(0),
    m_nLambdas(0),
    m_nFrictionParams(0),
...
void ContactGraph::setInclusionCommunicator(boost::shared_ptr<InclusionCommunicatorType> pInclusionComm) {
...
void ContactGraph::setLog(Logging::Log * solverLog) {
    m_pSolverLog = solverLog;
}

void ContactGraph::clearGraph() {
...
void ContactGraph::addNode(CollisionData * pCollData) {
...
std::pair<typename ContactGraph::SplitBodyNodeDataType *, bool> ContactGraph::addSplitBodyNode(RigidBodyType * body, const RankIdType & rank) {
...
void ContactGraph::computeParams(NodeDataType & nodeData) {
