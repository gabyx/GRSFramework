...
void SimulationState::enter()
{
    ...
    m_pSceneMgr = std::shared_ptr<Ogre::SceneManager>(
        RenderContext::getSingleton().m_pRoot->createSceneManager(ST_GENERIC, "SimulationStateSceneMgr"),
    ...
    m_pTrayMgr = std::shared_ptr<OgreBites::SdkTrayManager>(
        new OgreBites::SdkTrayManager("SimulationStateTray",
...
void SimulationState::setupScene()
{
    ...
    m_pOrbitCamera = std::shared_ptr<OrbitCamera>(
        new OrbitCamera(m_pSceneMgr.get(), "SimulationState::OrbitCam", 0.13, 50, 200, 0, M_PI / 4));
...
void SimulationState::updateParamsPanelSimulation()
{
    static std::stringstream GlobalTimeText, DynamicsThreadTimeText, SimTimeText, DelayText, TimeFactorText,
        AverageIterTimeText, MaxIterTimeText, NContactsText;
    static double Realtime, SimTime, AverageIterTime, MaxIterTime,
        NContacts;  // These variables are not acces by another thread!! only for the function updateTimerText !
