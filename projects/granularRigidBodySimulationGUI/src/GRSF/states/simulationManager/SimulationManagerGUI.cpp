...
SimulationManagerGUI::SimulationManagerGUI(std::shared_ptr<Ogre::SceneManager> pSceneMgr) : SimulationManagerBase()
{
...
void SimulationManagerGUI::setup(boost::filesystem::path sceneFilePath)
{
    ...
    m_pSceneParser = std::shared_ptr<SceneParserType>(
        new SceneParserType(c, m_pSimulationLog, ApplicationCLOptions::getSingleton().getMediaDir()));
    ...
    m_pSharedBuffer = std::shared_ptr<SharedBufferDynSys>(
        new SharedBufferDynSys(m_pDynSys->m_simBodies.beginKey(), m_pDynSys->m_simBodies.endKey()));
    ...
    m_dynCoordFrame.reserveCoordinateSystems(m_pDynSys->m_simBodies.size() *
                                             3);  // Approx. 3 contacts per body as init guess
    m_dynCoordFrame.addToScene(
        m_pDynSys->m_pContactFramesNode, "ContactFrameXAxis", "ContactFrameYAxis", "ContactFrameZAxis");
...
bool SimulationManagerGUI::writeInitialState()
{
    ...
    boost::filesystem::path file =
        FileManager::getSingleton().getNewSimFolderPath(SIMULATION_FOLDER_PATH, SIM_FOLDER_PREFIX_INIT);
    ...
    m_pSimulationLog->logMessage(std::string("Error while trying to open and write initial state file to:") +
                                 file.string());
    return false;
}
...
void SimulationManagerGUI::initSimThread()
{
    ...
    // Get new folder path
    m_SimFolderPath =
        FileManager::getSingleton().getNewSimFolderPath(SIMULATION_FOLDER_PATH, SIM_FOLDER_PREFIX_REALTIME);
...
bool SimulationManagerGUI::initRecordThread()
{
    ...
    {
        fileOK = m_pStateRecorder->createSimFileCopyFromReference(m_SimFilePath,
                                                                  m_pTimestepper->m_settings.m_simStateReferenceFile);
    }
    else
    {
    ...
    if (!m_pTimestepper->m_settings.m_simDataReferenceFile.empty())
    {
        ...
        simDataFile = FileManager::getSingleton().copyFile(
            m_pSceneParser->getParsedSceneFile(), m_pTimestepper->m_settings.m_simDataReferenceFile, true);
    }
...
void SimulationManagerGUI::startSimThread(Threads threadToStart)
{
    ...
    if (!isSimThreadRunning())
    {
        ...
        if (threadToStart & SimulationManagerBase::REALTIME)
        {
            ...
            m_pSimulationLog->logMessage(
                "---> SimulationManagerGUI:: Start Thread: SimThread in REALTIME mode started ...");
        }
        else if (threadToStart & SimulationManagerBase::RECORD)
        {
            ...
            m_pSimulationLog->logMessage(
                "---> SimulationManagerGUI:: Start Thread: SimThread in RECORD mode started ...");
        }
    }
    else
    {
...
void SimulationManagerGUI::stopSimThread(Threads threadToStop, bool force_stop)
{
    if (isSimThreadRunning())
    {
        ...
        else
        {
            ...
            {
                m_pSimulationLog->logMessage(
                    "---> SimulationManagerGUI:: Stop Thread: \n To stop the thread which is running in REALTIME mode, "
                    "you need to switch to REALTIME mode to stop it!");
            }
            else
            {
                m_pSimulationLog->logMessage(
                    "---> SimulationManagerGUI:: Stop Thread: \n To stop the thread which is running in RECORD mode, "
                    "you need to switch to RECORD mode to stop it!");
            }
        }
    }
    else
