...
PlaybackManager::PlaybackManager(std::shared_ptr<Ogre::SceneManager> pSceneMgr)
    : PlaybackManagerBase(), m_bSetupSuccessful(false)
{
...
bool PlaybackManager::setup()
{
    ...
    m_pSharedBuffer = std::shared_ptr<SharedBufferPlayback>(
        new SharedBufferPlayback(m_pDynSys->m_SceneNodeSimBodies.beginKey(), m_pDynSys->m_SceneNodeSimBodies.endKey()));
...
bool PlaybackManager::parseScene()
{
    ...
    LOG(m_pSimulationLog,
        "---> Scene parsing finshed: Added " << m_pDynSys->m_SceneNodeSimBodies.size() << " simulated & "
                                             << m_pDynSys->m_SceneNodeBodies.size()
                                             << " static bodies! "
                                             << std::endl;);
...
void PlaybackManager::initBeforeThreads()
{
    ...
    {
        logstream << "PlaybackManager: Drop Video: "
                  << "true" << std::endl;
    }
    else
    {
    ...
    {
        logstream << "PlaybackManager: Drop Sim File: "
                  << "true" << std::endl;
    }
    else
    {
...
void PlaybackManager::threadRunSimulation()
{
    ...
    //    }else{
    LOG(m_pThreadLog,
        "---> PlaybackManager: Advanced sim buffer to first state @ m_t: " << currentState->m_t << std::endl;);
    //    }
    ...
    while (!isSimThreadToBeStopped())
    {
        ...
        if (m_settingsSimThread.m_bVideoDrop)
        {
            ...
            // Make one step!
            currentState =
                m_pSharedBuffer->advanceSimBuffer(bchangedState);  // Will move to the initial state at the beginning!
            if (bchangedState)
            {
                state_time = currentState->m_t;
                deltaT     = state_time - old_state_time;  // Will be zero at the beginning

                m_pVideoDropper->addToCurrentTime(
                    deltaT);  // if deltaT==0 or if more than one frames fits into deltaT, toggle dropping!

                m_pVideoDropper->tryToWaitForFrameDrop();  // if toggled frame drop, wait for the drop, which happens in
                                                           // the vis thread!!
            }
        }
        else
        ...
        {
            if (m_settingsSimThread.m_bSimFileDrop)
            {  // When the sim buffer has been moved, then ... At the biginning,
                // this moves to the initial state basically!
                // Do a resampling of the states!
                LOG(m_pThreadLog,
                    "---> PlaybackManager: Try to resample state time: " << currentState->m_t << std::endl;);
            ...
            {
                LOG(m_pThreadLog,
                    "---> PlaybackManager: Detected end state at " << currentState->m_t << " --> leaving..."
                                                                   << std::endl;);
                break;
            }
...
void PlaybackManager::initSimThread()
{
    ...
    if (m_settingsSimThread.m_bVideoDrop)
    {
        ...
        // Get new folder path
        boost::filesystem::path videoFolderPath =
            FileManager::getSingleton().getNewSimFolderPath(VIDEO_FOLDER_PATH, VIDEO_FOLDER_PREFIX);
    ...
    else
    {
        ...
        m_pFileLoader->setReadFullState(true);  // This makes things faster, because of buffering i think, first thought
                                                // "false" would give better results which does not!
    }
}
