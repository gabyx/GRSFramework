...
void PlaybackState::enter()
{
    ...
    m_pSceneMgr = std::shared_ptr<Ogre::SceneManager>(
        RenderContext::getSingleton().m_pRoot->createSceneManager(ST_GENERIC, "PlaybackStateSceneMgr"),
        OgreSceneManagerDeleter());
    ...
    m_pTrayMgr = std::shared_ptr<OgreBites::SdkTrayManager>(
        new OgreBites::SdkTrayManager("PlaybackStateTray",
                                      RenderContext::getSingleton().m_pRenderWnd,
                                      ::InputContext::getSingleton().getInputContext(),
                                      this));
    ...
    // If no scene has been automatically loaded from the panel, and therefore no playback manager is instantiated, ->
    // change the scene!
    if (!m_pPlaybackMgr)
    {
        changeScene();
...
void PlaybackState::setupActiveModeSelection()
{
    ...
    m_pActiveModeSelectMenu = (m_pTrayMgr->createThickSelectMenu(
        TL_TOPRIGHT, "ActiveModeSelectionPlayback", "Simulations Mode", 200, 3, items));
}
...
void PlaybackState::setupGUI()
{
    ...
    m_pTrayMgr->hideBackdrop();
    m_pPlaybackFiles =
        (m_pTrayMgr->createThickSelectMenu(TL_TOP, "PlaybackFiles", "Playback Files", 600, 10, Ogre::StringVector()));
    updatePlaybackPanel();
    ...
    m_pCheckBoxSimFileInterpolate =
        m_pTrayMgr->createCheckBox(TL_BOTTOMRIGHT, "CheckBoxSimFileInterpolate", "Drop SimFile: Interpolate", 300);
    ...
    m_pSliderStartTime =
        m_pTrayMgr->createLongSlider(TL_BOTTOMRIGHT, "SliderStart", "Start Time", 500, 70, 0, 5, 10000);
...
void PlaybackState::changeScene()
{
    ...
    // Load new scene
    m_pPlaybackMgr = std::shared_ptr<PlaybackManager>(
        new PlaybackManager(m_pSceneMgr));  // overwrite the existing, which gets deleted!
...
void PlaybackState::switchSimulationMode()
{
    m_eSimulationActiveMode =
        static_cast<ActiveMode>(((int)m_eSimulationActiveMode + 1) % m_pActiveModeSelectMenu->getNumItems());
    // Set active in Menu
...
void PlaybackState::setupScene()
{
    ...
    m_pOrbitCamera = std::shared_ptr<OrbitCamera>(
        new OrbitCamera(m_pSceneMgr.get(), "PlaybackState::OrbitCam", 0.13, 150, 200, 0, M_PI / 4));
    ...
    // Add a fancy skybox :-)
    // m_pSceneMgr->setSkyBox(true,
    // "NoCloudsSkyBox",5000,true,Ogre::Quaternion(Ogre::Radian(M_PI/2),Ogre::Vector3(1,0,0)),ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME
    // );
}
...
{
    std::shared_ptr<RenderAppState> appState = this->findAppStateByName("SimulationState");
    if (appState)
    {
...
{
    static std::stringstream GlobalTimeText, DynamicsThreadTimeText, SimTimeText, DelayText, TimeFactorText,
        AverageIterTimeText, MaxIterTimeText;
    static double Realtime,
        SimTime;  // These variables are not acces by another thread!! only for the function updateTimerText !
    ...
    if (iterations % 50 == 0)
    {
        ...
        GlobalTimeText << std::setw(10) << std::fixed << std::setprecision(4)
                       << m_pTimelineRendering->getMilliseconds() * 1e-3;
