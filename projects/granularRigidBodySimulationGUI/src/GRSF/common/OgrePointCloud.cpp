    ...
    for (uint32_t current_point = 0; current_point < num_points; ++current_point)
    {
        ...
        while (!rend || current_vertex_count >= buffer_size)
        {
            if (rend)
            {
                ...
                GRSF_ASSERTMSG(
                    (op->vertexData->vertexCount + op->vertexData->vertexStart <= rend->getBuffer()->getNumVertices()),
                    "");
                vbuf->unlock();
...
{
    OgrePointCloudRenderablePtr rend(
        new OgrePointCloudRenderable(this, num_points, !current_mode_supports_geometry_shader_));
    ...
    : parent_(parent)
{
    ...
    Ogre::HardwareVertexBufferSharedPtr vbuf = Ogre::HardwareBufferManager::getSingleton().createVertexBuffer(
        mRenderOp.vertexData->vertexDeclaration->getVertexSize(0), num_points, Ogre::HardwareBuffer::HBU_DYNAMIC);
