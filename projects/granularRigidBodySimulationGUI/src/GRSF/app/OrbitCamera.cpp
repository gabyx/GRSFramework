...
void OrbitCamera::moveOrbitToNode(SceneNode* const Obj)
{
    ...
    Quaternion q_Orbit_Cam = Quaternion(Radian((Ogre::Real)theta), Vector3(0, 1, 0)) *
                             Quaternion(Radian((Ogre::Real)M_PI / 2), Vector3(0, 0, 1));
...
bool OrbitCamera::mouseMoved(const OIS::MouseEvent& e)
{
    ...
    if (e.state.buttonDown(OIS::MB_Left) && ::InputContext::getSingleton().getKeyboard()->isKeyDown(OIS::KC_LSHIFT))
    {
        m_pCamNode->translate(Vector3(0, 0, -1) * m_Move *
                                  (Ogre::Real)::InputContext::getSingleton().getMouse()->getMouseState().Y.rel / 100.0,
                              Node::TS_LOCAL);
    }
