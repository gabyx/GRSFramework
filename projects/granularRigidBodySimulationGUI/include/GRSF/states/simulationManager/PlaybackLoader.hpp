...
PlaybackLoader<TStatePool>::PlaybackLoader(const unsigned int nSimBodies, std::shared_ptr<TStatePool> pStatePool)
    : m_barrier_start(2), m_nSimBodies(nSimBodies)
{
    // Set the Log Output =========================================================================
    ...
    while (!isLoaderThreadToBeStopped() && current_state != EXIT)
    {
        if (current_state == LOAD_NEXT_FILE)
        {
            ...
            if (m_currentFileIndex != m_simFileList.size() - 1)
            {
                ...
                {
                    LOG(m_pThreadLog,
                        "---> SimFile: " << m_currentFileIndex + 1 << "/" << m_simFileList.size() << ":"
                                         << *m_currentFileIt
                                         << " loaded: Number of States = "
                                         << m_binarySimFile.getNStates()
                                         << std::endl;);
        ...
        else if (current_state == READ_IN)
        {
            ...
            if (i % 20 == 0)
            {
                LOG(m_pThreadLog, "---> File loader buffering (i: " << i << ") @ m_t: " << m_state->m_t << std::endl);
            }
            i++;
    ...
    if (boost::filesystem::exists(*m_currentFileIt))
    {
        if (is_regular_file(*m_currentFileIt))
        {
            if (!boost::filesystem::is_empty(*m_currentFileIt))
            {
                ...
                {
                    LOG(m_pThreadLog,
                        "PlaybackLoader:: Could not open file: " << m_currentFileIt->string() << std::endl
                                                                 << "---> File errors: "
                                                                 << std::endl
                                                                 << m_binarySimFile.getErrorString());
                }
            }
            else
            {
