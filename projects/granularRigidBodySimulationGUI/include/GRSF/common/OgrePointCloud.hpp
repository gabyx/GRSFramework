 ...
 *
 * Displays a set of points using any number of Ogre BillboardSets.  OgrePointCloud is optimized for sets of points that
 * change
 * rapidly, rather than for large clouds that never change.
 *
 * Most of the functions in OgrePointCloud are not safe to call from any thread but the render thread.  Exceptions are
 * clear() and addPoints(), which
