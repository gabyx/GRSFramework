...
void SimulationManagerMPI::setup(boost::filesystem::path sceneFilePath)
{
    ...
    m_sceneFileParsed =
        FileManager::getSingleton().copyFile(sceneFilePath, FileManager::getSingleton().getLocalDirectoryPath());
    ...
    else
    {
        ...
        m_pSimulationLog =
            Logging::LogManager::getSingleton().createLog("SimulationLog", SIMULATION_LOG_TO_CONSOLE, true, filePath);
    }
    ...
    m_pProcCommunicator = std::shared_ptr<ProcessCommunicatorType>(new ProcessCommunicatorType(
        MPILayer::MPIGlobalCommunicators::getSingleton().getCommunicator(MPILayer::MPICommunicatorId::SIM_COMM)));
    ...
    // Parse the SceneSettings and Static Bodies only ==========================
    {
        ...
        using SceneParserType1 =
            SceneParserMPI<DynamicsSystemType, DynamicsSystemType::ParserModulesCreator1::SceneParserTraits>;
        ...
        LOG(m_pSimulationLog,
            "---> Scene parsing finshed: Added " << m_pDynSys->m_simBodies.size() << " simulated & "
                                                 << m_pDynSys->m_staticBodies.size()
                                                 << " static bodies! "
                                                 << std::endl;);
        LOG(m_pSimulationLog,
            "---> SimulationManagerMPI:: Global Bodies in Simulation " << m_nGlobalSimBodies << " Bodies!"
                                                                       << std::endl;);
    }
    // ====================================================
    ...
    if (topoSettings.m_type == MPILayer::TopologyBuilderEnum::GRIDBUILDER)
    {
        m_pTopologyBuilder = std::shared_ptr<TopologyBuilderType>(
            new MPILayer::GridTopologyBuilder<ProcessCommunicatorType>(m_pDynSys,
                                                                       m_pProcCommunicator,
    ...
    {
        m_pTopologyBuilder = std::shared_ptr<TopologyBuilderType>(
            new MPILayer::KdTreeTopologyBuilder<ProcessCommunicatorType>(m_pDynSys,
                                                                         m_pProcCommunicator,
                                                                         topoSettings.m_rebuildSettings,
                                                                         topoSettings.m_massPointPredSettings,
                                                                         topoSettings.m_globalOutlierFilterSettings,
    ...
    // m_pStateRecorder  =  std::shared_ptr<StateRecorderType >(new
    // StateRecorderType(true,m_pProcCommunicator->getProcInfo()->getRank()));

    m_pStateRecorder = std::shared_ptr<StateRecorderType>(new StateRecorderType(
        m_nGlobalSimBodies, std::static_pointer_cast<ProcessCommunicatorType::ProcessInfoType>(m_pProcCommunicator)));
    ...
    m_pTimestepper =
        std::shared_ptr<TimeStepperType>(new TimeStepperType(m_pDynSys, m_pProcCommunicator, m_pTopologyBuilder));
    ...
    ApplicationSignalHandler::getSingleton().registerCallback(
        {SIGINT, SIGUSR2}, callBack, "SimulationManagerMPI::gracefullyExit");
...
{
    LOG(m_pSimulationLog,
        "---> SimulationManagerMPI:: Caught signal: " << signal
                                                      << " close file collectively (deadlock if not all cooperate!) "
                                                      << std::endl)
...
void SimulationManagerMPI::checkOwnerOfBodiesInProcess()
{
    for (auto& body : m_pDynSys->m_simBodies)
    {
        ...
        {
            GRSF_ERRORMSG("Body with id: " << body->m_id << " belongs to rank: " << r << "instead of: "
                                           << m_pProcCommunicator->getRank());
        }
    }
...
void SimulationManagerMPI::checkNumberOfBodiesInProcess()
{
    ...
    MPI_Reduce(&nSimBodies,
               &numberOfBodiesInSimulation,
               1,
               MPI_INT,
               MPI_SUM,
               m_pProcCommunicator->getMasterRank(),
               m_pProcCommunicator->getCommunicator());
    ...
    if (m_pProcCommunicator->hasMasterRank())
    {
        ...
        {
            LOG(m_pSimulationLog,
                "ERROR:---> Body check NOT successful! : " << numberOfBodiesInSimulation << "!=" << m_nGlobalSimBodies
                                                           << std::endl;);
            GRSF_ERRORMSG("Body check Not successfull");
        }

        LOG(m_pSimulationLog,
            "---> Body check successfull!: " << numberOfBodiesInSimulation << " SimBodies in Simulation" << std::endl;);
    }
    else
    {
...
void SimulationManagerMPI::startSim()
{
    ...
    while (1)
    {
        ...
        bool rebuild =
            m_pTopologyBuilder->checkAndRebuild(m_pTimestepper->getIterationCount(), m_pTimestepper->getTimeCurrent());
        if (rebuild)
        {
            // Reload Scene
        ...
        if (m_pTimestepper->finished())
        {
            ...
            LOG(m_pSimulationLog,
                "---> SimulationManager: Timestepper finished, needed: " << runtime << " sec. , exit..." << std::endl);
            break;
        }
...
void SimulationManagerMPI::initSim()
{
    ...
    {  // Only master process
        ...
        m_SimFolderPath =
            FileManager::getSingleton().getNewSimFolderPath(SIMULATION_FOLDER_PATH, SIM_FOLDER_PREFIX_RECORD);
    ...
    if (!m_pStateRecorder->createSimFile(true))
    {  // Truncate the files, important!
...
void SimulationManagerMPI::reloadScene()
{
    ...
    // Parse the scene objects from the XML, but selectively on the ids in m_pDynSys->bodiesInitStates
    {
        ...
        using SceneParserType2 =
            SceneParserMPI<DynamicsSystemType, DynamicsSystemType::ParserModulesCreator2::SceneParserTraits>;
