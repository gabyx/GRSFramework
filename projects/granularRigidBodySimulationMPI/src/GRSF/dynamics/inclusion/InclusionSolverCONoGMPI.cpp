...
InclusionSolverCONoGMPI::~InclusionSolverCONoGMPI()
{
    ...
    if (m_pSorProxStepSplitNodeVisitor != nullptr)
    {
...
void InclusionSolverCONoGMPI::reset()
{
    ...
    {
        delete m_pSorProxStepSplitNodeVisitor;
        m_pSorProxStepSplitNodeVisitor = nullptr;
    }
    ...
    if (m_settings.m_eMethod == InclusionSolverSettingsType::Method::SOR_CONTACT)
    {
        ...
        m_pContactSorProxStepNodeVisitor = new ContactSorProxStepNodeVisitor<ContactGraphType>(
            m_settings, m_bConverged, m_globalIterationCounter, m_pContactGraph);
    }
    ...
    m_pSorProxStepSplitNodeVisitor =
        new SorProxStepSplitNodeVisitor<ContactGraphType>(m_settings, m_bConverged, m_globalIterationCounter);
...
void InclusionSolverCONoGMPI::integrateAllBodyVelocities()
{
    for (auto bodyIt = m_simBodies.begin(); bodyIt != m_simBodies.end(); bodyIt++)
    {
        ...
        (*bodyIt)->m_pSolverData->m_uBuffer.m_front +=
            (*bodyIt)->m_pSolverData->m_uBuffer.m_back +
...
void InclusionSolverCONoGMPI::initContactGraphForIteration(PREC alpha)
{
    ...
    for (auto* body : m_simBodies)
    {
        ...
        body->m_pSolverData->m_uBuffer.m_front +=
            body->m_pSolverData->m_uBuffer.m_back +
...
void InclusionSolverCONoGMPI::sorProxOverAllNodes()
{
    ...
    {
        doConvergenceCheck =
            m_globalIterationCounter % (m_settings.m_convergenceCheckRatio * m_settings.m_splitNodeUpdateRatio) == 0 &&
            m_globalIterationCounter >= m_settings.m_MinIter;
    }
    ...
    if (doConvergenceCheck)
    {
            ...
            m_settings.m_eConvergenceMethod == InclusionSolverSettingsType::InEnergyVelocity)
        {
            ...
            {
                body->m_pSolverData->m_uBuffer.m_back =
                    body->m_pSolverData->m_uBuffer.m_front;  // Used for cancel criteria
            }
            for (auto& body : remotesWithContacts)
            {
                body->m_pSolverData->m_uBuffer.m_back =
...

    {
        ...
        if (m_nLocalNodes || m_nRemoteNodes)
        {
            switch (m_settings.m_eMethod)
            {
                ...
                case InclusionSolverSettingsType::Method::SOR_CONTACT:
                    ...
                    break;
                default:
                    GRSF_ERRORMSG(" Visitor method not defined for visitor: "
                                  << EnumConversion::toIntegral(m_settings.m_eMethod));
            }
        }
        ...
        m_avgIterTimeProxLocalRemoteNodes =
            (time + m_avgIterTimeProxLocalRemoteNodes * m_globalIterationCounter) / (m_globalIterationCounter + 1);
#endif
    }
...
// Safety test if all updates have been received!
        ...
        m_avgIterTimeProxSplitNodes =
            (time + m_avgIterTimeProxSplitNodes * m_globalIterationCounter) / (m_globalIterationCounter + 1);
#endif
    }
    ...
    if (doConvergenceCheck)
    {
        ...
        if (m_settings.m_eConvergenceMethod == InclusionSolverSettingsType::InVelocity)
        {
            ...
            {
                // std::cout << "before Criteria"<<std::endl;//std::cout <<"new "<<
                // (*it)->first->m_pSolverData->m_uBuffer.m_front.transpose() << std::endl; //std::cout <<"old "<<
                // (*it)->first->m_pSolverData->m_uBuffer.m_back.transpose() << std::endl;
            ...
            LOGSLLEVEL2_CONTACT(m_pSolverLog,
                                "---> Convergence criteria (InVelocity) converged: " << m_bConverged << std::endl;);
        }
        else if (m_settings.m_eConvergenceMethod == InclusionSolverSettingsType::InEnergyVelocity)
        {
            ...
            LOGSLLEVEL2_CONTACT(
                m_pSolverLog, "---> Convergence criteria (InEnergyVelocity) converged: " << m_bConverged << std::endl;);
        }
        else
        {
            GRSF_ERRORMSG("This cancelation criteria has not been implemented")
...
{
    std::stringstream s;
    s << std::fixed << m_bUsedGPU << "\t" << m_nContacts << "\t" << m_nLocalNodes << "\t" << m_nRemoteNodes << "\t"
      << m_nSplitBodyNodes << "\t" << m_globalIterationCounter << "\t" << m_bConverged << "\t" << m_isFinite << "\t"
      << m_timeProx << "\t" << m_proxIterationTime << "\t" << m_pDynSys->m_currentTotEnergy << "\t"
      << m_pDynSys->m_currentPotEnergy << "\t" << m_pDynSys->m_currentKinEnergy << "\t"
      << m_pDynSys->m_currentRotKinEnergy << "\t" << m_pDynSys->m_currentSpinNorm << "\t"
      << m_avgIterTimeProxLocalRemoteNodes << "\t" << m_avgIterTimeProxSplitNodes;
    return s.str();
}
