...
MultiBodySimFileMPI::MultiBodySimFileMPI()
{
    ...
    m_writebuffer.reserve(
        4000 * ((LayoutConfigType::LayoutType::NDOFqBody + LayoutConfigType::LayoutType::NDOFuBody) * sizeof(double) +
                1 * sizeof(RigidBodyIdType)) +
        1 * sizeof(double));  // t + 4000*(id,q,u)
}
                                           ...
                                           const typename DynamicsSystemType::RigidBodySimContainerType& bodyList)
{
    ...
    boost::iostreams::stream<boost::iostreams::back_insert_device<std::vector<char>>> stream(
        ins);  // is initialized second
    boost::archive::binary_oarchive oa(stream,
                                       boost::archive::no_codecvt | boost::archive::no_header);  // is initialized third
    ...
    GRSF_ASSERTMSG(
        (bodyList.size() == 0 && m_writebuffer.size() == 0) || (bodyList.size() != 0 && m_writebuffer.size() != 0),
        "m_writebuffer.size()" << m_writebuffer.size());

    // bytes need to be a multiple of the bytes for one body state
    GRSF_ASSERTMSG(m_writebuffer.size() % (m_nBytesPerBody) == 0,
                   m_writebuffer.size() << " bytes not a multiple of " << m_nBytesPerBody << " bytes");
    ...
    err = MPI_File_write_ordered(
        m_file_handle, const_cast<void*>((const void*)&m_writebuffer[0]), nBodies, m_bodyStateTypeMPI, &s);
                                         ...
                                         const typename DynamicsSystemType::RigidBodySimContainerType& bodyList)
{
    ...
    if (nBodies != 0)
    {
        ...
        boost::archive::binary_oarchive oa(
            stream, boost::archive::no_codecvt | boost::archive::no_header);  // is initialized third
                                          ...
                                          const typename DynamicsSystemType::RigidBodySimContainerType& bodyList)
{
    ...
    ASSERTMPIERROR(err, " write states");
    std::cout << "Rank: " << m_rank << " Size of buffer: " << m_writebuffer.size() << " Offset: " << bodyOffset
              << std::endl;
    err = MPI_File_write_at_all(
        m_file_handle, offsetMPI, const_cast<void*>((const void*)(&m_writebuffer[0])), nBodies, m_bodyStateTypeMPI, &s);
    ASSERTMPIERROR(err, " seek new state");
                                    ...
                                    bool                           truncate)
{
    ...
    else
    {
        int err = MPI_File_open(
            m_comm, file_path_c, MPI_MODE_APPEND | MPI_MODE_CREATE | MPI_MODE_RDWR, MPI_INFO_NULL, &m_file_handle);
        if (!mpiSucceded(err))
        {
            return false;
        }
    ...
    // int err = MPI_File_set_view(m_file_handle, (MPI_Offset)m_headerLength,
    // m_bodyStateTypeMPI,m_bodyStateTypeMPI,type,MPI_INFO_NULL);
