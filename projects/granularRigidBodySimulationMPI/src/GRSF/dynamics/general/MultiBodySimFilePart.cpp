                                     ...
                                     bool truncate)
{
    ...
    else
    {
        ...
        // We could make sure that the position is at the end of one state (unknown # bodies) by checking header for a
        // location
...
void MultiBodySimFilePart::writeHeader()
{
    ...
    *this << (unsigned int)SIM_FILE_PART_VERSION << (unsigned int)m_nStates  // is written at the end!
          << (unsigned int)m_nDOFqBody << (unsigned int)m_nDOFuBody << (unsigned int)m_additionalBytesPerBodyType
