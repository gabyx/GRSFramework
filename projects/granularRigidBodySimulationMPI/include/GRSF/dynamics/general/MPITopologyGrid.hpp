...
public:
    ...
    RankIdType getCellRank(const Vector3& I_point) const
    {
        ...
        if (m_axisAligned)
        {
            v = Base::template getCellIndexClosest<false>(
                I_point);  // do not transform point (grid aligned with I system)
        }
        else
        {
    ...
    NeighbourRanksListType getCellNeighbours(RankIdType cellRank) const
    {
        ...
        for (int i = 0; i < 26; i++)
        {
            ...
            if ((ind(0) >= 0 && ind(0) < m_dim(0)) && (ind(1) >= 0 && ind(1) < m_dim(1)) &&
                (ind(2) >= 0 && ind(2) < m_dim(2)))
            {
                // Add neighbour
                auto res = v.insert(getCellRank(ind));
                GRSF_ASSERTMSG(res.second,
                               "This neighbour number: " << getCellRank(ind) << " for cell number: " << cellRank
                                                         << " alreads exists!");
            }
        }
        return v;
