...
class NeighbourMessageWrapperBodies
    : public boost::serialization::traits<NeighbourMessageWrapperBodies<TNeighbourCommunicator>,
...
public:
    ...
    */
    ...
    {
        LOGASSERTMSG(m_initialized,
                     m_pSerializerLog,
                     "The NeighbourMessageWrapperBodies is not correctly initialized, Rank not set!");
        ...
        */
        ...
        if (!m_hasNoSimBodies)
        {
            ...
            LOGASSERTMSG(m_neighbourData,
                         m_pSerializerLog,
                         "There exists no NeighbourData for neighbourRank: " << m_neighbourRank << "in process rank: "
                                                                             << m_nc->m_rank
                                                                             << "!");
            ...
            {
                it_next++;  // take care here, because, we might invalidate this iterator it because we might remove
                            // local bodies, thats why it_next is used above!
    ...
    void load(Archive& ar, const unsigned int version) const
    {
        ...
        LOGASSERTMSG(m_nc->m_currentSimTime == simulationTime,
                     ...
                     "The message from rank: " << m_neighbourRank << " has timeStamp: " << simulationTime
                                               << " which does not fit our current simulation Time: "
                                               << m_nc->m_currentSimTime
                                               << " for rank: "
                                               << m_nc->m_rank
                                               << " !")
            ...
            if (size > 0)
            {
                LOGBC_SZ(m_pSerializerLog,
                         "BodyComm=================================================================================="
                             << std::endl;)
                LOGBC_SZ(m_pSerializerLog,
                         "DESERIALIZE Message from neighbour rank: " << m_neighbourRank << std::endl;);
...
private:
    ...
    void saveRemoval(Archive& ar, RigidBodyType* body) const
    {
        ...
        LOGASSERTMSG(res,
                     m_pSerializerLog,
                     "Could not delete local body with id: " << RigidBodyId::getBodyIdString(body)
        ...
        {
            LOGASSERTMSG(it->second.m_overlaps == false,
                         m_pSerializerLog,
                         "Body with id: " << RigidBodyId::getBodyIdString(body)
                                          << " overlaps (m_overlaps = true) neighbour rank: "
                                          << m_neighbourRank
                                          << " which should not because we have send a removal!");
        }
    ...
    void saveNotificationOrUpdate(Archive& ar, RigidBodyType* body, const SubMessageFlag& flag) const
    {
        ...
        if (m_bodyInfo->m_ownerRank == m_neighbourRank)
        {  // the body belongs now to m_neighbourRank
            ...
            {  // if it still overlaps our rank
                // Move local body to remote global list
                LOGBC_SZ(m_pSerializerLog,
                         "-----> Changing LOCAL body to REMOTE in neighbour structure rank: " << m_neighbourRank
                                                                                              << std::endl;);
                bool res = m_nc->m_globalRemote.addBody(body);
                LOGASSERTMSG(res,
                             m_pSerializerLog,
                             "Could not add body with id: " << RigidBodyId::getBodyIdString(body->m_id)
                                                            << "in global remote list")

                res = m_nc->m_globalLocal.removeBody(body);
                LOGASSERTMSG(res,
                             m_pSerializerLog,
                             "Could not remove body with id: " << RigidBodyId::getBodyIdString(body->m_id)
                                                               << "in global local list")
                ...
                LOGASSERTMSG(res,
                             m_pSerializerLog,
                             "Could not delete body with id: " << RigidBodyId::getBodyIdString(body->m_id)
                                                               << "in neighbour structure rank: "
                                                               << m_neighbourRank)
                auto resPair = m_neighbourData->addRemoteBodyData(body);
                LOGASSERTMSG(resPair.second == true,
                             m_pSerializerLog,
                             "Remote Body with id: " << RigidBodyId::getBodyIdString(body->m_id)
                                                     << " could not be added in neighbour data structure rank: "
                                                     << m_neighbourRank
                                                     << "!");
            }
            else
            {
                ...
                LOGBC_SZ(m_pSerializerLog,
                         "--->\t\t Body with id: " << RigidBodyId::getBodyIdString(body)
                                                   << " marked for deletion after send!"
                                                   << std::endl;)
                m_nc->m_localBodiesToDelete.insert(body);

                // Remove this local from this neighbour structure!
                LOGBC_SZ(m_pSerializerLog,
                         "-----> Deleting LOCAL body in neighbour structure rank: " << m_neighbourRank << std::endl;);
                bool res = m_neighbourData->eraseLocalBodyData(body);
                LOGASSERTMSG(res == true,
                             m_pSerializerLog,
                             "Could not delete local body with id: " << RigidBodyId::getBodyIdString(body->m_id)
                                                                     << "in neighbour structure rank: "
                                                                     << m_neighbourRank
                                                                     << "!");
        ...
        else if (m_bodyInfo->m_ownerRank != m_nc->m_rank)
        {  // if owner rank is not the sending neighbour and  not our rank!
            // Remove this local from this neighbour structure!
            LOGBC_SZ(m_pSerializerLog,
                     "-----> Deleting LOCAL body in neighbour structure rank: " << m_neighbourRank << std::endl;);
            bool res = m_neighbourData->eraseLocalBodyData(body);
            ...
            // Mark bodyinfo , dont remove this rank (only mark it) because we might send a notification which needs
            // access to the overlap bool!
    ...
    void loadUpdate(Archive& ar) const
    {
        ...
        LOGASSERTMSG(remoteData,
                     m_pSerializerLog,
                     "There exists no RemoteData for body with id: " << RigidBodyId::getBodyIdString(id)
                                                                     << " for neighbourRank: "
                                                                     << m_neighbourRank
                                                                     << "in process rank: "
                                                                     << m_nc->m_rank
                                                                     << "!");
        LOGASSERTMSG(m_nc->m_globalRemote.find(id) != m_nc->m_globalRemote.end(),
                     m_pSerializerLog,
                     "m_globalRemote does not contain body with id: " << RigidBodyId::getBodyIdString(id)
                                                                      << " in process rank: "
                                                                      << m_nc->m_rank
                                                                      << "!");
        ...
        {  // if the body is now our local body!
            LOGBC_SZ(m_pSerializerLog, "-----> Changing REMOTE to LOCAL" << std::endl;);

            std::set<RankIdType>
                overlappingNeighbours;  // these are only the common neighbours between m_neighbourRank and m_nc->m_rank
            ...
            {
                LOGASSERTMSG(*it != m_nc->m_rank,
                             m_pSerializerLog,
                             "overlappingNeighbours should not contain own rank: " << m_nc->m_rank);

                if (m_nc->m_nbRanks.find(*it) != m_nc->m_nbRanks.end())
                {  // If rank is neighbour rank
                    body->m_pBodyInfo->m_neighbourRanks[*it] =
                        typename BodyInfoType::Flags(true);  // Overlaps this rank!
                }
                else
                {
        ...
        else if (owningRank != m_neighbourRank)
        {  // if the body is not owned anymore by the sending process but by
            // another of our neighbours (hoppfully)
            LOGBC_SZ(m_pSerializerLog, "-----> Changing remote body to REMOTE" << std::endl;);
            LOGASSERTMSG(m_nc->m_nbRanks.find(owningRank) != m_nc->m_nbRanks.end(),
                         m_pSerializerLog,
                         "Owner Rank: " << owningRank << " for body with id: " << RigidBodyId::getBodyIdString(id)
                                        << " is no neighbour in process rank: "
                                        << m_nc->m_rank
                                        << "!")
            ...
            LOGASSERTMSG(pairRes.second,
                         m_pSerializerLog,
                         "Insertion of body with id: " << RigidBodyId::getBodyIdString(body)
                                                       << " in neighbour structure rank: "
                                                       << owningRank
                                                       << " failed!")
    ...
    void loadNotification(Archive& ar) const
    {
        ...
        LOGASSERTMSG(m_nc->m_globalRemote.find(id) == m_nc->m_globalRemote.end(),
                     m_pSerializerLog,
                     "m_globalRemote does contain body with id: " << RigidBodyId::getBodyIdString(id)
                                                                  << " in process rank: "
                                                                  << m_nc->m_rank
                                                                  << "!");
        LOGASSERTMSG(m_nc->m_globalLocal.find(id) == m_nc->m_globalLocal.end(),
                     m_pSerializerLog,
                     "m_globalLocal does contain body with id: " << RigidBodyId::getBodyIdString(id)
                                                                 << " in process rank: "
                                                                 << m_nc->m_rank
                                                                 << "!");
    ...
    void loadRemoval(Archive& ar) const
    {
        ...
        LOGASSERTMSG(res,
                     m_pSerializerLog,
                     "Could not delete remote body with id: " << RigidBodyId::getBodyIdString(id)
                                                              << " in neighbour structure rank: "
                                                              << m_neighbourRank
                                                              << " !");
    ...
    void serializeBodyUpdate(Archive& ar, RigidBodyType* body) const
    {
        ...
        {
            body->m_A_IK =
                body->m_q_KI.matrix();  // QuaternionHelpers::setRotFromQuaternion(body->m_q_KI , body->m_A_IK);
        }

        // Velocity
        LOGASSERTMSG(body->m_pSolverData,
                     m_pSerializerLog,
                     "No SolverData present in body with id: " << RigidBodyId::getBodyIdString(body) << "!");
        ...
        LOGBC_SZ(m_pSerializerLog,
                 "----->  m_uBuffer.m_back: " << body->m_pSolverData->m_uBuffer.m_back.transpose() << std::endl;);
    }
...
public:
    ...
    NeighbourMessageWrapperInclusion(NeighbourCommunicatorType* nc) : m_nc(nc), m_initialized(false)
    {
        if (Logging::LogManager::getSingleton().existsLog("SimulationLog"))
        {
            m_pSimulationLog = Logging::LogManager::getSingleton().getLog("SimulationLog");
...
/// CHECKED OK
template <typename TNeighbourCommunicator>
class NeighbourMessageWrapperInclusionContact
    : public NeighbourMessageWrapperInclusion<TNeighbourCommunicator>,
...
public:
    ...
    NeighbourMessageWrapperInclusionContact(NeighbourCommunicatorType* nc)
        : NeighbourMessageWrapperInclusion<NeighbourCommunicatorType>(nc), m_neighbourData(nullptr)
    {
    }
    ...
    void save(Archive& ar, const unsigned int version) const
    {
        LOGASSERTMSG(this->m_initialized,
                     this->m_pSerializerLog,
                     "The NeighbourMessageWrapperInclusionContact is not correctly initialized, Rank not set!");
        ...
        if (size > 0)
        {
            LOGIC_SZ(
                this->m_pSerializerLog,
                "InclusionComm-Contact=============================================================================="
                    << std::endl;)
            LOGIC_SZ(this->m_pSerializerLog,
                     "SERIALIZE Message for neighbour rank: " << this->m_neighbourRank << std::endl;);
            ...
            for (auto it = m_neighbourData->remoteBegin(); it != m_neighbourData->remoteEnd(); ++it)
            {
                ...
                LOGIC_SZ(this->m_pSerializerLog,
                         "---->  body id: " << RigidBodyId::getBodyIdString((it->first)) << std::endl;);
            }
        ...
        if (size > 0)
        {
            LOGIC_SZ(
                ...
                "InclusionComm-Contact=============================================================================="
                    << std::endl;)
            LOGIC_SZ(this->m_pSerializerLog,
                     "DESERIALIZE Message for neighbour rank: " << this->m_neighbourRank << std::endl;);
            ...
            m_neighbourData = this->m_nc->m_nbDataMap.getNeighbourData(this->m_neighbourRank);
            LOGASSERTMSG(
                m_neighbourData, this->m_pSerializerLog, " neighbour map non existent for: " << this->m_neighbourRank)
            ...
            for (unsigned int i = 0; i < size; i++)
            {
                ...
                auto* localData = this->m_nc->m_pBodyComm->getNeighbourMap()
                                      ->getNeighbourData(this->m_neighbourRank)
                                      ->getLocalBodyData(id);
                LOGASSERTMSG(
                    localData,
                    this->m_pSerializerLog,
                    "There is no bodydata for local body id: " << RigidBodyId::getBodyIdString(id)
                                                               << " in body communicators neighbour data for rank:"
                                                               << this->m_neighbourRank)

                RigidBodyType* body = localData->m_pBody;
                // add a local bodydata which connects to the billateral constraint
                LOGASSERTMSG(body,
                             this->m_pSerializerLog,
                             "Local body pointer null for id: " << RigidBodyId::getBodyIdString(id)
                                                                << " in body communicators neighbour data for rank:"
                                                                << this->m_neighbourRank)
                auto pairAddLocal = m_neighbourData->addLocalBodyData(body);
                LOGASSERTMSG(pairAddLocal.second,
                             this->m_pSerializerLog,
                             "Could not add body with id: " << RigidBodyId::getBodyIdString(id)
                                                            << " to neighbour data, already added!")
                ...
                LOGIC_SZ(this->m_pSerializerLog,
                         "----> added rank: " << this->m_neighbourRank << " to SplitBodyNode for body id: "
                                              << RigidBodyId::getBodyIdString(id)
                                              << std::endl);
...
template <typename TNeighbourCommunicator>
class NeighbourMessageWrapperInclusionMultiplicity
    : public NeighbourMessageWrapperInclusion<TNeighbourCommunicator>,
      public boost::serialization::traits<NeighbourMessageWrapperInclusion<TNeighbourCommunicator>,
                                          boost::serialization::object_serializable,
                                          boost::serialization::track_never>
...
public:
    ...
    void save(Archive& ar, const unsigned int version) const
    {
        ...
        if (size > 0)
        {
            LOGIC_SZ(
                this->m_pSerializerLog,
                "InclusionComm-Multiplicity============================================================================"
                "==" << std::endl;)
            LOGIC_SZ(this->m_pSerializerLog,
                     "SERIALIZE Message for neighbour rank: " << this->m_neighbourRank << std::endl;);

            LOGIC_SZ(this->m_pSerializerLog,
                     "---> # Local Split Bodies (with external Contacts): " << size << std::endl;);
            ...
            for (auto it = m_neighbourData->localBegin(); it != itEnd; ++it)
            {
                ...
                LOGIC_SZ(this->m_pSerializerLog,
                         "----> id: " << RigidBodyId::getBodyIdString((it->first)) << std::endl
                                      << "----> multiplicity: "
                                      << multiplicity
                                      << std::endl
                                      << "----> multiplicityWeight: "
                                      ...
                                      << std::endl
                                      << "----> h_term: "
                                      << it->second.m_pBody->m_h_term.transpose()
                                      << std::endl;)
                ar&(it->first);          // body id
                ...
                serializeEigen(
                    ar,
                    it->second.m_pBody
                        ->m_h_term);  // send the current h_term , mass matrix is already in the remote on the neighbour
            }
        }
        else
        {
            GRSF_ERRORMSG("We should send a message to neighbour " << this->m_neighbourRank
                                                                   << " which is non empty! This should not happen!")
    ...
    void load(Archive& ar, const unsigned int version) const
    {
        ...
        if (size > 0)
        {
            ...
            LOGASSERTMSG(m_neighbourData,
                         this->m_pSerializerLog,
                         "There exists no NeighbourData for neighbourRank: " << this->m_neighbourRank
                                                                             << "in process rank: "
                                                                             << this->m_nc->m_rank
                                                                             << "!");
            ...
            for (unsigned int i = 0; i < size; i++)
            {
                ...
                LOGASSERTMSG(remoteBodyData,
                             this->m_pSerializerLog,
                             "remoteBodyData is null for body id: " << RigidBodyId::getBodyIdString(id)
                                                                    << " in neighbour data rank "
                                                                    << this->m_neighbourRank)
                LOGASSERTMSG(remoteBodyData->m_pBody->m_pSolverData,
                             this->m_pSerializerLog,
                             "m_pSolverData is null for body id: " << RigidBodyId::getBodyIdString(id));

                RigidBodyType* remoteBody = remoteBodyData->m_pBody;

                LOGASSERTMSG(remoteBody->m_pBodyInfo,
                             this->m_pSerializerLog,
                             "bodyInfo is null for body id: " << RigidBodyId::getBodyIdString(id)
                                                              << " in neighbour data rank "
                                                              << this->m_neighbourRank);
                LOGASSERTMSG(remoteBody->m_pBodyInfo->m_isRemote,
                             this->m_pSerializerLog,
                ...
                LOGIC_SZ(this->m_pSerializerLog,
                         "----> id: " << RigidBodyId::getBodyIdString(id) << std::endl
                                      << "----> multiplicity: "
                                      << multiplicity
                                      << std::endl
                                      << "----> multiplicityWeight: "
...
template <typename TNeighbourCommunicator>
class NeighbourMessageWrapperInclusionSplitBodyUpdate
    : public NeighbourMessageWrapperInclusion<TNeighbourCommunicator>,
      public boost::serialization::traits<NeighbourMessageWrapperInclusion<TNeighbourCommunicator>,
                                          boost::serialization::object_serializable,
                                          boost::serialization::track_never>
...
public:
    ...
    void save(Archive& ar, const unsigned int version) const
    {
        ...
        LOGASSERTMSG(this->m_initialized,
                     this->m_pSerializerLog,
                     "The NeighbourMessageWrapperInclusionSplitBodyUpdate is not correctly initialized, Rank not set!");

        m_neighbourData = this->m_nc->m_nbDataMap.getNeighbourData(this->m_neighbourRank);
        LOGASSERTMSG(this->m_neighbourData,
                     this->m_pSerializerLog,
                     "There exists no NeighbourData for neighbourRank: " << this->m_neighbourRank << "in process rank: "
                                                                         << this->m_nc->m_rank
                                                                         << "!");
        ...
        if (size > 0)
        {
            ...
            for (auto it = m_neighbourData->remoteBegin(); it != m_neighbourData->remoteEnd(); it++)
            {
                ...
                LOGASSERTMSG(it->second.m_pBody->m_pSolverData,
                             this->m_pSerializerLog,
                ...
                LOGIC_SZ(this->m_pSerializerLog,
                         "----> id: " << RigidBodyId::getBodyIdString((it->first)) << std::endl
                                      << "----> uFront: "
                                      << it->second.m_pBody->m_pSolverData->m_uBuffer.m_front.transpose()
                                      << std::endl)
            }
        }
        else
    ...
    void load(Archive& ar, const unsigned int version) const
    {
        ...
        if (size > 0)
        {
            ...
            for (unsigned int i = 0; i < size; i++)
            {
                ...
                LOGASSERTMSG(localBodyData,
                             this->m_pSerializerLog,
                             "The local split body id: " << RigidBodyId::getBodyIdString(id)
                                                         << "is not in this neighbour rank: "
                                                         << this->m_neighbourRank)
        ...
        {
            GRSF_ERRORMSG("We should not receive an empty message from neighbour " << this->m_neighbourRank
                                                                                   << std::endl);
        }
...
template <typename TNeighbourCommunicator>
class NeighbourMessageWrapperInclusionSplitBodySolution
    : public NeighbourMessageWrapperInclusion<TNeighbourCommunicator>,
      public boost::serialization::traits<NeighbourMessageWrapperInclusion<TNeighbourCommunicator>,
                                          boost::serialization::object_serializable,
                                          boost::serialization::track_never>
...
public:
    ...
    void save(Archive& ar, const unsigned int version) const
    {
        ...
        LOGASSERTMSG(
            this->m_initialized,
            this->m_pSerializerLog,
            "The NeighbourMessageWrapperInclusionSplitBodySolution is not correctly initialized, Rank not set!");
        ...
        {
            LOGIC_SZ(this->m_pSerializerLog,
                     "InclusionComm-SplitBodySolution=================================================================="
                     "============"
                         << std::endl;)
            LOGIC_SZ(this->m_pSerializerLog,
                     "SERIALIZE Message for neighbour rank: " << this->m_neighbourRank << std::endl;);
    ...
    void load(Archive& ar, const unsigned int version) const
    {
        ...
        if (size > 0)
        {
            ...
            for (unsigned int i = 0; i < size; i++)
            {
                ...
                LOGASSERTMSG(remoteBodyData,
                             this->m_pSerializerLog,
                             "The remote split body id: " << RigidBodyId::getBodyIdString(id)
                                                          << "is not in this neighbour rank: "
                                                          << this->m_neighbourRank)

                LOGASSERTMSG(remoteBodyData->m_pBody->m_pSolverData,
                             this->m_pSerializerLog,
                             "No solver data in remote split body id: " << RigidBodyId::getBodyIdString(id)
                                                                        << " in this neighbour rank: "
                                                                        << this->m_neighbourRank)
...
class TopologyBuilderMessageWrapperBodies
    : public boost::serialization::traits<TopologyBuilderMessageWrapperBodies<TTopologyBuilder>,
                                          boost::serialization::object_serializable,
                                          boost::serialization::track_never>
{
public:
    ...
    TopologyBuilderMessageWrapperBodies(TopologyBuilderType* topoBuilder) : m_pTopoBuilder(topoBuilder){};
    ...
    void load(Archive& ar, const unsigned int version) const
    {
        ...
        for (unsigned int i = 0; i < size; ++i)
        {
            ...
            auto res = m_pTopoBuilder->m_initStates.emplace(
                std::piecewise_construct, std::make_tuple(id), std::make_tuple(id));
        ...
        if (m_pTopoBuilder->m_doComputations_loc)
        {
            ...
            {
                // Insert a AABB
                auto insertedAABB = m_pTopoBuilder->m_rankAABBs.emplace(
                    std::piecewise_construct, std::make_tuple(m_rank), std::make_tuple());
                // Get AABB
...
template <typename TTopologyBuilder>
class TopologyBuilderMessageWrapperOrientation
    : public boost::serialization::traits<TopologyBuilderMessageWrapperBodies<TTopologyBuilder>,
                                          boost::serialization::object_serializable,
                                          boost::serialization::track_never>
{
public:
    ...
    TopologyBuilderMessageWrapperOrientation(TopologyBuilderType* topoBuilder) : m_pTopoBuilder(topoBuilder){};
...
{
template <typename TTopologyBuilder>
class TopologyBuilderMessageWrapperResults
    : public boost::serialization::traits<TopologyBuilderMessageWrapperResults<TTopologyBuilder>,
                                          boost::serialization::object_serializable,
                                          boost::serialization::track_never>
{
public:
    ...
    TopologyBuilderMessageWrapperResults(TopologyBuilderType* topoBuilder) : m_pTopoBuilder(topoBuilder){};
    ...
    template <typename TopoType, typename Archive, SFINAE_ENABLE_IF(MPILayer::isGridTopoBuilder<TopoType>::value)>
    ...
    template <typename TopoType, typename Archive, SFINAE_ENABLE_IF(MPILayer::isKdTreeTopoBuilder<TopoType>::value)>
    void saveTopoData(Archive& ar) const
    {
        ar & m_pTopoBuilder->m_aabb;
        ar & m_pTopoBuilder->m_aligned;
