...
class TopologyBuilder
{
    ...
    bool checkAndRebuild(unsigned int timeStep, PREC currentTime)
    {
        ...
        if (m_rebuildSettings.m_mode == RebuildSettingsType::Mode::DYNAMIC && timeStep != 0 &&
    ...
    bool communicateRebuild(bool rebuild)
    {
        ...
        if (n > 0)
        {  // if one or more process wants to rebuild then rebuild!
...
class MassPointPrediction
{
    ...
    inline void reserve(Container& c, unsigned int n)
    {
                           ...
                           Vector3Vec&                 predictedPoints)
    {
        ...
        for (auto& massPoint : massPoints)
        {
            ...
            //            <<RigidBodyId::getBodyIdString(massPoint.m_state->m_id) << " last prediction: " <<
            //            predictedPoints.back().transpose()  <<std::endl);
        }

        //        LOGTBLEVEL2(m_pSimulationLog, "---> TopoBuilder: Predicted mass points: " << predictedPoints.size() <<
        //        std::endl;);
    }
};
...
class OutlierFilter
{
    ...
    std::size_t filter(std::vector<MassPointType>& massPoints, const AABB3d& aabb)
    {
...
#define DEFINE_OUTLIERFILTER_TYPES using OutlierFilterSettingsType = typename OutlierFilter::OutlierFilterSettingsType;
...
class GridTopologyBuilderBase : public TopologyBuilder<TProcCommunicator>
{
    ...
    typename std::unordered_map<RankIdType, std::vector<const RigidBodyState*>>
        m_bodiesPerRank;  ///< rank to all pointers in m_initStates;
    ...
    Vector6 m_I_theta_G;  ///< Local intertia tensor (binet tensor in center I) [ a_00,a_01,a_02,a_11,a_12,a_22] ) in
                          /// intertial frame I
    ...
    void makeMVBB(const Vector3Vec& I_points, Matrix33& A_IK, AABB3d& K_aabb)
    {
        ...
        PREC epsilon =
            0.01 * K_aabb.extent().matrix().norm();  // percentage of the approximate extent of the point cloud
        ...
        LOGTBLEVEL3(m_pSimulationLog,
                    "---> GridTopoBuilder: A_IK * A_IK^T: " << std::endl
                                                            << A_IK * A_IK.transpose()
                                                            << std::endl;);
    }
    ...
    void buildBinetTensor(const C& points, Vector6& I_theta_O, const Vector3& I_shift_OG = Vector3::Zero())
    {
    ...
    void shiftBinetTensor(Vector6& I_theta_G, const Vector3& I_r_OG, PREC scalar)
    {
    ...
    void solveOrientation(Matrix33& A_IK, const Vector6& theta_G)
    {
        ...
        LOGTBLEVEL3(m_pSimulationLog,
                    "---> GridTopoBuilderBase: Global Binet inertia tensor: " << std::endl
                                                                              << Theta_G
                                                                              << std::endl)
        ...
        LOGTBLEVEL3(m_pSimulationLog,
                    "---> GridTopoBuilderBase: Eigenvalues: " << std::endl
                                                              << eigenSolv.eigenvalues().transpose()
                                                              << std::endl;);
    ...
    void filterPoints(std::vector<MassPointType>& points, AABB3d& aabb)
    {
        ...
        LOGTBLEVEL1(m_pSimulationLog,
                    "---> GridTopoBuilderBase: Classified " << nOutlier << " outliers in point cloud." << std::endl);
    ...
    void initMakeBoundingBoxGlobal(RankIdType masterRank)
    {
        ...
        else if (m_settings.m_buildMode == SettingsType::BuildMode::ALIGNED)
        {
            ...
            m_predPoints.clear();
            LOGTBLEVEL1(m_pSimulationLog,
                        "---> GridTopoBuilderBase:mass points glo: " << m_massPoints.size() << std::endl);
            m_massPointPrediction.predictMassPoints(
                m_massPoints, m_pDynSys->m_externalForces, m_pDynSys->m_staticBodies, m_predPoints);
            LOGTBLEVEL1(m_pSimulationLog,
                        "---> GridTopoBuilderBase:mass points glo pred: " << m_predPoints.size() << std::endl);
    ...
    bool determineLocalComputation()
    {
        ...
        return (m_settings.m_buildMode == SettingsType::BuildMode::ALIGNED ||
    ...
    void rebuildMakeBoundingBoxLocal_PreSend(RankIdType masterRank)
    {
        ...
        for (auto& body : m_pDynSys->m_simBodies)
        {
            ...
            LOGTBLEVEL3(m_pSimulationLog,
                        "\t\t state: " << RigidBodyId::getBodyIdString(body->m_id) << " , "
                                       << res.first->second.m_q.transpose()
                                       << std::endl;);
        }
        ...
        else
        {
            // otherwise dont do any computation (it might be send anyway but is not used on the master!)
            LOGTBLEVEL1(m_pSimulationLog,
                        "---> GridTopoBuilderBase: No local computations performed, because disabled!" << std::endl);
        }
    }
    ...
    template <bool writeAABBList = true, bool writePoints = false, bool writeHistogram = false>
    void writeTopoInfo(XMLNodeType& root, PREC currentTime, PREC buildTime)
    {
        std::stringstream ss;
        ...
        if (writeAABBList)
        {
            ...
            {
                ss << aabbIt.first << " " << aabbIt.second.m_minPoint.transpose().format(MyMatrixIOFormat::SpaceSep)
                   << " " << aabbIt.second.m_maxPoint.transpose().format(MyMatrixIOFormat::SpaceSep) << std::endl;
            }
            node.append_child(nodePCData).set_value(ss.str().c_str());
        }
    }
...
class GridTopologyBuilder : public GridTopologyBuilderBase<TProcCommunicator, GridBuilderSettings>
{
    ...
    GridTopologyBuilder(std::shared_ptr<DynamicsSystemType>      pDynSys,
        ...
        : Base(settings,
               ...
               sceneFilePath,
               TopologyBuilderEnumType::GRIDBUILDER,
               pDynSys,
               pProcCommunicator,
               rebuildSettings)
    ...
    void initTopology(PREC currentTime = 0)
    {
        ...
        // get initial startTime which was parsed in ( gets overwritten by global initial condition if specified to
        // continue from)
        ...
        {
            if (m_settings.m_processDim(0) * m_settings.m_processDim(1) * m_settings.m_processDim(2) !=
                m_pProcCommunicator->getNProcesses())
            {
                GRSF_ERRORMSG("processDim: " << m_settings.m_processDim << " does not fit "
                                             << m_pProcCommunicator->getNProcesses()
                                             << "processes!");
            }
            ...
            SceneParserMPI<GridTopologyBuilder,
                           ParserModulesCreatorTopoBuilder<GridTopologyBuilder>::template SceneParserTraits>
                parser(c,
                       m_pSimulationLog,
            ...
            {
                GRSF_ERRORMSG("Parsed to little initial states in scene file: " << m_sceneFilePath << " states: "
                                                                                << m_initStates.size()
                                                                                << "globalSimBodies: "
                                                                                << m_nGlobalSimBodies
                                                                                << std::endl);
            ...
            for (auto& s : m_initStates)
            {
                ...
                LOGTBLEVEL3(m_pSimulationLog,
                            "\t\t state @" << &s.second << " : " << RigidBodyId::getBodyIdString(s.second.m_id) << " , "
                                           << s.second.m_q.transpose()
                                           << std::endl;);
            }
        ...
        else
        {
            ...
            m_pProcCommunicator->receiveMessageFromRank(
                m_messageWrapperResults, this->m_pProcCommunicator->getMasterRank(), m_messageWrapperResults.m_tag);
        ...
        LOGTBLEVEL1(m_pSimulationLog,
                    "---> GridTopoBuilder: States in m_bodiesInitStates: (" << m_pDynSys->m_bodiesInitStates.size()
                                                                            << ")"
                                                                            << std::endl;);
        for (auto& s : m_pDynSys->m_bodiesInitStates)
        {
            LOGTBLEVEL3(
                m_pSimulationLog,
                "\t\t state: " << RigidBodyId::getBodyIdString(s.second.m_id) << " , q: " << s.second.m_q.transpose()
                               << " u: "
                               << s.second.m_u.transpose()
                               << std::endl;);
        }
    ...
    void rebuildTopology(PREC currentTime)
    {
        ...
        if (this->m_pProcCommunicator->hasMasterRank())
        {
            ...
            {
                GRSF_ERRORMSG("m_nGlobalSimBodies: " << m_nGlobalSimBodies << " != " << m_initStates.size()
                                                     << std::endl);
            }
    ...
    void buildTopo()
    {
        ...
        LOGTBLEVEL1(m_pSimulationLog,
                    "\t aligned: " << m_aligned << std::endl
                                   << "\t min/max: "
                                   << " [ "
                                   << m_aabb.m_minPoint.transpose()
                                   << " , "
                                   << m_aabb.m_maxPoint.transpose()
                                   << "]"
                                   << std::endl
                                   << "\t A_IK: \n"
                                   << m_A_IK
                                   ...
                                   << "\t dim: "
                                   << "[ "
                                   << m_processDim.transpose()
                                   << "]"
                                   << std::endl
                                   << "\t extent: "
                                   << "[ "
                                   << m_aabb.extent().transpose()
                                   << "]"
                                   << std::endl;);
        m_pProcCommunicator->createProcTopoGrid(m_aabb, m_processDim, m_aligned, m_A_IK);
    ...
    void receiveGrid(RankIdType masterRank)
    {
        // All initial states get saved in m_pDynSys->m_bodiesInitStates
        m_messageWrapperResults.resetBeforLoad();
        m_pProcCommunicator->receiveMessageFromRank(m_messageWrapperResults, masterRank, m_messageWrapperResults.m_tag);
    }
    ...
    void writeTopoInfo(PREC currentTime, PREC buildTime)
    {
        ...
        node.append_child(nodePCData)
            .set_value(
                Utilities::typeToString(m_aabb.m_minPoint.transpose().format(MyMatrixIOFormat::SpaceSep)).c_str());

        node = gridN.first_element_by_path("./MaxPoint");
        node.append_child(nodePCData)
            .set_value(
                Utilities::typeToString(m_aabb.m_maxPoint.transpose().format(MyMatrixIOFormat::SpaceSep)).c_str());

        node = gridN.first_element_by_path("./A_IK");
        node.append_child(nodePCData)
            .set_value(Utilities::typeToString(m_A_IK.format(MyMatrixIOFormat::SpaceSep)).c_str());

        node = gridN.first_element_by_path("./Dimension");
        node.append_child(nodePCData)
            .set_value(Utilities::typeToString(m_processDim.transpose().format(MyMatrixIOFormat::SpaceSep)).c_str());
...
class KdTreeTopologyBuilder : public GridTopologyBuilderBase<TProcCommunicator, KdTreeBuilderSettings>
{
    ...
    KdTreeTopologyBuilder(std::shared_ptr<DynamicsSystemType>                   pDynSys,
        ...
        : Base(settings,
               ...
               globalOutlierFilterSettings,
               sceneFilePath,
               TopologyBuilderEnumType::KDTREEBUILDER,
               pDynSys,
               pProcCommunicator,
               rebuildSettings)
        , m_nGlobalSimBodies(nGlobalSimBodies)
    ...
    void initTopology(PREC currentTime = 0)
    {
        ...
        if (this->m_pProcCommunicator->hasMasterRank())
        {
            ...
            SceneParserMPI<KdTreeTopologyBuilder,
                           ParserModulesCreatorTopoBuilder<KdTreeTopologyBuilder>::template SceneParserTraits>
                parser(c,
                       m_pSimulationLog,
                       ApplicationCLOptions::getSingleton().getMediaDir());  // this class is the modules generator
        ...
        LOGTBLEVEL1(m_pSimulationLog,
                    "---> KdTreeTopoBuilder: States in m_bodiesInitStates: (" << m_pDynSys->m_bodiesInitStates.size()
                                                                              << ")"
                                                                              << std::endl;);
        for (auto& s : m_pDynSys->m_bodiesInitStates)
        {
    ...
    void rebuildTopology(PREC currentTime)
    {
        ...
        if (this->m_pProcCommunicator->hasMasterRank())
        {
            ...
            if (m_nGlobalSimBodies != m_initStates.size())
            {
                GRSF_ERRORMSG("m_nGlobalSimBodies: " << m_nGlobalSimBodies << "not equal to" << m_initStates.size()
                                                     << std::endl);
            }
    ...
    void buildKdTree()
    {
        ...
        auto rootData = std::unique_ptr<NodeDataType>(
            new NodeDataType(vec->begin(), vec->end(), std::unique_ptr<PointListType>(vec)));

        LOGTBLEVEL1(m_pSimulationLog,
                    "---> KdTreeTopoBuilder: Building KdTree with : "
                        << "minP: "
                        << this->m_aabb.m_minPoint.transpose()
                        << " maxP: "
                        << this->m_aabb.m_maxPoint.transpose()
                        << std::endl
                        << "points: "
                        << vec->size()
                        << std::endl)
        ...
        m_kdTree_temp->build(
            m_aabb, std::move(rootData), m_settings.m_maxTreeDepth, m_pProcCommunicator->getNProcesses());
        ...
        {
            GRSF_ERRORMSG("KdTree could only determine " << m_processes << " processes but "
                                                         << m_pProcCommunicator->getNProcesses()
                                                         << " are needed!")
        }
    ...
    void buildTopo()
    {
        ...
        {
            LOGTBLEVEL1(m_pSimulationLog,
                        "---> KdTreeTopoBuilder: Create static ProcessTopologyKdTree : " << std::endl;)
        }
        else
        {
        ...
        LOGTBLEVEL1(m_pSimulationLog,
                    "\t aligned: " << m_aligned << std::endl
                                   ...
                                   << "]"
                                   << std::endl;);
        m_pProcCommunicator->createProcTopoKdTree(m_kdTree_glo, m_neighbours, m_aabb, m_aligned, m_A_IK);
