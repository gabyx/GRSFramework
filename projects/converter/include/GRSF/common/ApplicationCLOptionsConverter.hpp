...
class ApplicationCLOptionsSimConverter : public Utilities::Singleton<ApplicationCLOptionsSimConverter>
{
    ...
    void checkArguments()
    {
        ...
        {
            GRSF_ERRORMSG("Output file supplied as argument: " << m_outputFile
                                                               << " does already exist (no overwrite is allowed)!")
        }
    }
    ...
    void printHelp()
    {
        std::cerr << "Help for the Application Sim: \n Options: \n"
                  << " \t -i|--input <path1> <path2> ... \n"
                  << " \t [Required] \n"
                  << "\t\t <path1> <path2> ... : These are multiple space delimited input sim file paths which are "
                     "processed \n"
...
class ApplicationCLOptionsRenderer : public Utilities::Singleton<ApplicationCLOptionsRenderer>
{
    ...
    inline const std::vector<boost::filesystem::path>& getInputFiles()
    {
        return m_inputFiles;
    }
    ...
    inline const boost::filesystem::path& getOutputFile()
    {
        return m_outputFile;
    }
    inline const boost::filesystem::path& getConverterLogicFile()
    {
        return m_converterLogicFile;
    }
    ...
    {
        std::cerr
            << "Help for the Application Renderer: \n Options: \n"
            << " \t -i|--input <path1> <path2> ... \n"
            << " \t [Required] \n"
...
class ApplicationCLOptionsAnalyzer : public Utilities::Singleton<ApplicationCLOptionsAnalyzer>
{
    ...
    {
        std::cerr
            << "Help for the Application " << m_name << ": \n Options: \n"
            << " \t -i|--input <path1> <path2> ... \n"
            << " \t [Required] \n"
            << "\t\t <path1> <path2> ... : These are multiple space delimited input sim (.sim)\n"
            << "\t\t file paths which are processed. \n"
