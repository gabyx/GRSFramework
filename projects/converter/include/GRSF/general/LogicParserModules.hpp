...
#define GRSF_ERRORMSG_PARSERTOOL(mess, id) GRSF_ERRORMSG(mess << " Tool id: " << id)
...
public:
    ...
    LogicModule(ParserType* p, SimFileExecutionGraph* g) : m_parser(p), m_executionGraph(g), m_pLog(p->getLog())
    {
        GRSF_ASSERTMSG(m_executionGraph, "null")
    }
    ...
    void parseTool(XMLNodeType& tool)
    {
        ...
        {
            GRSF_ERRORMSG_PARSERTOOL("---> String conversion 'type': " << type << " not found!", id);
        }
    }
    ...
    void addAllLinks(XMLNodeType& logicNode)
    {
        ...
        for (auto itNode = nodes.begin(); itNode != itNodeEnd; ++itNode)
        {
            ...
            {
                GRSF_ERRORMSG("---> String conversion in Get: outNode failed");
            }
            unsigned int outSocket;
            if (!Utilities::stringToType(outSocket, itNode->attribute("outSocket").value()))
            {
                GRSF_ERRORMSG("---> String conversion in Get: outSocket failed");
            }

            unsigned int fromNode;
            if (!Utilities::stringToType(fromNode, itNode->attribute("fromNode").value()))
            {
                GRSF_ERRORMSG("---> String conversion in Get: fromNode failed");
            }
            unsigned int fromSocket;
            if (!Utilities::stringToType(fromSocket, itNode->attribute("fromSocket").value()))
            {
                GRSF_ERRORMSG("---> String conversion in Get: fromSocket failed");
            }

            LOGLPLEVEL3(m_pLog,
                        "---> Linking Tool: Get " << outNode << " socket: " << outSocket << " --from--> " << fromNode
                                                  << " socket: "
                                                  << fromSocket
                                                  << std::endl;);
            // Link the nodes
        ...
        for (auto itNode = nodes.begin(); itNode != itNodeEnd; ++itNode)
        {
            ...
            if (!Utilities::stringToType(outNode, itNode->attribute("outNode").value()))
            {
                GRSF_ERRORMSG("---> String conversion in Write: outNode failed");
            }
            unsigned int outSocket;
            if (!Utilities::stringToType(outSocket, itNode->attribute("outSocket").value()))
            {
                GRSF_ERRORMSG("---> String conversion in Write: outSocket failed");
            }

            unsigned int toNode;
            if (!Utilities::stringToType(toNode, itNode->attribute("toNode").value()))
            {
                GRSF_ERRORMSG("---> String conversion in Write: toNode failed");
            }
            unsigned int toSocket;
            if (!Utilities::stringToType(toSocket, itNode->attribute("toSocket").value()))
            {
                GRSF_ERRORMSG("---> String conversion in Write: toSocket failed");
            }

            LOGLPLEVEL3(m_pLog,
                        "---> Linking Tool: Write from" << outNode << " socket: " << outSocket << " ---to---> "
                                                        << "to: "
                                                        << toNode
                                                        << " socket: "
                                                        << toSocket
...
protected:
    ...
    void addNodeToGroup(XMLNodeType& logicNode, unsigned int id, std::string defaultGroupName = "")
    {
        ...
        if (att)
        {
            ...
            {
                GRSF_ERRORMSG_PARSERTOOL("String conversion 'groupId' failed", id)
            }
        ...
        if (needsResetGroup)
        {
            ...
            if (att)
            {
                ...
                if (!Utilities::stringToType(l, att.value()))
                {
                    GRSF_ERRORMSG_PARSERTOOL("String conversion 'resetGroupId' failed", id)
                }
            ...
            {
                GRSF_ERRORMSG_PARSERTOOL("You need to specify a resetGroupId", id);
            }
        }
    }
...
#define DEFINE_CONSTANT3(type, typeName, arg)                                      \
    ...
    {                                                                              \
        ...
        {                                                                          \
            GRSF_ERRORMSG_PARSERTOOL("---> String conversion 'value' failed", id); \
        }                                                                          \
...
#define DEFINE_CONSTANT(type) DEFINE_CONSTANT2(type, type)
    ...
    void createToolConstant(XMLNodeType& logicNode, unsigned int id)
    {
        ...
        if
            ...
            DEFINE_CONSTANT(double)
        else if
            DEFINE_CONSTANT(char)
        else if
            DEFINE_CONSTANT(short)
        else if
            DEFINE_CONSTANT(int)
        else if
            DEFINE_CONSTANT(long int)
        else if
            DEFINE_CONSTANT(long long int)
        else if
            DEFINE_CONSTANT(unsigned char)
        else if
            DEFINE_CONSTANT(unsigned short)
        else if
            DEFINE_CONSTANT(unsigned int)
        else if
            DEFINE_CONSTANT(unsigned long int)
        else if
            DEFINE_CONSTANT(unsigned long long int)
        else if
            DEFINE_CONSTANT(Vector3)
        else if
            DEFINE_CONSTANT3(Quaternion, Quaternion, tt.coeffs())
        else if
            DEFINE_CONSTANT2(std::string, string)
        else if
            DEFINE_CONSTANT2(boost::filesystem::path, path)
        else
        {
            GRSF_ERRORMSG_PARSERTOOL("---> String conversion 'outputType': '" << t << "' not found!", id);
        }
...
#define ADD_STRINGFORMAT_SOCKET_OUT(type) ADD_STRINGFORMAT_SOCKET2(type, type, Output)
    ...
    void createToolStringFormat(XMLNodeType& logicNode, unsigned int id)
    {
        ...
        if (format.empty())
        {
            GRSF_ERRORMSG_PARSERTOOL("---> String conversion 'format': not defined!", id);
        }
        ...
        for (auto& n : logicNode.children("InputFormat"))
        {
            ...
            if
                ADD_STRINGFORMAT_SOCKET_IN(float)
            else if
                ADD_STRINGFORMAT_SOCKET_IN(double)
            else if
                ADD_STRINGFORMAT_SOCKET_IN(char)
            else if
                ADD_STRINGFORMAT_SOCKET_IN(short)
            else if
                ADD_STRINGFORMAT_SOCKET_IN(int)
            else if
                ADD_STRINGFORMAT_SOCKET_IN(long int)
            else if
                ADD_STRINGFORMAT_SOCKET_IN(long long int)
            else if
                ADD_STRINGFORMAT_SOCKET_IN(unsigned char)
            else if
                ADD_STRINGFORMAT_SOCKET_IN(unsigned short)
            else if
                ADD_STRINGFORMAT_SOCKET_IN(unsigned int)
            else if
                ADD_STRINGFORMAT_SOCKET_IN(unsigned long int)
            else if
                ADD_STRINGFORMAT_SOCKET_IN(unsigned long long int)
            else if
                ADD_STRINGFORMAT_SOCKET2_IN(std::string, string)
            else if
                ADD_STRINGFORMAT_SOCKET2_IN(boost::filesystem::path, path)
            else
            {
                GRSF_ERRORMSG_PARSERTOOL("---> String conversion 'InputFormat': '" << t << "' not found!", id);
            }
        }
        unsigned int outs = 0;
        for (auto& n : logicNode.children("OutputFormat"))
        {
            ...
            if
                ADD_STRINGFORMAT_SOCKET2_OUT(std::string, string)
            else if
                ADD_STRINGFORMAT_SOCKET2_OUT(boost::filesystem::path, path)
            else
            {
                GRSF_ERRORMSG_PARSERTOOL("---> String conversion 'OutputFormat': '" << t << "' not found!", id);
            }
            ++outs;
        }
        if (outs == 0)
...
#define DEFINE_SIMPLEFUNCTION2_IFPART(type)                                                                 \
    ...
    {                                                                                                       \
        if                                                                                                  \
            DEFINE_SIMPLEFUNCTION_S(float)                                                                  \
        else if                                                                                             \
            DEFINE_SIMPLEFUNCTION_S(double)                                                                 \
        else if                                                                                             \
            DEFINE_SIMPLEFUNCTION_S(bool)                                                                   \
        else if                                                                                             \
            DEFINE_SIMPLEFUNCTION_S(char)                                                                   \
        else if                                                                                             \
            DEFINE_SIMPLEFUNCTION_S(short)                                                                  \
        else if                                                                                             \
            DEFINE_SIMPLEFUNCTION_S(int)                                                                    \
        else if                                                                                             \
            DEFINE_SIMPLEFUNCTION_S(long int)                                                               \
        else if                                                                                             \
            DEFINE_SIMPLEFUNCTION_S(unsigned char)                                                          \
        else if                                                                                             \
            DEFINE_SIMPLEFUNCTION_S(unsigned short)                                                         \
        else if                                                                                             \
            DEFINE_SIMPLEFUNCTION_S(unsigned int)                                                           \
        else if                                                                                             \
            DEFINE_SIMPLEFUNCTION_S(unsigned long int)                                                      \
        else if                                                                                             \
            DEFINE_SIMPLEFUNCTION_S(unsigned long long int)                                                 \
        else                                                                                                \
        {                                                                                                   \
            GRSF_ERRORMSG_PARSERTOOL("---> String conversion 'outputType': '" << t2 << "' not found!", id); \
        }                                                                                                   \
    }
...
#define DEFINE_SIMPLEFUNCTION(type) DEFINE_SIMPLEFUNCTION2(type, type)
    ...
    void createToolSimpleFunction(XMLNodeType& logicNode, unsigned int id)
    {
        ...
        if (att)
        {
            ...
            {
                GRSF_ERRORMSG_PARSERTOOL("---> String conversion 'inputs' failed", id);
            }
        }
        // Make tool
...
#define ADD_SIMPLEFUNCTION_SOCKET_IN(type) ADD_SIMPLEFUNCTION_SOCKET2(type, type, Input)
    ...
    void createToolSimpleFunction_parseInputs(XMLNodeType& logicNode, unsigned int id, TSimpleFunction* simpleFNode)
    {
        ...
        {
            GRSF_ERRORMSG_PARSERTOOL("No expressions string in SimpleFunction", id)
        }
        ...
        for (auto& n : logicNode.children("InputFormat"))
        {
            ...
            if
                ADD_SIMPLEFUNCTION_SOCKET_IN(float)
            else if
                ADD_SIMPLEFUNCTION_SOCKET_IN(double)
            else if
                ADD_SIMPLEFUNCTION_SOCKET_IN(bool)
            else if
                ADD_SIMPLEFUNCTION_SOCKET_IN(char)
            else if
                ADD_SIMPLEFUNCTION_SOCKET_IN(short)
            else if
                ADD_SIMPLEFUNCTION_SOCKET_IN(int)
            else if
                ADD_SIMPLEFUNCTION_SOCKET_IN(long int)
            else if
                ADD_SIMPLEFUNCTION_SOCKET_IN(long long int)
            else if
                ADD_SIMPLEFUNCTION_SOCKET_IN(unsigned char)
            else if
                ADD_SIMPLEFUNCTION_SOCKET_IN(unsigned short)
            else if
                ADD_SIMPLEFUNCTION_SOCKET_IN(unsigned int)
            else if
                ADD_SIMPLEFUNCTION_SOCKET_IN(unsigned long int)
            else if
                ADD_SIMPLEFUNCTION_SOCKET_IN(unsigned long long int)
...
#define DEFINE_LINEWRITER2(type, typeName)                                                            \
    ...
    {                                                                                                 \
        ...
        {                                                                                             \
            ...
            if (file.empty())                                                                         \
            {                                                                                         \
                GRSF_ERRORMSG_PARSERTOOL("---> String conversion 'file': " << file << " failed!", id) \
            }                                                                                         \
        }                                                                                             \
...
#define DEFINE_LINEWRITER(type) DEFINE_LINEWRITER2(type, type)
    ...
    void createToolLineWriter(XMLNodeType& logicNode, unsigned int id)
    {
        ...
        LogicNode* n;
        if
            DEFINE_LINEWRITER(float)
        else if
            DEFINE_LINEWRITER(double)
        else if
            DEFINE_LINEWRITER(char)
        else if
            DEFINE_LINEWRITER(short)
        else if
            DEFINE_LINEWRITER(int)
        else if
            DEFINE_LINEWRITER(long int)
        else if
            DEFINE_LINEWRITER(long long int)
        else if
            DEFINE_LINEWRITER(unsigned char)
        else if
            DEFINE_LINEWRITER(unsigned short)
        else if
            DEFINE_LINEWRITER(unsigned int)
        else if
            DEFINE_LINEWRITER(unsigned long int)
        else if
            DEFINE_LINEWRITER(unsigned long long int)
        else if
            DEFINE_LINEWRITER2(std::string, string)
        else if
            DEFINE_LINEWRITER2(boost::filesystem::path, path)
        else
        {
            GRSF_ERRORMSG_PARSERTOOL("---> String conversion 'inputType': '" << t << "' not found!", id);
        }
...
#define DEFINE_XMLLINEWRITER(type) DEFINE_XMLLINEWRITER2(type, type)
    ...
    void createToolXMLLineWriter(XMLNodeType& logicNode, unsigned int id)
    {
        ...
        LogicNode* n;
        if
            DEFINE_XMLLINEWRITER(float)
        else if
            DEFINE_XMLLINEWRITER(double)
        else if
            DEFINE_XMLLINEWRITER(char)
        else if
            DEFINE_XMLLINEWRITER(short)
        else if
            DEFINE_XMLLINEWRITER(int)
        else if
            DEFINE_XMLLINEWRITER(long int)
        else if
            DEFINE_XMLLINEWRITER(long long int)
        else if
            DEFINE_XMLLINEWRITER(unsigned char)
        else if
            DEFINE_XMLLINEWRITER(unsigned short)
        else if
            DEFINE_XMLLINEWRITER(unsigned int)
        else if
            DEFINE_XMLLINEWRITER(unsigned long int)
        else if
            DEFINE_XMLLINEWRITER(unsigned long long int)
        else if
            DEFINE_XMLLINEWRITER2(std::string, string)
        else if
            DEFINE_XMLLINEWRITER2(boost::filesystem::path, path)
        else
        {
            GRSF_ERRORMSG_PARSERTOOL("---> String conversion 'inputType': " << t << "' not found!", id);
        }
    ...
    void createToolStopNode(XMLNodeType& logicNode, unsigned int id)
    {
        ...
        {
            GRSF_ERRORMSG_PARSERTOOL("No group found for "
                                         << "stopGroupId: "
                                         << stopGroupId,
                                     id)
        }
    ...
    void createToolOOBBCollider(XMLNodeType& logicNode, unsigned int id)
    {
        ...
        {
            GRSF_ERRORMSG_PARSERTOOL("---> String conversion 'minPoint' failed", id);
        }
        Vector3 maxPoint;
        ...
        LOGLPLEVEL3(m_pLog,
                    "---> OOBBCollider with aabb: min: " << aabb.m_minPoint << "max: " << aabb.m_maxPoint << " R_KI: "
                                                         << std::endl
                                                         << R_KI
                                                         << std::endl;)
    ...
    void createToolTransform3D(XMLNodeType& logicNode, unsigned int id)
    {
        ...
        LOGLPLEVEL3(m_pLog,
                    "---> Transform3D with  R_KI: " << std::endl
                                                    << q_KI.toRotationMatrix()
                                                    << std::endl
                                                    << " trans: "
                                                    << I_r_IK.transpose()
                                                    << std::endl;)
