...
public:
    ...
    void setOutput(boost::filesystem::path filePath, double time, unsigned int stateIdx)
    {
...
private:
    struct AddBytesVisitor
    {
        ...
        void operator()(
            ABD::AddBytes<EnumConversion::toIntegral(ABD::TypeEnum::PROCESS_MATERIAL_OVERLAP_GLOBGEOMID)>* add)
        {
            SET_OSOCKET_VALUE_PTR(m_p, ProcessId, add->m_processId);
            SET_OSOCKET_VALUE_PTR(m_p, MaterialId, add->m_materialId);
            SET_OSOCKET_VALUE_PTR(m_p, OverlapTotal, add->m_overlapTotal);
...
public:
    ...
    OOBBCollider(unsigned int id, AABB3d& aabb, Matrix33& R_KI) : LogicNode(id), m_aabb(aabb), m_R_KI(R_KI)
    {
