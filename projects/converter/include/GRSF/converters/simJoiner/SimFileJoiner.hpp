...
#ifndef GRSF_converters_simJoiner_SimFileJoiner_hpp
#define GRSF_converters_simJoiner_SimFileJoiner_hpp
...
private:
    ...
    struct ListOrRangeTypes
    {
        ...
        using VariantType =
            boost::variant<ListType, RangeType, AllType>;  // Indexes into the variants are defined above!
    };
...
private:
                 ...
                 bool firstFile = true)
    {
        ...
        for (std::streamoff stateIdx = 0; stateIdx < fromFile.m_nStates; stateIdx++)
        {
            ...
            fromFile.m_file_stream.seekg(fromFile.m_nBytesPerState - std::streamoff(sizeof(double)),
                                         std::ios_base::cur);
        }
        std::sort(fileTimesAndOff.begin(),
                  fileTimesAndOff.end(),
                  [](const TimeOffsetType& lhs, const TimeOffsetType& rhs) { return lhs.first < rhs.first; });
        // Scan finished
        std::cerr << "---> Scan finished!" << std::endl;
        std::cerr << "---> TimeRange File: [" << fileTimesAndOff.begin()->first << ","
                  << fileTimesAndOff.rbegin()->first << "]" << std::endl;
        ...
        std::vector<TimeOffsetType>
            matchedTimesAndRelOff;  // Offset relative to last time, first time has offset relative to m_beginOfStates !
        ...
        if (*timeList.rbegin() < itFileTime->first)
        {  // max range time < smallest filetime
            ...
            while (currMatchListTime != timeList.end() && itFileTime != fileTimesAndOff.end())
            {
                if (itFileTime->first < *currMatchListTime)
                {
                    itFileTime++;  // move file times
                         ...
                         ||
                         (std::next(currMatchListTime) == timeList.end() &&
                          *currMatchListTime <
                              itFileTime->first)  // or if we are at the last listTime and fileTime is bigger
                         ||
                         (std::next(currMatchListTime) != timeList.end() && *currMatchListTime < itFileTime->first &&
                          itFileTime->first < *std::next(currMatchListTime))  // // itFileTime->first im intervall
                                                                              // (currMatchListTime,
                                                                              // currMatchListTime++)
                         )
                {
                    // match found
                    std::cerr << "---> Matched: rangeTime: " << *currMatchListTime << "\t\t <--- \t\t"
                              << itFileTime->first << " : fileTime " << std::endl;
             ...
             matchedTimesIt++)
        {
            ...
            for (unsigned int body = 0; body < fromFile.m_nSimBodies; body++)
            {
                ...
                auto res = boost::apply_visitor(InListOrInRangeBodyVisitorType(RigidBodyId::getBodyNr(id), bodyCounter),
                                                bodyRange);
                if (res > 0)
                {
            ...
            if (!firstRun)
            {
                ...
                {
                    GRSF_ERRORMSG("At time :" << currentTime << " only " << bodyCounter << " bodies found, instead of "
                                              << initBodyCounter
                                              << " as in first found state!")
                }
            }
            else
            {
                 ...
                 bool firstFile = true)
    {
        ...
        if (lastTimeWritten > fileTimesAndOff.rbegin()->first)
        {  // max fileTime needs to be greater then lastTimeWritten
            GRSF_ERRORMSG("last time written: " << lastTimeWritten << " is greater then max file time: "
                                                << fileTimesAndOff.rbegin()->first)
        }
        ...
        if (!firstFile)
        {
            startIt = std::upper_bound(fileTimesAndOff.begin(),
                                       fileTimesAndOff.end(),
                                       lastTimeWritten,
