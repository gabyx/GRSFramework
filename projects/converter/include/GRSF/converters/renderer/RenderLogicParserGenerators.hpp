...
struct SceneParserGen
{
    ...
    createParserModules(TParser* p)
    {
        ...
        auto geom = std::unique_ptr<GeometryModuleType>(
            new GeometryModuleType(p, &m_p->m_globalGeometries, nullptr, &m_p->m_geometryMap));
        auto is  = std::unique_ptr<InitStatesModuleType>(nullptr);
        auto vis = std::unique_ptr<VisModuleType>(
            new VisModuleType(p, &m_p->m_scales, &m_p->m_visMeshs));  // no visualization needed
        auto bm =
            std::unique_ptr<BodyModuleType>(new BodyModuleType(p, geom.get(), nullptr, vis.get(), nullptr, nullptr));
...
struct LogicParserGen
{
    ...
    typename TParser::ParserTraits::TupleModules createParserModules(TParser* p)
    {
        ...
        auto logic =
            std::unique_ptr<LogicModuleType>(new LogicModuleType(p, m_g, &m_p->m_geometryMap, &m_p->m_materials));
