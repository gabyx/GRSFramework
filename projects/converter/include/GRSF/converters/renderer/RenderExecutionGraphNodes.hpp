...
class ColorList : public LogicNode
{
    ...
    template <typename T>
    ColorList(unsigned int id, T&& colors, double amplitude = 1.0) : LogicNode(id), m_colors{std::forward<T>(colors)}
    {
        ADD_ISOCK(Enable, true);
        ADD_ISOCK(Index, 0);
        ADD_OSOCK(Color, Vector3(0.5, 0.5, 0.5));
...
class MatteMaterial : public LogicNode
{
    ...
    MatteMaterial(unsigned int id, const Vector3& color = Vector3(0.5, 0.5, 0.5), double ka = 1.0, double kd = 1.0)
        : LogicNode(id)
    {
        ADD_ISOCK(Enable, true);
    ...
    virtual void compute()
    {
        if (GET_ISOCKET_REF_VALUE(Enable))
        {
            ...
            m_material << "Color [" << GET_ISOCKET_REF_VALUE(Color).format(MyMatrixIOFormat::SpaceSep) << "]\n"
                       << "Surface \"matte\" \"float Kd\" [" << std::to_string(GET_ISOCKET_REF_VALUE(Kd))
                       << "] \"float Ka\" [" << std::to_string(GET_ISOCKET_REF_VALUE(Ka))
    ...
    virtual void compute()
    {
        if (GET_ISOCKET_REF_VALUE(Enable))
        {
            ...
            m_material << "Bxdf \"PxrDisney\" \"0\" "
                       << " \"color baseColor\" ["
                       << GET_ISOCKET_REF_VALUE(BaseColor).transpose().format(MyMatrixIOFormat::SpaceSep) << "]"
                       << " \"color emitColor\" ["
                       << GET_ISOCKET_REF_VALUE(EmitColor).transpose().format(MyMatrixIOFormat::SpaceSep) << "]"
                       << " \"color subsurfaceColor\" ["
                       << GET_ISOCKET_REF_VALUE(SubsurfaceColor).transpose().format(MyMatrixIOFormat::SpaceSep) << "]"
...
class RendermanWriter : public RenderScriptWriter
{
    ...
    RendermanWriter(unsigned int id, GeometryMapType* geomMap, bool pipeToSubprocess = false, std::string command = "")
    ...
    void initFrame()
    {
        ...
        if (!GET_ISOCKET(Folder)->isConnected() || !GET_ISOCKET(Name)->isConnected() ||
            !GET_ISOCKET(Time)->isConnected() || !GET_ISOCKET(FrameNr)->isConnected())
        {
            GRSF_ERRORMSG(
                "RendermanWriter::initState --> one of Folder,Name,Time,FrameNr sockets not connected to any FrameData "
                "node!")
        }
    ...
    void dumpStream()
    {
        ...
        else
        {
            ...
            // ASSERTMSG(m_frameFile.good(),"FUCK"); // stream will fail because s can contain nothing, therefore
            // clear()
            m_frameFile.clear();
        }
        m_s.clear();
