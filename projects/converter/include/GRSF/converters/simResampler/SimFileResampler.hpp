...
private:
    ...
    void resample()
    {
        ...
        if (m_iFiles.size() != 1)
        {
            GRSF_ERRORMSG("Only one input file is accepted for resample!")
        }
        ...
        for (std::streamoff stateIdx = m_startStateIdx; stateIdx <= m_endStateIdx; stateIdx += m_stepSize)
        {
            if (m_splitStatesIntoFiles)
            {
    ...
    std::unique_ptr<MultiBodySimFile> openFile(MultiBodySimFile& fromFile, boost::filesystem::path f)
    {
        ...
        {
            GRSF_ERRORMSG(toFile->getErrorString());
        };
