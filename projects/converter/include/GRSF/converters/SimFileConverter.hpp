...
public:
    ...
    struct ConvertSettings
    {
        ...
        /** Providing addState and FullState=true , the implementation in this class which checks if addState is
        * provided and so on
        ...
        * Other this implementation is an overkill and this class should only provide addState which directly reads the
        * whole state always
        * if the stepper functors provided to convert need to iterate over the rigid body states they can for sure do
        * so.
        * we leave this overkill implementation as is because iterator interface for simfile is a very good idea, but
        * has no priority now.
        * We can also not hand some sim file read iterators over to addState because, we dont want to read several times
        * for multiple steppers
    ...
    void convert(TSimFileStepper&&... simFileStepper)
    {
        ...
        for (auto file : m_inputFiles)
        {
            ...
            if (file.extension() == ".xml")
            {
                ...
                for (auto n : node.children("File"))
                {
                    ...
                    if (att)
                    {
                        ...
                        if (useRange)
                        {
                            ...
                            if (att)
                            {
                                ...
                                if (endIdxS >= 0)
                                {  // if not negative set to parsed value
                                    endIdx = endIdxS;
                                }
                            }
                    ...
                    if (!useRange)
                    {
                        ...
                        std::sort(stateIndices.begin(), stateIndices.end(), [](StateIndex& a, StateIndex& b) {
                        ...
                        if (!stateIndices.empty())
                        {
                            for (auto it = stateIndices.begin(); it != (--stateIndices.end()); ++it)
                            {
                                ...
                                {
                                    GRSF_ERRORMSG(" The file: " << file.filename() << " contains duplicate states idx: "
                                                                << it->m_idx);
                                }
                            }
                        }
                        ...
                        LOG(m_log,
                            "---> Parsed " << stateIndices.size() << " state of file: " << path << "from XML: "
                                           << file.filename()
                                           << std::endl;)
                        if (stateIndices.size() > 0)
                        {
                            convertFile<TSettings, TSimFileStepper...>(path,
                                                                       uuid,
                                                                       stateIndices,
                                                                       startIdx,
                                                                       endIdx,
                    ...
                    {  // use range
                        ...
                        {
                            LOG(m_log,
                                "---> Use state range: [ " << startIdx << ":" << increment << ": -1 ) of file: " << path
                                                           << "from XML: "
                                                           << file.filename()
                                                           << std::endl;)
                        }
                        ...
                        {
                            LOG(m_log,
                                "---> Use state range: [ " << startIdx << ":" << increment << ":" << endIdx
                                                           << " ) of file: "
                                                           << path
                                                           << "from XML: "
                                                           << file.filename()
            ...
            else
            {
                ...
                convertFile<TSettings, TSimFileStepper...>(file,
...
protected:
    ...
    struct details
    {
        ...
        template <typename T, typename Dummy = decltype(ContainerTags::is_container(std::declval<T>()))>
        ...
        /** If stepper class fails to compile here, you either have missing addState or addBodyState function in your
        * stepper
        ...
        */
        ...
        template <typename T,
                  typename Dummy =
                      decltype(ContainerTags::is_container(std::declval<T>() /*make reference, no need for CTOR*/))>
...
#define SIMFILESTEPPER_DISPATCH_FUNC_STOP(__name__)  \
    ...
    {                                                \
        ...
        struct StepperDispatch<T, std::false_type>
        {
             ...
             * Making different steppers and stop and not stop in different loops, needs dynamic managing during the
             * loop
...
#undef SIMFILESTEPPER_DISPATCH_FUNC_CONT_STOP
                     ...
                     TSimFileStepper&&... simFileStepper)
    {
        ...
        ApplicationSignalHandler::getSingleton().registerCallback(
            SIGINT, std::bind(&SimFileConverter::callbackAbort, this), "SimFileConverter");
        ApplicationSignalHandler::getSingleton().registerCallback(
            SIGUSR2, std::bind(&SimFileConverter::callbackAbort, this), "SimFileConverter");
        ...
        EXPAND_PARAMETERPACK(details::StepperDispatch<TSimFileStepper>::initSimInfo(
            simFileStepper, f, perFileOutputFile, m_simFile.getNSimBodies(), m_simFile.getNStates()))
        ...
        while (m_simFile.isGood() && !m_abort)
        {
            ...
            LOG(m_log,
                "---> Init state with: \n\toutputFile: " << outputFile << "\nmappedIdx: " << mappedIdx << "\n\ttime: "
                                                         ...
                                                         << std::endl;)
            start = timer.elapsedMilliSec();
            EXPAND_PARAMETERPACK(
                details::StepperDispatch<TSimFileStepper>::initState(simFileStepper, outputFile, time, mappedIdx))
            avgInitFrameTime += timer.elapsedMilliSec() - start;

            start = timer.elapsedMilliSec();
            /** apply full state or body state loop  for every stepper type */
            EXPAND_PARAMETERPACK((details::FullStateOrBodyState<TSimFileStepper, TSettings::FullState>::apply(
                this, simFileStepper, states)))
            ...
            if (!stateIndices.empty())
            {
                if (++itStateIdx != stateIndices.end())
                {
                    ...
                    {
                        GRSF_ERRORMSG("Negative or same as privious state idx: " << itStateIdx->m_idx
                                                                                 << " in xml for file: "
                                                                                 << f)
                    }
        ...
        if (!stateIndices.empty() && m_terminatedByStepper == false)
        {
            ...
            {
                LOG(m_log,
                    "---> Warning: Reading simfile: " << f
                                                      << " became invalid before all state inidices have been converted"
                                                      << std::endl)
            }
        }

        LOG(m_log,
            "---> Converter Speed:" << std::endl
                                    << "Avg. Load State : "
                                    << (avgStateLoadTime / m_stateCounter)
                                    << " ms"
                                    << std::endl
                                    << "Avg. Init State : "
                                    << (avgInitFrameTime / m_stateCounter)
                                    << " ms"
                                    << std::endl
                                    << "Avg. Time per State : "
                                    << (avgStateTime / m_stateCounter)
                                    << " ms"
                                    << std::endl
                                    << "Avg. Time per Body: "
                                    << (avgStateTime / (m_bodyCounter))
                                    << " ms"
                                    << std::endl;)
