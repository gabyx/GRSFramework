                                                         ...
                                                         std::string folder_prefix)
{
    ...
    // std::cout << "RELDIR:" << relDirectoryPath << " FILEIDCOUNTER: "<< m_folderIdCounter <<" PREFIX: " <<
    // folder_prefix<< std::endl;
...
void FileManager::updateAllSimDataFiles(const boost::filesystem::path& directory, const bool& with_SubDirs = false)
{
    ...
    if (exists(directory))
    {
        ...
        for (directory_iterator iter(directory); iter != end; ++iter)
        {
            ...
            {
                if (with_SubDirs)
                    updateAllSimDataFiles(*iter, with_SubDirs);
            }
            else
            {
                // Check for filename
    ...
    if (exists(directory))
    {
        ...
        for (directory_iterator iter(directory); iter != end; ++iter)
        {
            if (is_directory(*iter))
            {
                ...
                if (with_SubDirs)
                    scanAllSimFolders(*iter, folder_prefix, with_SubDirs);
            }
        }
    }
