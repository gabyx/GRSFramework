...
bool StateRecorder::createSimFile(boost::filesystem::path file_path)
{
    ...
    if (m_binarySimFile.openWrite(
            file_path, LayoutConfigType::LayoutType::NDOFqBody, LayoutConfigType::LayoutType::NDOFuBody, m_nSimBodies))
    {
        return true;
    }
    return false;
...
bool StateRecorder::createSimFileCopyFromReference(boost::filesystem::path new_file_path,
                                                   boost::filesystem::path ref_file_path)
{
    ...
    bool fileOK = tmpFile.openRead(ref_file_path,
                                   ...
                                   m_nSimBodies);  // Open file to see if this file fits our simulation!!
    tmpFile.close();
