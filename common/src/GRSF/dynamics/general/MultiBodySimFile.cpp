...
void MultiBodySimFile::setByteLengths()
{
    ...
    m_nBytesPerBody =
        m_nBytesPerUBody + m_nBytesPerQBody + std::streamsize(sizeof(RigidBodyIdType)) + m_nAdditionalBytesPerBody;
                                 ...
                                 bool                           truncate)
{
    ...
    {
        m_errorString << "Wrong openWrite parameters: nDOFqBody:" << nDOFqBody << " nDOFuBody: " << nDOFuBody
                      << " nSimBodies:" << nSimBodies << std::endl;
        return false;
    }

    // Determine bytesType, and additional bytes, if necessary

    return openWrite_impl(file_path, nDOFqBody, nDOFuBody, nSimBodies, truncate);
}
                                      ...
                                      std::streamsize                additionalBytesPerBody)
{
    ...
    if (truncate)
    {
        m_file_stream.open(file_path.string().c_str(),
    ...
    {
        // Here we need to check that, we set the put position exactly at the last state, we might have overhanging bits
        // because user has cancled and some binary stuff is hanging at the end!!!
...
void MultiBodySimFile::writeHeader()
{
    ...
    *this << (unsigned int)SIM_FILE_VERSION << (unsigned int)m_nSimBodies << (unsigned int)m_nDOFqBody
          << (unsigned int)m_nDOFuBody << (unsigned int)(EnumConversion::toIntegral(m_additionalBytesPerBodyType))
...
bool MultiBodySimFile::readHeader()
{
    ...
    if (std::strncmp(signature, m_simFileSignature, SIM_FILE_SIGNATURE_LENGTH) == 0)
    {
        ...
        *this >> version;
        ...
        {
            m_errorString << " Binary file has version: " << version
                          << " which does not fit the compiled version: " << SIM_FILE_VERSION << std::endl;
            return false;
        }
        ...
        *this >> nBodies >> nDofqBody >> nDofuBody >> addType >> addBytesPerBody;
        ...
        {
            m_errorString << "Binary File, m_nAdditionalBytesPerBody not valid : " << m_nAdditionalBytesPerBody
                          << std::endl;
            return false;
        }
        ...
        {
            m_errorString << " Binary file does not correspond to the number of bodies which should be simulated: "
                          << std::endl
                          << " Binary File describes: \t\tnSimBodies = " << nBodies << "\tnDofqBody = " << nDofqBody
                          << "\tnDofuBody = " << nDofuBody << std::endl
                          << " Simulation requests: \t\tnSimBodies = " << m_nSimBodies
                          << "\tnDofqBody = " << m_nDOFqBody << "\tnDofuBody = " << m_nDOFuBody << std::endl;
        }
    }
    else
    {
        m_errorString << "Binary file contains a wrong header and is not equal to: '" << m_simFileSignature << "'"
                      << std::endl;
    }
