...
MoreauTimeStepper::~MoreauTimeStepper(){DESTRUCTOR_MESSAGE};
...
const DynamicsState* MoreauTimeStepper::getBackStateBuffer()
{
    return m_StateBuffers.m_pBack;
}

const DynamicsState* MoreauTimeStepper::getFrontStateBuffer()
{
...
void MoreauTimeStepper::doTimeStep()
{
    ...
    LOGSLLEVEL1(m_pSolverLog,
                "---> Iteration Time: " << std::setprecision(5) << (m_endTime - m_startTime) << "[sec]" << std::endl
                                        << "---> End time-step ===================================="
                                        << std::endl
                                        << std::endl;);
