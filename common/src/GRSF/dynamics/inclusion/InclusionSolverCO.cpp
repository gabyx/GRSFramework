...
void InclusionSolverCO::reset()
{
    ...
    m_pCollisionSolver->addContactDelegate(
        CollisionSolverType::ContactDelegateType::from_method<ContactGraphType, &ContactGraphType::addNode>(
            &m_contactGraph));
    ...
    LOG(m_pSimulationLog,
        "Set GPU Device : " << props.name << ", PCI Bus Id: " << props.pciBusID << ", PCI Device Id: "
                            << props.pciDeviceID
                            << std::endl;);
#endif
}
...
void InclusionSolverCO::solveInclusionProblem()
{
    ...
    for (auto it = m_simBodies.begin(); it != m_simBodies.end(); ++it)
    {
        ...
        pBody->m_pSolverData->m_uBuffer.m_front =
            pBody->m_pSolverData->m_uBuffer.m_back +
        ...
        for (auto& currentContactNode : nodes)
        {
            ...
            for (it = currentContactNode->m_edgeList.begin(); it != currentContactNode->m_edgeList.end(); ++it)
            {
                ...
                {  // We are on a self referencing edge! Thats good build diagonal of G and parts of c
                    ...
                    m_T.block<(ContactDim), (ContactDim)>((ContactDim)*i, (ContactDim)*j).noalias() +=
                        W_i_bodyT_M_body * (*W_i_body);
                    m_d.segment<ContactDim>((ContactDim)*i).noalias() +=
                        W_i_bodyT_M_body * edgesBody->m_h_term * m_settings.m_deltaT +
                        I_plus_eps.asDiagonal() *
                            ((*W_i_body).transpose() * edgesBody->m_pSolverData->m_uBuffer.m_back);
                }
                else
                {
                    ...
                    m_T.block<(ContactDim), (ContactDim)>((ContactDim)*j, (ContactDim)*i).noalias() +=
                        G_part.transpose();
                }
            }
            ...
            m_d.segment<ContactDim>((ContactDim)*i).noalias() +=
                I_plus_eps.asDiagonal() * currentContactNode->m_nodeData.m_chi;
        }
// =============================================================================================================
        ...
        m_G_notDiagDominant =
            (m_T.diagonal().array().abs() <
             (m_T.array().abs().matrix().rowwise().sum() - m_T.diagonal().array().abs().matrix()).array())
                .count();
#endif

        LOGSLLEVEL3_CONTACT(m_pSolverLog,
                            " G= ..." << std::endl
                                      << m_T.format(MyMatrixIOFormat::Matlab)
                                      << ";"
                                      << std::endl;);
        LOGSLLEVEL3_CONTACT(m_pSolverLog,
                            " c= " << m_d.transpose().format(MyMatrixIOFormat::Matlab) << "';" << std::endl;)
        LOGSLLEVEL2_CONTACT(m_pSolverLog,
                            " P_back= " << P_back.transpose().format(MyMatrixIOFormat::Matlab) << "';" << std::endl;);
        ...
        LOGSLLEVEL2_CONTACT(m_pSolverLog,
                            " P_front= " << P_front.transpose().format(MyMatrixIOFormat::Matlab) << "';" << std::endl;);
        ...
        for (auto& node : nodes)
        {
            ...
            {
                pBody->m_pSolverData->m_uBuffer.m_front +=
                    pBody->m_MassMatrixInv_diag.asDiagonal() * node->m_nodeData.m_W_body[0] *
                    (*m_P_front).segment<ContactDim>(ContactDim * node->m_nodeNumber);
            }
            ...
            {
                pBody->m_pSolverData->m_uBuffer.m_front +=
                    pBody->m_MassMatrixInv_diag.asDiagonal() * node->m_nodeData.m_W_body[1] *
                    (*m_P_front).segment<ContactDim>(ContactDim * node->m_nodeNumber);
            }
        }
    }
...
void InclusionSolverCO::setupRMatrix(PREC alpha)
{
    ...
    LOGSLLEVEL3_CONTACT(m_pSolverLog,
                        " R= "
                            << "diag("
                            << m_R.transpose().format(MyMatrixIOFormat::Matlab)
                            << "');"
                            << std::endl;);
}
...
void InclusionSolverCO::doSorProx()
{
    ...
    while (true)
    {
        ...
        for (unsigned int i = 0; i < m_nContacts; i++)
        {
            ...
            P_front.template segment<ContactDim>((ContactDim)*i) =
                m_T.block((ContactDim)*i, 0, ContactDim, m_nLambdas) * P_front +
                m_d.template segment<ContactDim>((ContactDim)*i);

            Prox::ProxFunction<ConvexSets::RPlusAndDisk>::doProxSingle(
                m_mu(i), P_front.template segment<ContactDim>((ContactDim)*i));
