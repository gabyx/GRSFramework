...
CollisionSolver::CollisionSolver(std::shared_ptr<DynamicsSystemType> pDynSys)
    : m_simBodies(pDynSys->m_simBodies), m_staticBodies(pDynSys->m_staticBodies), m_Collider(&m_collisionSet)
{
}
...
const typename CollisionSolver::CollisionSetType& CollisionSolver::getCollisionSetRef()
{
    ...
    if (m_collisionSet.size() != 0)
    {
        for (auto colDataPtr : m_collisionSet)
        {
            GRSF_ASSERTMSG(std::abs(colDataPtr->m_cFrame.m_e_x.dot(colDataPtr->m_cFrame.m_e_y)) < 1e-3 &&
                               std::abs(colDataPtr->m_cFrame.m_e_y.dot(colDataPtr->m_cFrame.m_e_z)) < 1e-3,
                           "Vectors not orthogonal");

            LOGSLLEVEL3_CONTACT(m_pSolverLog,
                                "---> Contact Frame: n: " << colDataPtr->m_cFrame.m_e_z.transpose() << std::endl;)
