...
public:
    ...
    SettingsModule(ParserType* p, RecorderSettingsType* r, TimeStepperSettingsType* t, InclusionSolverSettingsType* i)
        : m_recorderSettings(r)
        , m_timestepperSettings(t)
        , m_inclusionSettings(i)
        , m_parser(p)
    ...
    void parse(XMLNodeType sceneSettings)
    {
        ...
        if (m_timestepperSettings)
        {
            ...
            if (node)
            {
                ...
                if (enabled)
                {
                    ...
                    {
                        GRSF_ERRORMSG("---> String conversion in SimulateFromReference: type. The type '" + type +
        ...
        if (m_inclusionSettings)
        {
            ...
            if (att)
            {
                ...
                {
                    GRSF_ERRORMSG(
                        "---> String conversion in InclusionSolverSettings: matrixRStrategy failed: not a valid "
                        "setting");
                }
            }
            else
            ...
            if (att)
            {
                if (!Utilities::stringToType(m_inclusionSettings->m_computeOverlap, att.value()))
                {
            ...
            if (att)
            {
                ...
                {
                    if (!Utilities::stringToType(m_inclusionSettings->m_driftCorrectionGapAlpha,
                                                 node.attribute("driftCorrectionGapAlpha").value()))
                    {
                        GRSF_ERRORMSG(
                            "---> String conversion in InclusionSolverSettings: driftCorrectionGapAlpha failed");
                    }
                }
            }
        ...
        if (m_recorderSettings)
        {
            ...
            else if (method == "everyXTimeStep")
            {
                ...
                else
                {
                    ...
                    {
                        GRSF_ERRORMSG(
                            "---> String conversion in RecorderSettings: everyXTimeStep failed: everyXStep (dominant) "
                            "nor statesPerSecond present")
                    }
                }
            }
...
public:
    ...
    GeometryModule(ParserType* p,
        ...
        , m_externalGeometryCache(geomMap)
        , m_externalScalesGroupCache(scalesGroup){
...
private:
    ...
    void parseSphereGeometry(XMLNodeType sphere)
    {
        ...
        if (type == "uniform")
        {
            ...
            else
            {
                ...
                {
                    LOGSCLEVEL3(m_pSimulationLog,
                                "\t---> Body id:" << RigidBodyId::getBodyIdString(b.m_id) << ", GeometryType: Sphere"
                                                  << std::endl);
        ...
        else if (type == "random")
        {
            ...
            else
            {
                ...
                for (auto& b : *m_bodiesGroup)
                {
                    ...
                    radius = Utilities::genRandomValues<PREC>(
                        radius, sampleGen, b.m_id - diffId);  // (id:16 - id:13 = 3 values, 13 is already generated)
        ...
        {
            GRSF_ERRORMSG("---> The attribute 'distribute' '" + type +
    ...
    void parseCapsuleGeometry(XMLNodeType capsule)
    {
        ...
        if (type == "uniform")
        {
            ...
            */
            ...
            if (m_addToGlobalGeoms && Options::m_allocateGeometry)
            {
                ...
                // no scale (here)
                addToGlobalGeomList(id,
                                    std::shared_ptr<CapsuleGeometry>(new CapsuleGeometry(/*,p*/ n, length, radius)));
            }
            else
            {
                if (Options::m_allocateGeometry)
        ...
        {
            GRSF_ERRORMSG("---> The attribute 'type' '" + type +
                          std::string("' of 'Capsule' has no implementation in the parser"));
        }
    }
    ...
    void parseGlobalGeomId(XMLNodeType globalGeomId)
    {
        ...
        else if (distribute == "linear")
        {
            ...
            for (auto& b : *m_bodiesGroup)
            {
                ...
                {
                    auto globalScaleIt =
                        m_scalesGlobal.find(id);  // some geometries might not have a scale , e.g. halfspace
                    if (globalScaleIt != m_scalesGlobal.end())
                    {
                        (*m_externalScalesGroupCache)[bodyIdx] = globalScaleIt->second;
                    }
        ...
        {
            GRSF_ERRORMSG("---> The attribute 'distribute' '" + distribute +
...
public:
    ...
    ContactParamModule(ParserType* p, ContactParameterMapType* c)
        : m_pSimulationLog(p->getSimLog()), m_contactParams(c){};
    ...
    void parseForceField(XMLNodeType forceField)
    {
        ...
        if (type == "spatialspherical-timerandom")
        {
            ...
            m_forceList->addExternalForceCalculation(new SpatialSphericalTimeRandomForceField(
                seed, boostTime, pauseTime, startTime, endTime, amplitude, AABB3d(boxMin, boxMax), randomOn));
...
public:
    ...
    void parseGlobalInitialCondition(XMLNodeType sceneSettings)
    {
        ...
        if (initCond)
        {
            ...
            if (useTime)
            {
                ...
                {
                    GRSF_ERRORMSG(
                        "---> In GlobalInitialCondition: useTimeToContinue = true, but cannot set timestepper "
                        "settings!");
                }
                               ...
                               bool addToInitList = true)
    {
        ...
        else if (s == "posvel")
        {
            ...
            {
                GRSF_ERRORMSG("---> The attribute 'distribute' '"
                              << s
        ...
        {
            LOGSCLEVEL3(m_pSimulationLog,
                        "\t---> InitState id: " << itState->m_id << ": " << itState->m_q.transpose() << " , "
                                                << itState->m_u.transpose()
                                                << std::endl;)
            if (b.m_body)
            {
...
private:
    ...
    void parseInitialPositionLinear(XMLNodeType initCond)
    {
        ...
        InitialConditionBodies::setupPositionBodiesLinear(
            *m_bodiesGroup, m_statesGroup, m_startIdGroup, pos, dir, dist, jitter, delta, seed);
    }
    void parseInitialPositionGrid(XMLNodeType initCond)
    {
        ...
        InitialConditionBodies::setupPositionBodiesGrid(
            *m_bodiesGroup, m_statesGroup, m_startIdGroup, gridX, gridY, dist, trans, jitter, delta, seed, dirZ, dirX);
    }
    ...
    void parseInitialVelocityTransRot(XMLNodeType initCond)
    {
        ...
        for (auto itNode = nodes.begin(); itNode != itNodeEnd; ++itNode)
        {
            if (bodyIt == itEnd)
            {
                LOGSCLEVEL2(
                    m_pSimulationLog,
                    "---> InitialVelocityTransRot: You specified to many velocities (size: " << nodeCounter
                                                                                             << "), -> neglecting ..."
                                                                                             << std::endl;);
                break;
            }
    ...
    //    template<template C, typename std::enable_if< ContainerTags::is_associative(*static_cast<C*>(0)), int >::type
    //    = 0 >
    ...
    //    template<template C, typename std::enable_if< ! ContainerTags::is_associative(*static_cast<C*>(0)), int
    //    >::type  = 0 >
    //    bool addState(C & cont, RigidBodyIdType id, RigidBodyState & state){
...
struct BodyModuleDynamicOptions
{
    ...
    BodyRangeType m_bodyIdRange;  ///< Range of body ids, original list which is handed to parseScene, this range is
                                  /// only applied to enableSelectiveIds="true"
};
...
struct BodyModuleStaticOptions
{
    ...
    static const bool m_parseAllBodiesNonSelGroup =
        parseAllBodiesNonSelGroup;  ///< Parse all bodies in groups where m_bodyIdRange is not applied
    ...
    static const bool m_allocateSimBodies =
        allocateSimBodies;  ///< if false, does only parse the nodes which do not need the body -> initial condition
    static const bool m_allocateStaticBodies =
        allocateStaticBodies;  ///< if false, does only parse the nodes which do not need the body -> initial condition

    static const bool m_parseInitialCondition =
        parseInitialCondition;  ///< if false, the initial conditions are not parsed!
};
...
public:
    ...
    {
        LOGSCLEVEL1(m_pSimulationLog,
                    "==== BodyModule: parsing (ModuleOptions) ===========================" << std::endl;)
        ...
        if (selectIds)
        {
            ...
            else
            {
                ...
                if (n)
                {
                    ...
                    LOGSCLEVEL2(
                        m_pSimulationLog,
                        "---> Overwrite SelectiveIdRange with Range: [" << RigidBodyId::getBodyIdString(r.first) << ", "
                                                                        << RigidBodyId::getBodyIdString(r.second)
                                                                        << ", linear: "
                                                                        << m_opts.m_bodyIdRange.isLinear()
                                                                        << "]"
                                                                        << std::endl;)
                }
            }
    ...
    {
        LOGSCLEVEL1(m_pSimulationLog,
                    "==== BodyModule: parsing (SceneObjects) ============================" << std::endl;)

        LOGSCLEVEL1(m_pSimulationLog,
                    "---> BodyModule Options: " << std::endl
                                                << "\t parse simulated bodies:"
                                                << (bool)Options::m_parseSimBodies
                                                << std::endl
                                                << "\t parse static bodies:"
                                                << (bool)Options::m_parseStaticBodies
                                                << std::endl
                                                << "\t parse all bodies in group with disabled selective ids:"
                                                << (bool)Options::m_parseAllBodiesNonSelGroup
                                                << std::endl
                                                << "\t parse selective ids: "
                                                << m_parseSelectiveBodyIds
                                                << std::endl
                                                << "\t allocate simulated bodies: "
                                                << (bool)Options::m_allocateSimBodies
                                                << std::endl
                                                << "\t allocate static bodies: "
                                                << (bool)Options::m_allocateStaticBodies
                                                << std::endl;)
    ...
    {
        std::stringstream s;
        s << suffix << "BodyModule Statistics: " << std::endl << suffix << "\tGroup Mass (id,[kg]):";
...
private:
    void parseRigidBodies(XMLNodeType rigidbodies)
    {
        ...
        if ((!Options::m_parseAllBodiesNonSelGroup && !hasSelectiveFlag) ||
            ...
            (m_parseSelectiveBodyIds && hasSelectiveFlag &&
             m_startRangeIdIt ==
                 m_opts.m_bodyIdRange.end())  // out of m_bodyIdRange, no more id's which could be parsed in
            )
        {
        ...
        LOGSCLEVEL2(m_pSimulationLog,
                    "---> Group range: [" << RigidBodyId::getBodyIdString(m_startIdGroup) << ","
                                          << RigidBodyId::getBodyIdString(endIdGroup)
                                          << "]"
                                          << std::endl;)
        ...
        if (m_parseSelectiveBodyIds && hasSelectiveFlag)
        {
            ...
            if (m_startRangeIdIt == m_bodyIdRangePtr->end() || *m_startRangeIdIt > endIdGroup)
            {  // no ids in the range
                *currGroupIdToNBodies += instances;
        ...
        *currGroupIdToNBodies += instances;
        ...
        if (m_eBodiesState == RigidBodyType::BodyMode::SIMULATED)
        {
            // LOGSCLEVEL1(m_pSimulationLog,"---> Copy Simulated RigidBody References to DynamicSystem
            // ..."<<std::endl;);
        ...
        if (m_pVisMod && visualizationNode)
        {
            if (m_pInitStatesMod)
            {
                m_pVisMod->parse(visualizationNode,
                                 &m_bodiesGroup,
                                 m_pInitStatesMod->getStatesGroup(),
                                 m_startIdGroup,
                                 m_eBodiesState);
            }
            else
    ...
    void parseDynamicState(XMLNodeType dynProp)
    {
        ...
        else if (type == "animated")
        {
            ...
            GRSF_ERRORMSG("---> The attribute 'type' '" << type
                                                        << "' of 'DynamicState' has no implementation in the parser");
        }
        else
        {
            GRSF_ERRORMSG("---> The attribute 'type' '" << type
    ...
    void parseDynamicPropertiesSimulated(XMLNodeType dynProp)
    {
        ...
        if (Options::m_allocateSimBodies)
        {
            ...
            {
                GRSF_ERRORMSG("---> The attribute 'distribute' '" << s
                                                                  << "' of 'Mass' has no implementation in the parser");
            }
    ...
    void parseMaterial(XMLNodeType n)
    {
        ...
        {
            GRSF_ERRORMSG("---> The attribute 'distribute' '" << s
                                                              << "' of 'Material' has no implementation in the parser");
        }
    }
