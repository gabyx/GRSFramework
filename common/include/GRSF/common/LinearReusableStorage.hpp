...
class LinearReusableStorage
{
    ...
    LinearReusableStorage() : m_insertIdx(0)
    {
    }

    explicit LinearReusableStorage(const AllocatorType& alloc) : m_allocator(alloc), m_insertIdx(0)
    {
    }
    ...
    class iterator
    {
        ...
        template <typename Other>
        iterator& operator=(const iterator<Other>& rhs)
        {
            m_it = rhs.m_it;
        };
    ...
    */
    ...
    template <template <typename> class Dist = std::uniform_int_distribution,
