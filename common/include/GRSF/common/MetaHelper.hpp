...
struct toRowColSym_impl
{
    ...
    using res = typename std::conditional<(L >= N),
                                          ...
                                          toRowColSym_impl<T, N, R, C + L, 0>>::type;
...
/// Get the row (::i) and column (::j) of the linear index \p L (meta::size_t) of the upper part of a symetric matrix of
/// size \p N  (meta::size_t).
...
template <typename T, typename List>
using remove =
    meta::filter<List, meta::compose<meta::quote<meta::not_>, meta::bind_front<meta::quote<std::is_same>, T>>>;
...
struct find_indices
{
    ...
    using invoke = find_ind_state<meta::if_<std::is_same<Type, T>,
