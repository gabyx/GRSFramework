...
public:
    ...
    void registerCallback(const std::initializer_list<int>& signals, const FuncType& callBack, const std::string& name)
    {
        ...
        for (auto signum : signals)
        {
            ...
            {
                GRSF_ERRORMSG("You cannot register signal type: "
                              << signum
            ...
            {
                GRSF_ERRORMSG("This signal: " << signum << " with callback name" << c.getName()
    ...
    void unregisterCallback(int signum, const std::string& name)
    {
        ...
        {
            GRSF_ERRORMSG("No callbacks for signal: " << signum
                                                      << " because not added to SignalHandler at instantiation!")
        }
    }
    ...
    void installDefaultSignal(int signum)
    {
        ...
        if (it == m_signalHandlers.end())
        {
            GRSF_ERRORMSG("You cannot install default handler for signal: " << signum
                                                                            << "because not registered in CTOR")
        }
    ...
    void installGlobalDispatcher(int signum)
    {
        ...
        if (it == m_signalHandlers.end())
        {
            GRSF_ERRORMSG("You cannot install global dispatcher for signal: " << signum
                                                                              << "because not registered in CTOR")
        }
    ...
    void handlePendingSignals()
    {
        ...
        m_handlingStackIdx = (m_handlingStackIdx == 1)
                                 ? 0
                                 : 1;  // atomic write (sig catcher could jump in here in between ... does not matter)
        ...
        for (auto signum : *m_handlingStack)
        {
            ...
            if (it != m_signalHandlers.end())
            {
                if (it->second.size() == 0)
                {
                    WARNINGMSG(
                        ...
                        "---> Caught signal: "
                            << signum
                            << " which was not handled by the application, because no call back for this signal)!");
                }
                else
                {
            ...
            else
            {
                WARNINGMSG(false,
                           "---> Caught signal: "
                               << signum
                               << " which was not handled by the application because signal not registered!");
            }
        }
...
private:
     ...
     * We have two stacks, one is the current stack \p m_currentStack which gets updated when ever a signal appears we
     * listen to
     * and the second stack \p m_handlingStack is the stack which gets handled and is emptied when handling completes.
     * the atomic \p m_handlingStackIdx makes sure no race condition happens. (e.g. when writing and just a async signal
     * happens)
     */
