...
inline typename std::enable_if<std::is_same<TypeConverter, StdTypeConverter>::value, bool>::type convert(
...
template <typename TypeConverter, typename T, SFINAE_ENABLE_IF(!isVectorType<T>::value)>
...
inline bool stringToTypeDispatch(T& v, const std::string& s)
{
    ...
    auto func = [&](unsigned int i, const typename T::Scalar& n) { v(i) = n; };
...
inline bool stringToTypeDispatch(std::set<T, Comp, Alloc>& m, const std::string& s)
{
    auto func = [&](unsigned int i, const T& n) { m.insert(n); };
...
inline bool stringToTypeDispatch(std::vector<T, Alloc>& v, const std::string& s)
{
    auto func = [&](unsigned int i, const T& n) { v.push_back(n); };
    return stringToTypeFunctorImpl<-1, T, decltype(func), TypeConverter>(func, s);
}

/** Custom dispatch for container types: std::pair<T> */
template <typename TypeConverter, typename T>
inline bool stringToTypeDispatch(std::pair<T, T>& v, const std::string& s)
{
    auto func = [&](unsigned int i, const T& n) { i == 0 ? v.first = n : v.second = n; };
...
inline bool stringToTypeDispatch(std::string& v, const std::string& s)
{
    v = s;  // just assign
    return true;
}
...
template <typename TypeConverter>
inline bool stringToTypeDispatch(boost::filesystem::path& v, const std::string& s)
{
    v = s;  // just assign
    return true;
}
