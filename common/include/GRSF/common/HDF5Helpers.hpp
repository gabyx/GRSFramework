...
{
    return detail::getNativeType_imp<typename std::remove_reference<typename std::remove_const<T>::type>::type>();
}
...
{
    return detail::mapNativeTypeToLE_imp<typename std::remove_reference<typename std::remove_const<T>::type>::type,
                                         N>();
}
...
inline H5::DataSet saveArithmeticArray(const TFileGroup& fg, Scalar* data, H5::DataSpace& dataSpace, std::string name)
{
...
void saveMatrixOrArray_artihmScalar(const TFileGroup& fg, const T& m, std::string name)
{
    ...
    /* Scalar = arithmetic */
    hsize_t dims[2] = {static_cast<hsize_t>(m.rows()), static_cast<hsize_t>(m.cols())};
...
template <typename TFileGroup, typename Derived, SFINAE_ENABLE_IF(std::is_arithmetic<typename Derived::Scalar>::value)>
...
/** Tensor saving, we accept here only the Tensor sub class, not TensorRef because TensorRef of Tensor with fixed size
* vector type as Scalar
*   has some non fixed bug (since TensorRef<Tensor<Vector3>>> is not yet fully supported, Tensor<Vector3>> however
* works.
...
template <typename TFileGroup,
          ...
          int Indices,
          int Options,
          SFINAE_ENABLE_IF(!std::is_arithmetic<TScalar>::value && TScalar::IsVectorAtCompileTime)>
void saveData(const TFileGroup& fg, const MyMatrix::TensorDyn<TScalar, Indices, Options>& m, std::string name)
{
...
template <typename TFileGroup,
          typename T,
          SFINAE_ENABLE_IF((std::is_same<T, AABB3d>::value || std::is_same<T, AABB2d>::value))>
