...
public:
    ...
    Transform3D(unsigned int id, const Quaternion& rot, const Vector3& trans, Vector3 value = Vector3(0, 0, 0))
        : LogicNode(id)
    {
    ...
    {
        GET_OSOCKET_REF_VALUE(Result) =
            GET_ISOCKET_REF_VALUE(Rotation) * GET_ISOCKET_REF_VALUE(Value) + GET_ISOCKET_REF_VALUE(Translation);
    }
};
};
