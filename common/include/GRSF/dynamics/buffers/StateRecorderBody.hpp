...
#ifndef GRSF_dynamics_buffers_StateRecorderBody_hpp
#define GRSF_dynamics_buffers_StateRecorderBody_hpp
...
StateRecorderBody::StateRecorderBody(bool logWriteAccess, unsigned int id)
{
    ...
    LOG(m_pSimulationLog,
        "---> StateRecorderBody:: File Descriptor Info: " << std::endl
                                                          << "\t\t"
                                                          << s.str()
                                                          << std::endl;)
}
...
bool StateRecorderBody::createSimFile(RigidBodyType* body, bool truncate)
{
    ...
    std::pair<typename FileMap::iterator, bool> res =
        m_BinarySimFiles.insert(typename FileMap::value_type(body->m_id, nullptr));
    if (!res.second)
    {
    ...
    else
    {
        ...
        if (!pBodyFile->openWrite(
                file, LayoutConfigType::LayoutType::NDOFqBody, LayoutConfigType::LayoutType::NDOFuBody, 1, truncate))
        {
            LOG(m_pSimulationLog,
                "---> StateRecorderBody:: Could not open SimFile: " << file.string() << " number of files open: "
                                                                    << m_BinarySimFiles.size()
                                                                    << std::endl;);
    ...
    if (m_logWriteAccess)
    {
        ...
        else
        {
            ...
            {
                LOG(m_pSimulationLog,
                    "---> StateRecorderBody:: Could not open LogSimFile: " << file.string() << ", error: "
                                                                           << std::strerror(errno));
                delete pLogFile;
...
void StateRecorderBody::getSimBodyFileName(typename DynamicsSystemType::RigidBodyType* body, std::stringstream& s)
{
    s.str("");
    s << "SimDataBody"
      << "-" << RigidBodyId::getGroupNr(body) << "-" << RigidBodyId::getBodyNr(body) << SIM_FILE_EXTENSION;
}

void StateRecorderBody::getSimBodyLogFileName(typename DynamicsSystemType::RigidBodyType* body, std::stringstream& s)
{
    s.str("");
    s << "SimDataBody"
      << "-" << RigidBodyId::getGroupNr(body) << "-" << RigidBodyId::getBodyNr(body) << "-Access"
      << SIM_FILE_ACCESS_LOG_EXTENSION;
}
...
void StateRecorderBody::write(PREC time, const TRigidBodyContainer& body_list)
{
    ...
    for (auto it = body_list.begin(); it != body_list.end(); it++)
    {
        ...
        LOGASSERTMSG(fileIt != m_BinarySimFiles.end(),
                     m_pSimulationLog,
                     "StateRecorderBody:: Did not found SimFile for Body Id:"
                         << RigidBodyId::getBodyIdString(*it)
                         << ". There is no SimFile corresponding to this body!"
                         << std::endl);
        ...
        if (m_logWriteAccess)
        {
            ...
            {
                LOG(m_pSimulationLog,
                    "StateRecorderBody:: Did not found LogSimFile for Body Id:"
                        << RigidBodyId::getBodyIdString(*it)
                        << ". There is no SimFile corresponding to this body!"
                        << std::endl);
            }
...
bool StateRecorderBody::closeSimFile(RigidBodyType* body)
{
    ...
    {
        LOG(m_pSimulationLog,
            "---> StateRecorderBody:: No Sim file to remove for Body Id: " << body->m_id << std::endl);
        return false;
    }
    ...
    if (m_logWriteAccess)
    {
        ...
        {
            LOG(m_pSimulationLog,
                "---> StateRecorderBody:: No LogSimFile to remove for Body Id: " << body->m_id << std::endl);
            return false;
        }
    }
...
{
    LOG(m_pSimulationLog,
        "---> StateRecorderBody:: Add body with id: " << RigidBodyId::getBodyIdString(body) << std::endl;);
    bool res = createSimFile(body, false);  // no truncate!
    LOGASSERTMSG(res,
                 m_pSimulationLog,
                 "---> StateRecorderBody:: Add body with id: " << RigidBodyId::getBodyIdString(body) << "failed!"
                                                               << ", StateRecorderBody has "
                                                               << m_BinarySimFiles.size() + m_LogSimFiles.size()
                                                               << " files open currently!");
}

void StateRecorderBody::removeBody(RigidBodyType* body)
{
    LOG(m_pSimulationLog,
        "---> StateRecorderBody:: Remove body with id: " << RigidBodyId::getBodyIdString(body) << std::endl;);
    bool res = closeSimFile(body);
    LOGASSERTMSG(res,
                 m_pSimulationLog,
                 "---> StateRecorderBody:: Remove body with id: " << RigidBodyId::getBodyIdString(body) << "failed!"
                                                                  << ", StateRecorderBody has "
                                                                  << m_BinarySimFiles.size() + m_LogSimFiles.size()
                                                                  << " files open currently!");
}
