...
* It provides a ring buffer with several states and three indices. The first index gives the state for the visualization
* thread, the second index
...
template <typename RigidBodyIterator>
StateRingPoolVisBackFront::StateRingPoolVisBackFront(RigidBodyIterator itBegin, RigidBodyIterator itEnd) : StatePool(3)
{
    ...
    m_logfile.clear();
    m_logfile
        << "This is the State Ring Pool log file: each line describes the actual mode in which the state pool is\n";
}
...
void StateRingPoolVisBackFront::resetStateRingPool(const RigidBodyStateContainerType& state_init)
{
    ...
    m_logfile << "vis: \t" << (unsigned int)m_idx[0] << "\t back: \t" << (unsigned int)m_idx[1] << "\t front: \t"
              << (unsigned int)m_idx[2] << std::endl;
#endif
}
...
StateRingPoolVisBackFront::StateType* StateRingPoolVisBackFront::getSimBuffer()
{
...
StateRingPoolVisBackFront::StateType* StateRingPoolVisBackFront::advanceSimBuffer(bool& out_changed)
{
...
const StateRingPoolVisBackFront::StateType* StateRingPoolVisBackFront::updateVisBuffer(bool& out_changed)
{
    boost::mutex::scoped_lock l(m_change_pointer_mutex);
...
const StateRingPoolVisBackFront::StateType* StateRingPoolVisBackFront::updateVisBuffer()
{
    bool out_changed;
