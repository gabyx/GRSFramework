...
template <typename Derived, unsigned int M>
...
template <typename Derived, unsigned int N>
...
#define DEFINE_MATRIX_SPECIALTYPES                                        \
    template <typename Derived>                                           \
    using MatrixBase = typename MyMatrix::MatrixBase<Derived>;            \
    template <typename Derived>                                           \
    using ArrayBase = typename MyMatrix::ArrayBase<Derived>;              \
                                                                          \
    template <typename Derived>                                           \
    using VectorBDyn = typename MyMatrix::VectorBDyn<Derived>;            \
    template <typename Derived, int M>                                    \
    using VectorBStat = typename MyMatrix::VectorBStat<Derived, M>;       \
                                                                          \
    template <typename Derived>                                           \
    using MatrixBDynDyn = typename MyMatrix::MatrixBDynDyn<Derived>;      \
    template <typename Derived, int N>                                    \
    using MatrixBDynStat = typename MyMatrix::MatrixBDynStat<Derived, N>; \
    template <typename Derived, int M>                                    \
    using MatrixBStatDyn = typename MyMatrix::MatrixBStatDyn<Derived, M>; \
                                                                          \
    template <typename EigenType>                                         \
    using MatrixRef = typename MyMatrix::MatrixRef<EigenType>;            \
    template <typename EigenType>                                         \
    using MatrixMap = typename MyMatrix::MatrixMap<EigenType>;            \
                                                                          \
    template <typename T>                                                 \
...
#define DEFINE_MATRIX_TYPES_OF(_PREC_)                                          \
    ...
    using MatrixDynDynRow = typename MyMatrix::MatrixDynDynRow<_PREC_>;         \
                                                                                \
    template <int M>                                                            \
    using MatrixStatDyn = typename MyMatrix::MatrixStatDyn<_PREC_, M>;          \
    template <int N>                                                            \
    using MatrixDynStat = typename MyMatrix::MatrixDynStat<_PREC_, N>;          \
    template <int M, int N>                                                     \
    using MatrixStatStat = typename MyMatrix::MatrixStatStat<_PREC_, M, N>;     \
    template <int M>                                                            \
    using VectorStat = typename MyMatrix::VectorStat<_PREC_, M>;                \
                                                                                \
    ...
    using MatrixSparseTriplet = typename MyMatrix::MatrixSparseTriplet<_PREC_>; \
                                                                                \
    template <int M>                                                            \
    using ArrayStatDyn = typename MyMatrix::ArrayStatDyn<_PREC_, M>;            \
    template <int N>                                                            \
    using ArrayDynStat = typename MyMatrix::ArrayDynStat<_PREC_, N>;            \
    template <int M, unsigned int N>                                            \
    using ArrayStatStat = typename MyMatrix::ArrayStatStat<_PREC_, M, N>;       \
    template <int M>                                                            \
    using ArrayStat = typename MyMatrix::ArrayStat<_PREC_, M>;                  \
