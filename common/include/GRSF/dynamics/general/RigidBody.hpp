...
class RigidBodyBase
{
    ...
    GlobalGeomIdType m_globalGeomId;  ///< The Id for the global geometry, if this is 0 then the geometry belongs to the
    ...
    typedef boost::variant<SphereGeomPtrType, HalfspaceGeomPtrType, BoxGeomPtrType, MeshPtrType, CapsuleGeomPtrType>
        GeometryType;
    ...
    VectorUBody m_MassMatrix_diag;  ///< The mass matrix which is diagonal, \f$ \textrm{diag}(m,m,m,
    ...
    */
    ...
    unsigned char
        m_flags;  ///< Different Flags which can be used during the timestep process, introduced because of MPI

    BodySolverDataType* m_pSolverData;  /// Simulated bodies have a solverData. For all others, animated and not
