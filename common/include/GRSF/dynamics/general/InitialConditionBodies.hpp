                               ...
                               unsigned int seed)
{
    ...
    if (jitter)
    {
        random_vec(0) = uni(gen);
        random_vec(1) = uni(gen);
        random_vec(2) = uni(gen);
    }
                             ...
                             Vector3 dirX = Vector3(1, 0, 0))
{
    ...
    for (auto& b : bodyDataCont)
    {
        ...
        stateIt->setDisplacement(
            A_IK * Vector3(index_x * d - 0.5 * (gDim_x - 1) * d, index_y * d - 0.5 * (gDim_y - 1) * d, index_z * d) +
                vec_trans,
            Quaternion(1, 0, 0, 0));
                                         ...
                                         short which  = 2)
{
    ...
    simFile.close();
    GRSF_ERRORMSG("setupInitialConditionBodiesFromFile:: failed: "
                  << "path: "
                  << file_path
                  << " error:"
                  << simFile.getErrorString());
    return false;
}
...
inline void applyBodyStatesTo(const RigidBodyStatesContainer& states, RigidBodyContainer& bodies)
{
    for (auto bodyIt = bodies.begin(); bodyIt != bodies.end(); ++bodyIt)
    {
        ...
        if (resIt == states.end())
        {
            GRSF_ERRORMSG(" There is no initial state for sim body id: " << RigidBodyId::getBodyIdString(*bodyIt)
                                                                         << std::endl);
        }
