...
*   such that ((N+1)*N)/2 = E, which means the upper triangular part of the node to edge type correlation table is fully
* filled
*
...
struct GraphTraitsSymmetric
{
    ...
    static_assert((nNodes * (nNodes + 1)) / 2 == nEdgesOrig,
                  "Number of edges e not equal to nodes n, ((n+1)*n)/2 = e!");
    ...
    using outEdgesForNode = metaAdd::remove<
        void,
        meta::transform<
            meta::as_list<meta::make_index_sequence<nNodes>>,
            meta::compose<meta::bind_front<meta::quote<meta::at>, EdgeDataTypesOrig>,
                          meta::bind_back<meta::bind_front<meta::quote<toLinearIdx>, getNodeDataIdx<NodeData>>,
                                          meta::size_t<nNodes>>>>>;

    /** build map , node data to all input edge data types (without void) */
    template <typename NodeData>
    using inEdgesForNode = metaAdd::remove<
        void,
        meta::transform<
            meta::as_list<meta::make_index_sequence<nNodes>>,
            meta::compose<meta::bind_front<meta::quote<meta::at>, EdgeDataTypesOrig>,
    ...
    using makeTupleContainer = meta::apply<meta::quote<std::tuple>, meta::transform<List, meta::compose<Container, F>>>;
...
#define SWITCH_CASE(z, N, data)                                \
    case N:                                                    \
        v(*static_cast<meta::at<Types, meta::size_t<N>>*>(p)); \
        break;
    ...
    {                                                                                                       \
        ...
        {                                                                                                   \
            ...
            {                                                                                               \
                ...
                default:                                                                                    \
                    GRSF_ERRORMSG("Tried to cast type: " << demangle::type<T>() << " to wrong id: " << id); \
                    break;                                                                                  \
            }                                                                                               \
        }                                                                                                   \
    };
...
public:
    ...
    {  // Universal reference (binds to temporaries and lvalues)
...
public:
    ...
    using OutEdgeStoragesTypes = typename Traits::template makeTupleContainer<
        OutEdgeDataTypes,
    ...
    using InEdgeStoragesTypes = typename Traits::template makeTupleContainer<
        InEdgeDataTypes,
        meta::compose<meta::quote<metaAdd::addPointer>, meta::quote<Traits::template toEdge>>>;
...
private:
    ...
    /** EdgeStorageVisitor: Visit all edges, moves over all storages and then loops through all edges calling the
     * visitor */
    template <typename Visitor>
...
public:
    ...
    using NodeStorageTuple = typename Traits::
        template makeTupleContainer<NodeDataTypes, meta::quote<Traits::template toNode>, meta::quote<NodeStorageType>>;
