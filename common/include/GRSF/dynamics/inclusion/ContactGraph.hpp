...
using ContactGraphTraits =
    Graph::GraphTraitsSymmetric<meta::list<ContactGraphNodeDataUCF>, meta::list<ContactGraphEdgeData>>;
...
class ContactGraph : public Graph::GeneralGraph<ContactGraphTraits>
{
    ...
    void addNode(CollisionData* pCollData)
    {
        ...
        GRSF_ASSERTMSG(pCollData->m_pBody[0] != nullptr && pCollData->m_pBody[1] != nullptr,
                       " Bodys are null pointers?");
        // cout << "add node : "<<m_nodeCounter<< " body id:" << RigidBodyId::getBodyIdString(pCollData->m_pBody[0]) <<"
        // and "<< RigidBodyId::getBodyIdString(pCollData->m_pBody[1]) <<endl;
        // std::cout <<"b1:" << pCollData->m_pBody[0]->m_q_KI<< std::endl << pCollData->m_pBody[0]->m_r_S<< std::endl;;
        // std::cout <<"b2:" << pCollData->m_pBody[1]->m_q_KI << std::endl << pCollData->m_pBody[1]->m_r_S<<
        // std::endl;;;
    ...
    struct NodeInit
    {
        ...
        void initNodeSimBody(TNode* pNode, TNodeData& nodeData, RigidBodyType* pBody)
        {
    ...
    */
    ...
    CType cFunc = [](std::pair<PREC, NodeType*> const& a, std::pair<PREC, NodeType*> const& b) -> bool {
