                                                                      ...
                                                                      DynamicsState*       state_e)
{
    ...
    LOGSLLEVEL3_CONTACT(m_pSolverLog,
                        "h= " << m_h.transpose().format(MyMatrixIOFormat::Matlab) << "';" << std::endl
                              << "Minv_diag= "
                              << "diag("
                              << m_Minv_diag.transpose().format(MyMatrixIOFormat::Matlab)
                              << ");"
                              << std::endl
                              << "Minv_h_dt= "
                              << m_Minv_h_dt.transpose().format(MyMatrixIOFormat::Matlab)
                              << "';"
                              << std::endl;);
    ...
    if (m_nContacts > 0)
    {
        ...
        for (unsigned int contactIdx = 0; contactIdx < m_nContacts; contactIdx++)
        {
            LOGSLLEVEL3_CONTACT(m_pSolverLog,
                                << "e_x= "
                                << collSet[contactIdx].m_cFrame.m_cFrame.m_e_x.transpose().format(
                                       MyMatrixIOFormat::Matlab)
                                << "';"
                                << std::endl
                                << "e_y= "
                                << collSet[contactIdx].m_cFrame.m_e_y.transpose().format(MyMatrixIOFormat::Matlab)
                                << "';"
                                << std::endl
                                << "e_z= "
                                << collSet[contactIdx].m_cFrame.m_e_z.transpose().format(MyMatrixIOFormat::Matlab)
                                << "';"
                                << std::endl
                                << collSet[contactIdx].m_cFrame.m_cFrame.m_e_x.dot(collSet[contactIdx].m_cFrame.m_e_y)
                                << collSet[contactIdx].m_cFrame.m_e_y.dot(collSet[contactIdx].m_cFrame.m_e_z)
                                << std::endl
                                << "r_s1c1="
                                << collSet[contactIdx].m_r_SC[0].transpose().format(MyMatrixIOFormat::Matlab)
                                << "';"
                                << std::endl;);

            GRSF_ASSERTMSG(
                std::abs(collSet[contactIdx].m_cFrame.m_cFrame.m_e_x.dot(collSet[contactIdx].m_cFrame.m_e_y)) < 1e-3 &&
                    std::abs(collSet[contactIdx].m_cFrame.m_e_y.dot(collSet[contactIdx].m_cFrame.m_e_z)) < 1e-3,
            ...
            if (collSet[contactIdx].m_pBody[0]->m_eMode == RigidBodyType::BodyMode::SIMULATED)
            {
                ...
                m_WT_Minv_h_dt(m_nDofFriction * contactIdx) +=
                    w_T_part.dot(m_Minv_h_dt.template segment<NDOFuBody>(id1 * NDOFuBody));
                ...
                m_WT_Minv_h_dt(m_nDofFriction * contactIdx + 1) +=
                    w_T_part.dot(m_Minv_h_dt.template segment<NDOFuBody>(id1 * NDOFuBody));
            }
            ...
            if (collSet[contactIdx].m_pBody[1]->m_eMode == RigidBodyType::BodyMode::SIMULATED)
            {
                ...
                m_WT_Minv_h_dt(m_nDofFriction * contactIdx) +=
                    w_T_part.dot(m_Minv_h_dt.template segment<NDOFuBody>(id2 * NDOFuBody));

                // T2 direction =================================================
                w_T_part.template head<3>() = collSet[contactIdx].m_cFrame.m_e_y;
                w_T_part.template tail<3>() = I_Jacobi_2 * collSet[contactIdx].m_cFrame.m_e_y;
                // pop into W_T
                m_W_T.template block<NDOFuBody, 1>(id2 * NDOFuBody, m_nDofFriction * contactIdx + 1) =
                    w_T_part;  // I frame
                // compute part to m_WT_uS and to m_WT_Minv_h_dt
            ...
            ContactParameter<LayoutConfigType>& params = m_ContactParameterMap.getContactParams(
                collSet[contactIdx].m_pBody[0]->m_eMaterial, collSet[contactIdx].m_pBody[1]->m_eMaterial);
        ...
        LOGSLLEVEL3_CONTACT(m_pSolverLog,
                            << "W_N= ..." << std::endl
                            << m_W_N.format(MyMatrixIOFormat::Matlab)
                            << ";"
                            << std::endl
                            << "W_T= ..."
                            << std::endl
                            << m_W_T.format(MyMatrixIOFormat::Matlab)
                            << ";"
                            << std::endl
                            << "mu= "
                            << "diag("
                            << m_mu.transpose().format(MyMatrixIOFormat::Matlab)
                            << ");"
                            << std::endl
                            << "I_epsilon_N= "
                            << "diag("
                            << m_I_epsilon_N.transpose().format(MyMatrixIOFormat::Matlab)
                            << ");"
                            << std::endl
                            << "I_epsilon_T= "
                            << "diag("
                            << m_I_epsilon_T.transpose().format(MyMatrixIOFormat::Matlab)
                            << ");"
                            << std::endl
                            << "WT_uS= "
                            << m_WN_uS.transpose().format(MyMatrixIOFormat::Matlab)
                            << std::endl
                            << "WN_uS= "
                            << m_WT_uS.transpose().format(MyMatrixIOFormat::Matlab)
                            << "';"
                            << std::endl
                            << "WN_Minv_h_dt= "
                            << m_WN_Minv_h_dt.transpose().format(MyMatrixIOFormat::Matlab)
                            << "';"
                            << std::endl
                            << "WT_Minv_h_dt= "
                            << m_WT_Minv_h_dt.transpose().format(MyMatrixIOFormat::Matlab)
                            << "';"
                            << std::endl;);
        ...
        LOGSLLEVEL3_CONTACT(m_pSolverLog,
                            "G= ..." << std::endl
                                     << G.format(MyMatrixIOFormat::Matlab)
                                     << ";"
                                     << std::endl;);
        ...
        LOGSLLEVEL3_CONTACT(m_pSolverLog,
                            "G_NN= ..." << std::endl
                                        << m_G_NN.format(MyMatrixIOFormat::Matlab)
                                        << ";"
                                        << std::endl
                                        << "G_NT= ..."
                                        << std::endl
                                        << m_G_NT.format(MyMatrixIOFormat::Matlab)
                                        << ";"
                                        << std::endl
                                        << "G_TT= ..."
                                        << std::endl
                                        << m_G_TT.format(MyMatrixIOFormat::Matlab)
                                        << ";"
                                        << std::endl;);
        ...
        LOGSLLEVEL3_CONTACT(m_pSolverLog,
                            "c_N= " << m_c_N.transpose().format(MyMatrixIOFormat::Matlab) << "';" << std::endl
                                    << "c_T= "
                                    << m_c_T.transpose().format(MyMatrixIOFormat::Matlab)
                                    << "';"
                                    << std::endl;);

        LOGSLLEVEL3_CONTACT(m_pSolverLog,
                            "P_N_back= " << P_N_back.transpose().format(MyMatrixIOFormat::Matlab) << "';" << std::endl
                                         << "P_T_back= "
                                         << P_T_back.transpose().format(MyMatrixIOFormat::Matlab)
                                         << "';"
                                         << std::endl;);
        ...
        {
            state_e->m_SimBodyStates[i].m_u =
                state_s->m_SimBodyStates[i].m_u + m_delta_u_E.template segment<NDOFuBody>(i * NDOFuBody);
        }
    }
    else
    {
        ...
        {
            state_e->m_SimBodyStates[i].m_u =
                state_s->m_SimBodyStates[i].m_u + m_Minv_h_dt.template segment<NDOFuBody>(i * NDOFuBody);
        }
    }
}
...
void InclusionSolverNT<TInclusionSolverConfig>::setupRMatrix(PREC alpha)
{
    ...
    {
        r_T_i = m_settings.m_alphaSORProx /
                (m_G_TT.diagonal().template segment<NDOFFriction>(m_nDofFriction * i)).maxCoeff();
    ...
    LOGSLLEVEL3_CONTACT(m_pSolverLog,
                        << "R_N= "
                        << "diag("
                        << m_R_N.transpose().format(MyMatrixIOFormat::Matlab)
                        << "');"
                        << std::endl
                        << "R_T= "
                        << "diag("
                        << m_R_T.transpose().format(MyMatrixIOFormat::Matlab)
                        << "');"
                        << std::endl);
}
...
void InclusionSolverNT<TInclusionSolverConfig>::doJorProx()
{
    ...
    while (m_globalIterationCounter < m_settings.m_MaxIter)
    {
        ...
        m_bConverged = Numerics::cancelCriteriaValue(
            P_N_back, P_N_front, P_T_back, P_T_front, m_settings.m_AbsTol, m_settings.m_RelTol);

        LOGSLLEVEL3_CONTACT(m_pSolverLog,
                            "P_N_front= " << P_N_front.transpose().format(MyMatrixIOFormat::Matlab) << "';" << std::endl
                                          << "P_T_front= "
                                          << P_T_front.transpose().format(MyMatrixIOFormat::Matlab)
                                          << "';"
                                          << std::endl;)
...
void InclusionSolverNT<TInclusionSolverConfig>::doSorProx()
{
    ...
    while (m_globalIterationCounter < m_settings.m_MaxIter)
    {
        ...
        for (unsigned int i = 0; i < m_nContacts; i++)
        {
            ...
            P_N_front(i) =
                P_N_front(i) - m_R_N(i) * (m_G_NN.row(i).dot(P_N_front) + m_G_NT.row(i).dot(P_T_front) + m_c_N(i));
            Prox::ProxFunction<ConvexSets::RPlus>::doProxSingle(P_N_front(i));

            // Solve Prox on Tangential direction
            P_T_front.template segment<NDOFFriction>(NDOFFriction * i) =
                P_T_front.template segment<NDOFFriction>(NDOFFriction * i) -
                (m_R_T.template segment<NDOFFriction>(NDOFFriction * i)).asDiagonal() *
            ...
            Prox::ProxFunction<ConvexSets::Disk>::doProxSingle(
                m_mu(i) * P_N_front(i), P_T_front.template segment<NDOFFriction>(NDOFFriction * i));
        }
...
std::string InclusionSolverNT<TInclusionSolverConfig>::getIterationStats()
{
    ...
    s << -1 << "\t"                                                                                     // NO GPU
      << m_nContacts << "\t" << m_globalIterationCounter << "\t" << m_bConverged << "\t" << -1 << "\t"  // No is Finite
      << -1 << "\t"                                                                                     // No time prox
      << -1 << "\t"                                                            // No proxIterationtime
      << m_pDynSys->m_currentTotEnergy << "\t" << m_G_conditionNumber << "\t"  // No m_G_conditionNumber
