...
struct InclusionSolverSettings
{
    ...
    *  SOR_NORMAL_TANGENTIAL (for all contacts  first normal , then for all contacts tangential (iteratively solve two
    ...
    */
    enum Method
    {
        SOR_CONTACT,
        SOR_FULL,
        SOR_NORMAL_TANGENTIAL,
        JOR
    } m_eMethod;

    /** Additional setting for SOR_NORMAL_TANGENTIAL */
    unsigned int m_normalTangentialUpdateRatio =
        4;  ///< Update 4 times normal direction, then once tangential direction
    /** =============================================*/
