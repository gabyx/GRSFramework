...
/**
* @brief Spezialisation for a single Prox onto  \f$ C_1 = \mathcal{R}_{+} \f$   and a scaled unit disk \f$ C_2 =\{ x |
* |x| < 1 \} \f$ . The scale factor \f$ r\f$  is the proxed value on to \f$ C_1 \f$ .
*/
template <>
struct ProxFunction<ConvexSets::RPlusAndDisk>
{
    ...
    * @param y Input/output vector, where the first value in y has been proxed onto  \f$ C_1 \f$  and the second 2
    * values onto the unit disk which is scaled by the first proxed value times scale_factor.
    */
    template <typename PREC, typename Derived>
    ...
    */
                                                ...
                                                const Eigen::MatrixBase<DerivedOther2>& y)
    {
        ...
        for (int i = 0; i < scale_factor.rows(); i++)
        {
            ...
            {
                y_ref.segment<2>(3 * i + 1) =
                    x.segment<2>(3 * i + 1) * (scale_factor(i, 0) * y_ref(3 * i) / std::sqrt(absvalue));
            }
        }
    }
};
...
{
    template <typename PREC, typename Derived>
    static INLINE_PROX_KEYWORD void doProxSingle(const PREC& slopeFactor, Eigen::MatrixBase<Derived>& y)
    {
        GRSF_ASSERTMSG(y.rows() % 3 == 0, "wrong size");
        ...
        if (normT - slopeFactor * y(0) <= 0.0)
        {  // In Friction cone, do nothing!
            return;
        }
        else if (testFricCone <= 0.0)
        {  // In polar cone to friction cone, set to zero!
            y.setZero();
            return;
        }
                                              ...
                                              double                            RelTol)
{
    ...
    // LOGSLLEVEL3_CONTACT(m_pSolverLog, "Cancel Criterion :" << RelNormP << " < " << NormP * m_settings.m_RelTol +
    // m_settings.m_AbsTol << std::endl;);
                                                  ...
                                                  PREC&                                   residual)
{
    ...
    residual = sqrt(abs((PREC)(diff.transpose() * NormMatrix_diag.asDiagonal() * diff)));
    if (residual - (sqrt(abs((PREC)(P_old.transpose() * NormMatrix_diag.asDiagonal() * P_old))) * RelTol + AbsTol) >
        0.0)
    {
        return false;
    }
                                                  ...
                                                  PREC                                    RelTol)
{
    ...
    EIGEN_STATIC_ASSERT_VECTOR_ONLY(DerivedOther);
    EIGEN_STATIC_ASSERT_VECTOR_ONLY(DerivedOther2);
    GRSF_ASSERTMSG((P_old.rows() == P_new.rows()) && (NormMatrix1_diag.rows() == NormMatrix2.rows()) &&
                       (u_old.rows() == NormMatrix2.cols()),
                   "Vectors are not equal lenght!");
    ...
    using std::abs;
    using std::sqrt;
    if (sqrt(abs((PREC)(diff.transpose() * NormMatrix2 * diff))) >
        sqrt(abs((PREC)(u_old.transpose() * NormMatrix1_diag.asDiagonal() * u_old))) * RelTol + AbsTol)
    {
        return false;
    }
    return true;
