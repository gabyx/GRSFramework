...
public:
    ...
    void reset();
    void resetForNextIter();  // Is called each iteration in the timestepper, so that the InclusionSolver is able to
    ...
    : m_SimBodies(pCollisionSolver->m_SimBodies), m_Bodies(pCollisionSolver->m_Bodies)
{
    ...
    m_pCollisionSolver->m_ContactDelegateList.addContactDelegate(
        ContactDelegateList<TLayoutConfig>::ContactDelegate::from_method<TContactGraph, &TContactGraph::addNode>(
            &m_ContactGraph));
...
template <typename TLayoutConfig, typename TDynamicsSystem, typename TCollisionSolver, typename TContactGraph>
void InclusionSolverNTContactOrdered<TLayoutConfig, TDynamicsSystem, TCollisionSolver, TContactGraph>::initializeLog(
    Ogre::Log* pSolverLog)
{
...
template <typename TLayoutConfig, typename TDynamicsSystem, typename TCollisionSolver, typename TContactGraph>
unsigned int
InclusionSolverNTContactOrdered<TLayoutConfig, TDynamicsSystem, TCollisionSolver, TContactGraph>::getNObjects()
{
    return m_nSimBodies;
}
...
void InclusionSolverNTContactOrdered<TLayoutConfig, TDynamicsSystem, TCollisionSolver, TContactGraph>::reset()
{
    ...
    GRSF_ASSERTMSG(m_SimBodies.size() * NDOFuObj == m_nDofu,
                   "InclusionSolverNTContactOrdered:: Error in Dimension of System!");
    GRSF_ASSERTMSG(m_SimBodies.size() * NDOFqObj == m_nDofq,
                   "InclusionSolverNTContactOrdered:: Error in Dimension of System!");
...
template <typename TLayoutConfig, typename TDynamicsSystem, typename TCollisionSolver, typename TContactGraph>
void InclusionSolverNTContactOrdered<TLayoutConfig, TDynamicsSystem, TCollisionSolver, TContactGraph>::
    reservePercussionPoolSpace(unsigned int nExpectedContacts)
{
    ...
    if (m_nContacts > 0)
    {
        ...
        for (unsigned int contactIdx = 0; contactIdx < m_nContacts; contactIdx++)
        {
            ...
            for (it = currentContactNode->m_edgeList.begin(); it != currentContactNode->m_edgeList.end(); it++)
            {
                ...
                // We skip the edges which ar pointing towards the current node, we add them anyway, and also the
                // symetric part!
                ...
                if (i == j)
                {  // We are on a self referencing edge! Thats good build diagonal of G and parts of c
                    W_i_bodyT_M_body = (*W_i_body).transpose() * edgesBody->m_MassMatrixInv_diag.asDiagonal();
                    m_G.block<(NDOFFriction + 1), (NDOFFriction + 1)>((NDOFFriction + 1) * i, (NDOFFriction + 1) * j)
                        .noalias() += W_i_bodyT_M_body * (*W_i_body);
                    m_c.segment<NDOFFriction + 1>((NDOFFriction + 1) * i).noalias() +=
                        W_i_bodyT_M_body * edgesBody->m_h_term * m_Settings.m_Timestep +
                        currentContactNode->m_nodeData.m_I_plus_eps.asDiagonal() *
                            ((*W_i_body).transpose() * state_s->m_SimBodyStates[bodyId].m_u);
                }
                else
                {
                    ...
                    m_G.block<(NDOFFriction + 1), (NDOFFriction + 1)>((NDOFFriction + 1) * i, (NDOFFriction + 1) * j)
                        .noalias() = G_part;
                    m_G.block<(NDOFFriction + 1), (NDOFFriction + 1)>((NDOFFriction + 1) * j, (NDOFFriction + 1) * i)
                        .noalias() = G_part;
                }
            }
            ...
            // add once xi to c
            m_c.segment<NDOFFriction + 1>((NDOFFriction + 1) * i).noalias() +=
                currentContactNode->m_nodeData.m_I_plus_eps.asDiagonal() * currentContactNode->m_nodeData.m_xi;
...
// TODO update ContactPercussions
        ...
        for (unsigned int i = 0; i < m_nSimBodies; i++)
        {
            ...
            TContactGraph::BodyToContactsListIterator itList =
                m_ContactGraph.m_BodyToContactsList.find(m_SimBodies[i].get());
            ...
            if (itList != m_ContactGraph.m_BodyToContactsList.end())
            {
                ...
                {
                    delta_u_E.noalias() += *(TContactGraph::getW_body((*it)->m_nodeData, m_SimBodies[i].get())) *
                                           P_front.segment<NDOFFriction + 1>((*it)->m_nodeNumber * (NDOFFriction + 1));
                }
            }

            state_e->m_SimBodyStates[i].m_u =
                state_s->m_SimBodyStates[i].m_u + m_SimBodies[i]->m_MassMatrixInv_diag.asDiagonal() * (delta_u_E);
        }
    }
    else
    {
        ...
        {
            state_e->m_SimBodyStates[i].m_u =
                state_s->m_SimBodyStates[i].m_u +
                m_SimBodies[i]->m_MassMatrixInv_diag.asDiagonal() * m_SimBodies[i]->m_h_term * m_Settings.m_Timestep;
            ;
        }
    }
}
...
template <typename TLayoutConfig, typename TDynamicsSystem, typename TCollisionSolver, typename TContactGraph>
void InclusionSolverNTContactOrdered<TLayoutConfig, TDynamicsSystem, TCollisionSolver, TContactGraph>::setupRMatrix(
    PREC alpha)
{
    PREC r_T_i;
    ...
    CLEARLOG;
    logstream << "R= "
              << "diag(" << m_R.transpose().format(MyIOFormat::Matlab) << "');" << endl;
    LOG(m_pSolverLog);
#endif
}
...
void InclusionSolverNTContactOrdered<TLayoutConfig, TDynamicsSystem, TCollisionSolver, TContactGraph>::doSorProx()
{
    ...
    while (m_iterationsNeeded < m_Settings.m_MaxIter)
    {
        ...
        for (unsigned int i = 0; i < m_nContacts; i++)
        {
            ...
            // This statement looks like aliasing but it does not! All Matrices*Matrices or MAtrices*Vector get
            // evaluated into temporary by default
            ...
            P_front.segment<NDOFFriction + 1>((NDOFFriction + 1) * i) =
                ...
                (m_R.segment<NDOFFriction + 1>((NDOFFriction + 1) * i)).asDiagonal() *
                    ((m_G.block((NDOFFriction + 1) * i, 0, NDOFFriction + 1, m_nLambdas) * P_front) +
                     m_c.segment<NDOFFriction + 1>((NDOFFriction + 1) * i));

            Prox::ProxFunction<ConvexSets::RPlusAndDisk>::doProxSingle(
                m_mu(i), P_front.segment<NDOFFriction + 1>((NDOFFriction + 1) * i));
            Numerics::cancelCriteriaValue(PContact_back,
                                          P_front.segment<NDOFFriction + 1>((NDOFFriction + 1) * i),
                                          m_Settings.m_AbsTol,
                                          m_Settings.m_RelTol,
                                          counterConverged);
...
template <typename TLayoutConfig, typename TDynamicsSystem, typename TCollisionSolver, typename TContactGraph>
void InclusionSolverNTContactOrdered<TLayoutConfig, TDynamicsSystem, TCollisionSolver, TContactGraph>::
    updatePercussionPool(const VectorDyn& P_old)
{
...
template <typename TLayoutConfig, typename TDynamicsSystem, typename TCollisionSolver, typename TContactGraph>
void InclusionSolverNTContactOrdered<TLayoutConfig, TDynamicsSystem, TCollisionSolver, TContactGraph>::
    readFromPercussionPool(unsigned int index, const CollisionData<TLayoutConfig>* pCollData, VectorDyn& P_old)
{
    static VectorPContact P_contact;
