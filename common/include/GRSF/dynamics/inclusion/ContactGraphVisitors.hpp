...
public:
    ...
    //
    //        // u_S + Minv *h * deltaT
    //        *pUBuffer = pBody->m_pSolverData->m_uBegin + pBody->m_MassMatrixInv_diag.asDiagonal() * pBody->m_h_term *
    //        m_settings.m_deltaT;
    //
    //        // Iterate over all nodes and add contribution
    //        auto nodeList = m_pGraph->m_simBodiesToContactsMap[pBody];
    //        for(auto it = nodeList.begin(); it!=nodeList.end(); ++it) {
    //            *pUBuffer += pBody->m_MassMatrixInv_diag.asDiagonal() *
    //            ContactGraphType::getW_bodyRef((*it)->m_nodeData,pBody) * (*it)->m_nodeData.m_LambdaFront;
    //        }
    //
    //    }
    ...
    void recalculateR(UCFNodeDataType& nodeData, ContactParameter& contactParameter)
    {
        ...
        {
            if (nodeData.m_pCollData->m_pBody[i]->m_eMode == RigidBodyType::BodyMode::SIMULATED)
            {
                nodeData.m_G_ii += nodeData.m_W_body[i].transpose() *
        ...
        else if (nodeData.m_contactParameter.m_contactModel == ContactModels::Enum::UCFDD)
        {
            ...
            {  // dinvT = gammaMax / (mu *lambdaN)
                dinv.tail<2>().setConstant(contactParameter.m_params[CMT::gamma_maxIdx] /
                                           (contactParameter.m_params[CMT::muIdx] * nodeData.m_LambdaBack(0)));
            }
            nodeData.m_G_ii.diagonal() += 1.0 / m_settings.m_deltaT * dinv;
        }
        ...
        // nodeData.m_R_i_inv_diag(0) = m_alpha / std::max(std::max(nodeData.m_G_ii(0,0),
        // nodeData.m_mu(0)*nodeData.m_G_ii(0,1)), nodeData.m_mu(0)*nodeData.m_G_ii(0,2));
        // Take only diagonal
        ...
        */
            ...
            nodeData.m_contactParameter.m_contactModel == ContactModels::Enum::UCFDD)
        {
            ...
            for (unsigned int i = 0; i < 2; i++)
            {
                if (nodeData.m_pCollData->m_pBody[i]->m_eMode == RigidBodyType::BodyMode::SIMULATED)
                {
                    nodeData.m_LambdaFront += nodeData.m_W_body[i].transpose() * nodeData.m_uBufferPtr[i]->m_front;
                }
            }
            ...
            if (nodeData.m_contactParameter.m_contactModel == ContactModels::Enum::UCFD)
            {
                ...
                nodeData.m_LambdaFront(0) +=
                    nodeData.m_LambdaBack(0) * nodeData.m_contactParameter.m_params[CMT::d_NIdx] / m_settings.m_deltaT;
                nodeData.m_LambdaFront(1) +=
                    nodeData.m_LambdaBack(1) * nodeData.m_contactParameter.m_params[CMT::d_TIdx] / m_settings.m_deltaT;
                nodeData.m_LambdaFront(2) +=
                    nodeData.m_LambdaBack(2) * nodeData.m_contactParameter.m_params[CMT::d_TIdx] / m_settings.m_deltaT;
            }
            else if (nodeData.m_contactParameter.m_contactModel == ContactModels::Enum::UCFDD)
            {
                using CMT = typename CONTACTMODELTYPE(ContactModels::Enum::UCFDD);
                ...
                {  // dinvT = gammaMax / (mu *lambdaN)
                    dinvT = nodeData.m_contactParameter.m_params[CMT::gamma_maxIdx] /
                            (nodeData.m_contactParameter.m_params[CMT::muIdx] * nodeData.m_LambdaBack(0));
                }
            ...
            if (m_settings.m_eSubMethodUCF == InclusionSolverSettingsType::UCF_DS)
            {
                ...
                nodeData.m_LambdaFront(0) +=
                    nodeData.m_contactParameter.m_params[CMT::muIdx] * nodeData.m_LambdaFront.template tail<2>().norm();
                // Multiply R_i_inverse
                nodeData.m_LambdaFront =
                    -(nodeData.m_R_i_inv_diag.asDiagonal() * nodeData.m_LambdaFront)
                         .eval();  // No alias due to diagonal!!! (if normal matrix multiplication there is aliasing!
                nodeData.m_LambdaFront += nodeData.m_LambdaBack;
                // Prox onto friction cone
                Prox::ProxFunction<ConvexSets::Cone3D>::doProxSingle(nodeData.m_contactParameter.m_params[CMT::muIdx],
                                                                     nodeData.m_LambdaFront);
            }
            else
            {
            ...
            LOGSLLEVEL3_CONTACT(m_pSolverLog,
                                "\t---> nd.m_LambdaBack: " << nodeData.m_LambdaBack.transpose() << std::endl);
            LOGSLLEVEL3_CONTACT(m_pSolverLog,
                                "\t---> nd.m_LambdaFront: " << nodeData.m_LambdaFront.transpose() << std::endl);
            if (Numerics::cancelCriteriaValue(
                    nodeData.m_LambdaBack, nodeData.m_LambdaFront, m_settings.m_AbsTol, m_settings.m_RelTol))
            {
            ...
            for (unsigned int i = 0; i < 2; i++)
            {
                if (nodeData.m_pCollData->m_pBody[i]->m_eMode == RigidBodyType::BodyMode::SIMULATED)
                {
                    uCache1 = nodeData.m_uBufferPtr[i]->m_front;

                    // Velocity update (wahrscheinlich Auslöschung bei Lambda)
                    nodeData.m_uBufferPtr[i]->m_front =
                        uCache1 +
                        nodeData.m_pCollData->m_pBody[i]->m_MassMatrixInv_diag.asDiagonal() * nodeData.m_W_body[i] *
                            deltaLambda;
                    ...
                    if (m_settings.m_eConvergenceMethod == InclusionSolverSettingsType::InVelocityLocal)
                    {
                        ...
                        {
                            nodeData.m_bConverged = Numerics::cancelCriteriaValue(
                                uCache1, nodeData.m_uBufferPtr[i]->m_front, m_settings.m_AbsTol, m_settings.m_RelTol);
                    ...
                    else if (m_settings.m_eConvergenceMethod == InclusionSolverSettingsType::InEnergyLocalMix)
                    {
                        if (m_globalIterationCounter >= m_settings.m_MinIter && m_bConverged)
                        {
                            nodeData.m_bConverged =
                                Numerics::cancelCriteriaMatrixNorm(uCache1,
                                                                   nodeData.m_pCollData->m_pBody[i]->m_MassMatrix_diag,
        ...
        */
            ...
            nodeData.m_contactParameter.m_contactModel == ContactModels::Enum::UCFD)
        {
            ...
            for (unsigned int i = 0; i < 2; i++)
            {
                ...
                {
                    uCache1 = nodeData.m_uBufferPtr[i]->m_front;
                    lambda_N += nodeData.m_W_body[i].transpose().row(0) * uCache1;
                }
            }
            ...
            for (unsigned int i = 0; i < 2; i++)
            {
                if (nodeData.m_pCollData->m_pBody[i]->m_eMode == RigidBodyType::BodyMode::SIMULATED)
                {
                    nodeData.m_uBufferPtr[i]->m_front =
                        uCache1 +
                        nodeData.m_pCollData->m_pBody[i]->m_MassMatrixInv_diag.asDiagonal() *
                            nodeData.m_W_body[i].col(0) * deltaLambda_N;
                }
            }
            ...
            for (unsigned int i = 0; i < 2; i++)
            {
                ...
                {
                    lambda_T +=
                        nodeData.m_W_body[i].transpose().template bottomRows<2>() * nodeData.m_uBufferPtr[i]->m_front;
                }
            }
            ...
            if (nodeData.m_contactParameter.m_contactModel == ContactModels::Enum::UCFD)
            {
                ...
                lambda_T += nodeData.m_LambdaBack.template tail<2>() *
                            (nodeData.m_contactParameter.m_params[CMT::d_TIdx] / m_settings.m_deltaT);
            }
            ...
            // Prox
            using CMT = typename CONTACTMODELTYPE(ContactModels::Enum::UCFD);
            Prox::ProxFunction<ConvexSets::Disk>::doProxSingle(
                nodeData.m_contactParameter.m_params[CMT::muIdx] * lambda_N, lambda_T);
    ...
    void operator()(TNode& node)
    {
        ...
        */
            ...
            nodeData.m_contactParameter.m_contactModel == ContactModels::Enum::UCFD)
        {
            ...
            using CMT = typename CONTACTMODELTYPE(ContactModels::Enum::UCFD);
            Prox::ProxFunction<ConvexSets::Disk>::doProxSingle(
                nodeData.m_contactParameter.m_params[CMT::muIdx] * nodeData.m_LambdaFront(0), lambda_T);
    ...
    public:
        ...
        inline void operator()(GeometryPtrType& g)
        {
            if (bodyNr == 1)
            {
                ...
                m_pNodeData->m_W_body[0].col(0).template tail<3>() =
                    -m_rotJacobi.m_I_JacobiT_rot * m_pCollData->m_cFrame.m_e_z;

                // T1 direction =================================================
                m_pNodeData->m_W_body[0].col(1).template head<3>() = -m_pCollData->m_cFrame.m_e_x;  // I frame
                m_pNodeData->m_W_body[0].col(1).template tail<3>() =
                    -m_rotJacobi.m_I_JacobiT_rot * m_pCollData->m_cFrame.m_e_x;

                // T2 direction =================================================
                m_pNodeData->m_W_body[0].col(2).template head<3>() = -m_pCollData->m_cFrame.m_e_y;  // I frame
                m_pNodeData->m_W_body[0].col(2).template tail<3>() =
                    -m_rotJacobi.m_I_JacobiT_rot * m_pCollData->m_cFrame.m_e_y;
            }
            else
            {
                // Set matrix size!
        ...
        inline void operator()(SphereGeomPtrType& g)
        {
            if (bodyNr == 1)
            {
                ...
                // N direction =================================================
                m_pNodeData->m_W_body[0].col(0).template head<3>() = -m_pCollData->m_cFrame.m_e_z;  // I frame
                m_pNodeData->m_W_body[0]
                    .col(0)
                    .template tail<3>()
                    .setZero();  ///< sepzial operation here: m_I_JacobiT_rot * contact normal = 0;
    ...
    void operator()(TNode& node)
    {
            ...
            nodeData.m_contactParameter.m_contactModel == ContactModels::Enum::UCFDD)
        {
            ...
            else if (state == RigidBodyType::BodyMode::ANIMATED)
            {
                ...
                GRSF_ASSERTMSG(
                    false, "RigidBody<TLayoutConfig>::ANIMATED objects have not been implemented correctly so far!");
            }
            ...
            for (unsigned int i = 0; i < 2; i++)
            {
                if (pCollData->m_pBody[i]->m_eMode == RigidBodyType::BodyMode::SIMULATED)
                {
                    ...
                    nodeData.m_uBufferPtr[i]->m_front += pCollData->m_pBody[i]->m_MassMatrixInv_diag.asDiagonal() *
                                                         (nodeData.m_W_body[i] * nodeData.m_LambdaBack);
                    /// + initial values M^⁻1 W lambda0 from percussion pool

                    nodeData.m_b += nodeData.m_eps.asDiagonal() * nodeData.m_W_body[i].transpose() *
                                    nodeData.m_uBufferPtr[i]->m_back /* m_u_s */;
                    nodeData.m_G_ii += nodeData.m_W_body[i].transpose() *
                                       pCollData->m_pBody[i]->m_MassMatrixInv_diag.asDiagonal() * nodeData.m_W_body[i];
                }
            }
            ...
            {
                nodeData.m_b(0) =
                    -1 * pCollData->m_overlap / m_settings.m_deltaT * 2.0 * m_settings.m_driftCorrectionGapAlpha;
            }
            ...
            else
            {
                ...
                {
                    nodeData.m_R_i_inv_diag(0) = m_alpha / nodeData.m_G_ii(0, 0);
                    nodeData.m_R_i_inv_diag.template tail<2>().setConstant(
                        m_alpha / nodeData.m_G_ii.diagonal().template tail<2>().sum());
                }
                else if (m_settings.m_RStrategy == InclusionSolverSettingsType::RSTRATEGY_MAX)
                {
                    // Take also offdiagonal values for lambda_N
            ...
            LOGSLLEVEL3_CONTACT(m_pSolverLog,
                                "\t ---> nd.m_b: " << nodeData.m_b.transpose() << std::endl
                                                   << "\t ---> nd.m_G_ii: "
                                                   << std::endl
                                                   << nodeData.m_G_ii
                                                   << std::endl
                                                   << "\t ---> nd.m_R_i_inv_diag: "
                                                   << nodeData.m_R_i_inv_diag.transpose()
                                                   << std::endl;);

            LOGSLLEVEL3_CONTACT(m_pSolverLog,
                                "\t ---> nd.m_mu: " << nodeData.m_contactParameter.m_params[CMT::muIdx] << std::endl;);
        }
        else
        {
            GRSF_ERRORMSG(" You specified a contact model which has not been implemented so far!");
...
private:
    ...
    inline void computeMaxOverlap(CollDataType* pColData)
    {
        if (pColData->m_pBody[0]->m_eMode == RigidBodyType::BodyMode::SIMULATED)
        {
            pColData->m_pBody[0]->m_pSolverData->m_overlapMax =
                std::max(pColData->m_pBody[0]->m_pSolverData->m_overlapMax, pColData->m_overlap);
        }
        if (pColData->m_pBody[1]->m_eMode == RigidBodyType::BodyMode::SIMULATED)
        {
            pColData->m_pBody[1]->m_pSolverData->m_overlapMax =
                std::max(pColData->m_pBody[1]->m_pSolverData->m_overlapMax, pColData->m_overlap);
        }
    }
...
#define DEFINE_CONTACT_GRAPH_VISITORS_AS_FRIEND    \
    template <typename TContactGraph>              \
    friend class SorProxStepNodeVisitor;           \
    template <typename TContactGraph>              \
    friend class ContactSorProxStepNodeVisitor;    \
    template <typename TContactGraph>              \
    friend class FullSorProxStepNodeVisitor;       \
    template <typename TContactGraph>              \
    friend class NormalSorProxStepNodeVisitor;     \
    template <typename TContactGraph>              \
    friend class TangentialSorProxStepNodeVisitor; \
    template <typename TContactGraph>              \
    friend class SorProxInitNodeVisitor;
