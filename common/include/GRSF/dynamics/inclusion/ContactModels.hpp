...
/*
    * @brief UnilateralAndCoulombFrictionContactModel: This is the friction model for a unilateral contact with spatial
 * coulomb friction.
    * Abreviations : UCF
    */
template <>
...
/*
    * @brief UnilateralAndCoulombFrictionDampedContactModel: This is the friction model for a unilateral contact with
...
/*
    * @brief UnilateralAndCoulombFrictionDampingDependendContactModel: This is the friction model for a unilateral
 * contact with spatial coulomb friction
    * where for each law a damping is added serially (coulomb damping d_N = mu lambdaN / gamma_max is defined by a given
 * maximal
...
struct ContactModel<EnumConversion::toIntegral(Enum::UCFDD)>
{
    ...
    static const int nDOFFriction    = 2;
    static const int nFrictionParams = 1;  // mu
    static const int nDampingParams =
        4;  // d_N normal and gamma_max tangential and epsilon and d_Tfix in the case where lambda_N <= epsilon
...
{
    return (e == Enum::U)
               ? CONTACTMODELTYPE(Enum::U)::ConvexSet::Dimension
               : ((e == Enum::UCF)
                      ? CONTACTMODELTYPE(Enum::UCF)::ConvexSet::Dimension
                      :

                      ((e == Enum::UCFD)
                           ? CONTACTMODELTYPE(Enum::UCFD)::ConvexSet::Dimension
                           : ((e == Enum::UCFDD)
                                  ? CONTACTMODELTYPE(Enum::UCFDD)::ConvexSet::Dimension
                                  : (
                                        /*(e==Enum::UCCF)?*/
                                        +CONTACTMODELTYPE(
                                            Enum::UCCF)::ConvexSet::Dimension  //(make an lvalue to rvalue conversion
                                                                               //(all other static variables inherit
                                                                               // this behaviour) with the unary+
                                        // operator, otherwise linking errors)
