...
inline bool collideSpherePointAndProx(const MatrixBase<Derived>& I_center, PREC radius, Vector3& I_p)
{
    // d = c1 - p;
...
template <typename Derived1, typename Derived2>
bool collideCapsulePointAndProx(
    const MatrixBase<Derived1>& I_pos, const MatrixBase<Derived2>& I_normal, PREC length, PREC radius, Vector3& I_p)
{
    ...
    for (faceIt = mesh.m_Faces.begin(); faceIt != mesh.m_Faces.end(); ++faceIt)
    {
        ...
        if (overlap >= 0.0 && normI_r_SC > 0.0)
        {
            ...
            for (unsigned int j = 0; j < pointSet.size(); j++)
            {
                ...
                // std::cout <<"Cos : "<< acos( pointSet[j].get<1>().dot( I_r_SC )) << "<" <<
                // (5.0/180.0*M_PI)<<std::endl;
...
// do: http://www.geometrictools.com/Documentation/DistancePoint3Triangle3.pdf However it's heavy on math and does not
// give the complete source code. Here is the complete source code:
