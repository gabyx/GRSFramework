...
public:
    ...
    template <typename TMatrix>
    void diameterTest(
        unsigned int N, const TMatrix& v, bool dump = true, unsigned int optLoops = 10, PREC epsilon = 0.001)
    {
        using namespace PointFunctions;
        using namespace TestFunctions;
...
public:
                  ...
                  unsigned int gridSearchOptLoops = 10)
    {
        ...
        START_TIMER(start)
        auto oobb = ComputeMVBB::approximateMVBB(v, eps, 400, 5, mvbbDiamOptLoops, gridSearchOptLoops);
        STOP_TIMER_SEC(count, start)
        std::cout << "Timings: " << count << " sec for " << v.cols() << " points" << std::endl;
