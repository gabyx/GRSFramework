...
class Range
{
    ...
    class iterator : public std::iterator_traits<typename RangeType::iterator>
    {
        ...
        iterator(const typename RangeType::iterator& it) : m_it(it), m_diffValue(0)
        {
        }

        iterator(const typename RangeType::iterator&& it) : m_it(std::move(it)), m_diffValue(0)
        {
        }

        void setDiffValue(IntType v)
        {
            m_diffValue = v;
        }
        IntType getCurrentDiff()
        {
            return *m_it - m_diffValue;
        }
...
class Range
{
    ...
    // Constructor for std::vector
    Range(const std::vector<Type>& v) : m_v(v)
    {
        sort();
    }
    Range(std::vector<Type>&& v) : m_v(std::move(v))
    {
        sort();
    }
    ...
    class iterator
    {
        ...
        iterator(const typename std::vector<Type>::iterator&& it) : m_it(std::move(it)), m_diffValue(0)
        {
        }

        void setDiffValue(Type v)
        {
            m_diffValue = v;
        }
        Type getCurrentDiff()
...
class Range : public std::vector<Type>
{
    ...
    // Constructor for std::vector
    Range(const std::vector<Type>& v) : std::vector<Type>(v)
    {
        sort();
    }
    Range(std::vector<Type>&& v) : std::vector<Type>(std::move(v))
    {
        sort();
    }
    ...
    Range& operator=(const std::vector<Type>& v)
    {
        std::vector<Type>::operator=(v);
        sort();
    }
    Range& operator=(std::vector<Type>&& v)
    {
        std::vector<Type>::operator=(std::move(v));
        sort();
    }
    ...
    iterator begin()
    {
        return iterator(std::vector<Type>::begin());
    }
    iterator end()
    {
...
class Range
{
    ...
    Range(Range const& r)
        : m_which(r.m_which)
        , m_ptr(r.m_which == 0 ? static_cast<void*>(new Pair(*static_cast<Pair*>(r.m_ptr)))
                               : static_cast<void*>(new Set(*static_cast<Set*>(r.m_ptr))))
    {
    }

    // Move Constructor (rip the guts of temporary r)
    Range(Range&& r) : m_which(r.m_which), m_ptr(r.m_ptr)
    {
        r.m_ptr = nullptr;
    }
    ...
    Range& operator=(Range&& r)
    {
        assert(r.m_ptr);
        if (this != &r)
        {  // Prevent self-assignment ( each object has a different m_ptr, it cannot happen that two
            // objects have the same m_ptr)
            deleteResources();
