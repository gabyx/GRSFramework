...
class ConvexHull2D
{
    ...
    void compute()
    {
        ...
        auto d2 = moveConsecutiveToBackIf(indices.begin() + 1, indices.end(), [&](unsigned int a, unsigned int b) {
        ...
        while (i < indices.size())
        {
            ...
            if (areaSign(m_p.col(lPtIdx), m_p.col(mPtIdx), m_p.col(currIdx)) > 0)
            {  // if we have a left turn!
                // std::cout << "Left turn: c:" << currIdx << " m:" << mPtIdx << " l: "<< lPtIdx << " sign: " <<
                // areaSign( m_p.col(lPtIdx), m_p.col(mPtIdx), m_p.col(currIdx)) << std::endl;
            ...
            {  // if we have a right turn
                // std::cout << "Right turn: c:" << currIdx << " m:" << mPtIdx << " l: "<< lPtIdx << " sign: " <<
                // areaSign( m_p.col(lPtIdx), m_p.col(mPtIdx), m_p.col(currIdx)) << std::endl;
                m_indicesCH.pop_back();  // remove middle point;
                // move triplet back if possible
                if (m_indicesCH.size() <= 1)
                {  // Degenerate Case if we come back to the begining
    ...
    bool verifyHull()
    {
        ...
        if (m_indicesCH.size() > 2)
        {
            ...
            {
                if (!(areaSign(m_p.col(m_indicesCH[i - 2]), m_p.col(m_indicesCH[i - 1]), m_p.col(m_indicesCH[i])) >= 0))
