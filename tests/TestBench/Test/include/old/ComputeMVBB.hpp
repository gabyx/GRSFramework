...
void samplePointsGrid(Matrix3Dyn& newPoints, const MatrixBase<Derived>& points, const unsigned int nPoints, OOBB oobb)
{
    ...
    for (unsigned int i = 0; i < halfSampleSize; ++i)
    {
        if (topPoints[i] != 0)
        {
            ...
            newPoints.col(k++) =
                points.col(topPoints[i] - 1);  // A_KI.transpose()*(oobb.m_minPoint + a.matrix()).eval() ;
            if (topPoints[i] != bottomPoints[i])
            {
                // Array3 a(i % gridSize,i/gridSize,0);
                // a.head<2>()*=dxdyInv.inverse();
                newPoints.col(k++) =
                    points.col(bottomPoints[i] - 1);  // A_KI.transpose()*(oobb.m_minPoint + a.matrix()).eval() ;
            }
        }
    }
    // Add random points!
...
*/
template <typename Derived>
OOBB optimizeMVBB(const MatrixBase<Derived>& points, OOBB oobb, unsigned int times = 10)
{
...
*/
template <typename Derived>
OOBB approximateMVBBDiam(const MatrixBase<Derived>& points, const PREC epsilon, const unsigned int optLoops = 10)
{
    EIGEN_STATIC_ASSERT_MATRIX_SPECIFIC_SIZE(Derived, 3, Eigen::Dynamic)
