...
void serializeKdTree()
{
    ...
    PREC minExtent = 5e-3;
    tree.initSplitHeuristic(
        std::initializer_list<SplitHeuristicType::Method>{
            SplitHeuristicType::Method::MEDIAN,
            /*SplitHeuristicType::Method::GEOMETRIC_MEAN,
                    SplitHeuristicType::Method::MIDPOINT*/
        },
            ...
            if (n->isLeaf())
            {
                ...
                for (auto* b : n->getBoundaries())
                {
                    if (b && *itO)
                    {
                        ...
                        {
                            GRSF_ERRORMSG("Boundary wrong for idx: " << n->getIdx() << " new idx: " << b->getIdx()
                                                                     << "and orig: "
                                                                     << (*itO)->getIdx())
                        }
                    }
                    ...
                    {
                        GRSF_ERRORMSG("Boundary wrong for idx: " << n->getIdx() << b << "or" << (*itO))
                    }
                    ++itO;
                }
            }
    ...
    for (auto& pa : serClasses)
    {
        ...
        for (auto i : pa.second)
        {
            ...
            {
                GRSF_ERRORMSG("Serialized version leaf idx: " << pa.first << "contains point idx: " << i
                                                              << " but orig not!")
            }
        }
    }
