...
void runTest() {
    ...
    sort(idToidxVec.begin(), idToidxVec.end(),
...
//    }
...

    {
        ...
        for(int i = 0;i<itEnd;i++) {
           auto id = sortedIds[i];
           auto it = std::find_if(idToidxVec.begin(),idToidxVec.end(),[&id](const std::pair<unsigned int, unsigned int>& v){return v.first == id;});
           if(it==idToidxVec.end()){
            std::cout << "FAIL" << std::endl; exit(-1);
           }
           n+= vec[it->second].id;
        }
