...
public:
    ...
    template<typename...T>
    Edge(std::size_t i, T &&... t)
        : EdgeBase(EdgeDataType::enumType), m_counter(i), m_data( std::forward<T>(t)... ) {
    }

    void setStart(TNodeStart * p){ m_startNode = p;}
    void setEnd(TNodeEnd * p){ m_endNode = p;}
    void setTwin(TEdgeTwin * p){ m_twinEdge = p;}

    TNodeStart * getStart(){ return m_startNode;}
    TNodeEnd * getEnd(){ return m_endNode;}
    TEdgeTwin * getTwin(){ return m_twinEdge;}
...
public:
    ...
    template<typename...T>
    Node(std::size_t i, T &&... t)
        : NodeBase(NodeDataType::enumType), m_counter(i), m_data( std::forward<T>(t)... ) {
    }
...
private:
    ...
    // Take care, if you want to switch this to a value-type all returned pointer in insertNode are invalidate if reallocation happens
...
namespace CGTraitsDetails{
    ...
    struct mplToTuple{
        using type = typename mpl::fold< Result, std::tuple<>,
            details::ToStdTuple< mpl::_1, mpl::_2 > >::type;
    };


    template<template<typename> class Wrap, typename T> struct makeTupleWrap;
...
struct ContactGraphTraits< std::tuple<TNodaData...>, std::tuple<TEdgeData...> > {
    ...
    */
    ...
    /** mpl::vector */
    using UniqueEdgeTypes        = typename mpl::fold<
                                        typename details::unique_edge_types
                                        , mpl::vector<>
                                        , mpl::push_front<mpl::placeholders::_1, mpl::placeholders::_2> /** _1 = State = set<...>, _2 element of NodeTypes*/
                                        >::type;

    using UniqueEdgeStorageTuple = typename CGTraitsDetails::makeTupleWrap<EdgeStorage,
                                                                                        typename CGTraitsDetails::mplToTuple<
                                                                                            UniqueEdgeTypes
                                                                                        >::type
                                                                           >::type;
    ...
    */
    ...
    struct edgeTypeByNodeType {
        ...
        using edgetype = typename mpl::at_c<EdgeTypes,linearIdx>::type; // type in map -> (node i, node j) -> edge type
    ...
    struct edgeDataByNodeData {
        using res =  edgeDataByEdgeType<
                                typename edgeTypeByNodeType< typename nodeTypeByNodeData<TNodeData1>::type,
                                                    typename nodeTypeByNodeData<TNodeData2>::type
                                                  >::type
                                >;
    ...
    struct nodeTypesByIndex{
        ...
        static const unsigned int m = mpl::divides< mpl::int_<Idx> ,
                                                    mpl::int_<mpl::size<NodeTypes>::type::value-1>  >::type::value;

        static const unsigned int i = (mpl::size<NodeTypes>::type::value-1 < m)? mpl::size<NodeTypes>::type::value-1 : m;
    ...
    struct makeEdge{
        ...
        struct apply{
           using type = Edge< T1 ,
                      typename nodeTypeAByIndex< T2::value >::type,
                      typename nodeTypeBByIndex< T2::value >::type
                       >;
        };
    };


    using EdgeTypes2  = typename mpl::transform<    EdgeDataTypes,
                                                    mpl::range_c<int, 0, mpl::size<EdgeDataTypes>::type::value >,
                                                    makeEdge
                                                >::type;

    using EdgeTypes3 = typename mpl::fold< EdgeTypes2
                        , mpl::vector<>
                        , mpl::push_front<mpl::placeholders::_1, mpl::placeholders::_2> /** _1 = State = set<...>, _2 element of NodeTypes*/
                        >::type;
    /** Experiment to inject the start end node into the edgetypes */
...
public:
    ...
    constexpr std::size_t sizeOfNodeStorages(){return std::tuple_size<NodeStorageTuple>::value;};
    constexpr std::size_t sizeOfEdgeStorages(){return std::tuple_size<EdgeStorageTuple>::value;};
...
void contactGraphTest() {
    ...
    using Traits = ContactGraphTraits<  std::tuple< NodeDataA , NodeDataB >,
                                        std::tuple< EdgeDataAA, EdgeDataAB, EdgeDataBB>
          >;
...
//    g.reserveNodes<NodeB>(size);
    ...
    std::cout << "Visitor took: " << count << " ms" << " counter: " << v.c <<  std::endl;
    ...
    std::cout << "visitor took: " << count2 << " ms" << " counter: " << v.c <<  std::endl;
