...
void kNearstNeighboursTest()
{
    ...
    int loops = 1000;
    {
        ...
        for (int l = 0; l < loops; l++)
        {
            ...
            // sort others (all)
            std::partial_sort(
                others.begin(), (k >= others.size()) ? others.end() : std::next(others.begin(), k), others.end());
            // merge (inplace, first insert at end, then merge inplace)
            auto s = prio.size();

            prio.insert(std::next(prio.begin(), s),
                        others.begin(),
                        (k >= others.size()) ? others.end() : std::next(others.begin(), k));
        ...
        std::cout << "Merge " << k << " smallest of " << n << " elements  into list: " << totalT / loops << "sec."
                  << std::endl;
    ...
    }
    {
        ...
        std::cout << "Merge " << k << " smallest of " << n << " elements  into vector: " << totalT / loops << "sec."
                  << std::endl;
        std::cout << "result: " << std::endl;
        for (auto i : prio)
        {
    ...
    }
    {
        ...
        std::cout << "Merge " << k << " smallest of " << n << " elements  into priority_queue: " << totalT / loops
                  << "sec." << std::endl;
        std::cout << "result: " << std::endl;
