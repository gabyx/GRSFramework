...
#define CALCULATE_DIAGDOM_OF_G \
    0  ///< {0,1} Set if the diagonal dominant criteria is calculated, the number shows how many rows are not diagonal
       /// dominant!
...
/* @{ */
#ifndef NDEBUG
#define DESTRUCTOR_MESSAGE LOG(m_pSimulationLog, "Destructor: " << typeid(*this).name() << " , @ : " << this;);
#define CONSTRUCTOR_MESSAGE LOG(m_pSimulationLog, "Constructor: " << typeid(*this).name() << " , @ : " << this;);
#else
