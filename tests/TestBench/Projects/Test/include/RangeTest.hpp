    ...
    public:
        ...
        */
        ...
        bool operator==(const iterator &rhs) {
            if(m_listPtr) {
                if(rhs.m_listPtr == m_listPtr) { // if sequences are not the same
                    return m_listIt  == rhs.m_listIt;
                }
            } else if(m_rangePtr) {
                if(rhs.m_rangePtr == m_rangePtr ) { // if sequences are not the same
...
#define STOP_TIMER(name)  \
    double count = std::chrono::duration<double,std::milli>(  \
