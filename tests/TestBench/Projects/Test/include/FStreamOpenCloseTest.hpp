...
//            //Calculate timing
//            fsec time = (t2 - t1);
//            output1 << bufL << "\t" << time.count() <<"\t" << (bufL/(time.count()/(nFiles*writeNTimes))) / (1024*1024) << std::endl;
//        }
        ...
        for(int i = 0 ; i<nFiles; i++){
            ...
            //Calculate timing
            fsec time = (t2 - t1);
            output2 << bufL << "\t" << time.count() <<"\t" << (bufL/(time.count()/(nFiles*writeNTimes))) / (1024*1024) << std::endl;
        }
