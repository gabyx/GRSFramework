...
struct JorProxGPUVariantSettings{
   ...
   struct JorProxGPUVariantType{
      typedef typename JorProxGPUVariant< JorProxGPUVariantSettingsWrapper<_PREC,_VariantId,_alignMatrix,_bAbortIfConverged, _nCheckConvergedFlag> >  TGPUVariant;
   };
...
int main(){
  ...
  //typedef double PREC;
  //Eigen::Matrix<PREC,Eigen::Dynamic, Eigen::Dynamic,Eigen::ColMajor> T_internal(3+2,3); // We save row-major which is T' in col-major interpretation which makes cuBlas faster!
  // T_internal.setRandom();
  // //Eigen::Matrix<PREC,Eigen::Dynamic, Eigen::Dynamic>  & T =  T_internal.block(0,0,3,3); // Copied! Stupidloo
  // Eigen::Block< Eigen::Matrix<PREC,Eigen::Dynamic, Eigen::Dynamic,Eigen::ColMajor> ,Eigen::Dynamic, Eigen::Dynamic>  T = T_internal.block(0,0,3,3);
  // T(0,0)=1000;
  // func2(T);
  // func2(T_internal);
  ...
  // //cout << int(Eigen::Matrix<PREC,Eigen::Dynamic, Eigen::Dynamic,Eigen::RowMajor>::Flags & Eigen::RowMajorBit)<<endl;
