...
inline void save_construct_data(Archive& ar, const int* foo, const unsigned int file_version)
{
    ar << *foo;
}

template <class Archive>
inline void load_construct_data(Archive& ar, int* foo, const unsigned int file_version)
{
    if (!foo)
    {
...
void testSerializeVariant()
{
    ...
    std::cout << "Variant to serialize which:" << var.which() << " : std::string*: " << boost::get<std::string*>(var)
              << *(boost::get<std::string*>(var)) << std::endl;
    // save data
