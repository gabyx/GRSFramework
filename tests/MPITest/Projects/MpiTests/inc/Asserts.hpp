...
namespace debug {
	...
	*/
	 #define GRSF_ASSERTMSG(condition,message) { if(!(condition)){ std::cerr << "ASSERT FAILED: " << #condition << " @ " <<std::endl<< message << std::endl << __FILE__ << " (" << __LINE__ << ")" << std::endl; abort();} }
   ...
   #define GRSF_ERRORMSG(message) { std::cerr << "ERROR :  @ " <<std::endl<< message << std::endl << __FILE__ << " (" << __LINE__ << ")" << std::endl; abort(); }
