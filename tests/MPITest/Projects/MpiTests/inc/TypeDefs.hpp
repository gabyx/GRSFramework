...
*			- \b TLayout:
*				The layout can be choosed statically or dynamically. Dynamically means that NDOFq and NDOFu = -1 which
*is
*used for theater
*				matrix library which then creates dynamical allocated space. Normally we use a Dynamic layout because we
*dont know yet how many bodies
*				are simulated.
...
*/
typedef LayoutConfig<double, DynamicLayout<7, 6>> DoubleDynamicLayout;
...
typedef ConfigTimeStepper<MyDynamicsSystem,
                          MyCollisionSolver,
                          MyInclusionSolver,
                          StatePoolVisBackFront<DoubleDynamicLayout>>
    MyConfigTimeStepper;
...
*/
#define DEFINE_CONFIG_TYPES_OF(_ConfigName_) DEFINE_SOLVER_CONFIG_TYPES_OF(_ConfigName_::SolverConfigType)
...
/**
* @brief This macro is used to typedef all template arguments in a class with e.g template argument typename
* â€œLayoutConfigâ€
*/
