...
template <typename TLayoutConfig>
const char MultiBodySimFile<TLayoutConfig>::m_simHeader[SIM_FILE_SIGNATURE_LENGTH] = SIM_FILE_SIGNATURE;
...
void MultiBodySimFile<TLayoutConfig>::writeHeader()
{
    ...
    *this << (unsigned int)m_nSimBodies << (unsigned int)NDOFqObj
          << (unsigned int)NDOFuObj;  // Precision output is always double!
...
template <typename TLayoutConfig>
MultiBodySimFile<TLayoutConfig>& MultiBodySimFile<TLayoutConfig>::operator>>(
    boost::shared_ptr<DynamicsState<TLayoutConfig>>& state)
{
        ...
        *this >> nBodies >> nDofqObj >> nDofuObj;
        ...
        else
        {
            m_errorString << "Binary file does not correspond to the number of bodies which should be simulated: "
                          << std::endl
                          << " Binary File describes: \tnSimBodies = " << nBodies << "\tnDofqObj = " << nDofqObj
                          << "\tnDofuObj = " << nDofuObj << std::endl
                          << " Simulation requests: \t\tnSimBodies = " << m_nSimBodies << "\tnDofqObj = " << NDOFqObj
                          << "\tnDofuObj = " << NDOFuObj << std::endl;
        }
    }
    else
    {
        m_errorString << "Binary file contains a wrong header and is not equal to: '" << m_simHeader << "'"
                      << std::endl;
    }
